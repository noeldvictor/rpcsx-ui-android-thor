...
- Current refiner/atlas next action: extend the newest valid `loader-control-left200x2` base by exactly one tiny `diag200` micro-pulse with `CleanAfterField`; keep lane-2 HLE/GPU promotion blocked until field, Options/menu, and first-battle visuals are valid.
