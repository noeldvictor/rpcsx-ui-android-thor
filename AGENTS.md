...
- Current refiner next action: run no-movement `cpu4-loader-control-visualgate-windows` with `CleanAfterField` before any TopSlot movement, BattleRoute retry, HLE/GPU fast mode, speed claim, or 200% promotion. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
