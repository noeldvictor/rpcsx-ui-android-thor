...
- Current refiner/atlas next action: latest run black-overlayed before accepted field. Re-prove the clean `loader-control-left200x2` boundary with `CleanAfterField` before adding movement again; keep lane-2 HLE/GPU promotion blocked until field, Options/menu, and first-battle visuals are valid.
