...
- Current refiner/atlas next action: the `left400` bridge recovered from the `left800` window-loss boundary, but the refiner still blocks lane-2 HLE/GPU promotion until first-battle route repair/proof under `Verify25ccShadow` with body/skip/GPU modes off.
