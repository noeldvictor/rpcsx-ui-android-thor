...
- Current refiner next action: back off from the latest non-field/cutscene route and re-prove the last clean `loader-control-left200x2-confirm` boundary with `CleanAfterField`; keep diagonal, first-battle, HLE/GPU, and lane-2 fast modes blocked until that field proof is clean again.
