...
- Current refiner next action: use `20260528-090516` as the route base and add exactly one small state-aware `left200` movement step with `CleanAfterField`. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
