...
- Current refiner/atlas next action: use the latest clean `left200x2+diag200` field route as the base and add only one small state-aware movement step with `CleanAfterField`. The `left800` bridge attempt lost/exited the RPCS3 window at `129s`, so do not promote lane-2 HLE/GPU paths from those clean counters.
