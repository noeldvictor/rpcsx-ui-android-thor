...
- Before trusting an Eternal Sonata FPS result, inspect `config/custom_configs/config_BLUS30161.yml`. A stale full custom config can override Thor defaults back to `Max SPURS Threads: 6`, `Thread Scheduler Mode: Operating System`, `SPU Reservation Busy Waiting Percentage: 0`, `Accurate SPU Reservations: true`, `SPU Verification: true`, and `Accurate ZCULL stats: true`. Use `.\tools\push_eternal_sonata_thor_profile.ps1` to back it up and replace it with the managed Thor profile before retesting.
