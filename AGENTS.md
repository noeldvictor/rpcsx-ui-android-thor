...
- Current refiner next action: back off from the TopSlot cutscene/non-field route and re-prove the last clean loader-control-left200x2 boundary with `CleanAfterField` before diagonal, battle, HLE/GPU, speed, or 200% promotion. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
