...
- Transformers state on 2026-09-08 (rounds K to M, `docs/arm64/transformers-30fps.md`): restored combat runs 19.8 to 20.1 FPS across six controls in one day. Closed with measurements: `PPU Threads` 4 and 8 (minus 19 and 22 percent), the async compute scheduler and usleep-only timers (null), every explicit thread placement (SPUs on cpu0-2: 6.4 FPS; SPUs on cpu0-3: 13.1; PPU on cpu3-7 with RSX on the X3 and SPUs free: 13.2; the OS scheduler wins), Multithreaded RSX (twice). SPU work is on the frame's chain: the only SPU threads are the six SPURS kernels, and the frame rate follows the CPU they get. The FIFO levers engaged and moved no frames: the trim fix, the ARMSX3 4 KB refill (refills 830 to 215 a frame), GET publish lag, all three together; all stay properties, default off. The RSX FIFO fetch retries about 400 times a frame whatever the fetch size or PUT distance, at 10.4 us per retry (`busy_wait(200)` is 200 raw ticks here, two ticks upstream), 4 ms of the RSX thread per frame; round N splits the retries by cause and measures the backoff. SPU DMA PUT on ARM64 locks and bumps the reservation of every 128-byte line it writes (no TSX path). Real render passes: 98 a frame for 1,500 draws (the round K figure of 1,550 counted calls before an early-out). Instruments added: per-class affinity masks, RSX counters on the Frames line (refills, retries by cause, render passes and what ended them), a SPURS workload census (`debug.rpcsx.thor.spurs_wkl_census=1`), a sampled FIFO retry log. The `rsx_auditor` is a compile-time option (`-PrpcsxThorRsxAuditor=1`); an arm that sets its property on a normal build measures nothing. Next: round N results, then stage 2 (JIT perf-map profile of the render thread and the six SPUs, Ghidra names for the wait sites and the SPURS workloads). Do not spend device time on placement or HLE SPURS for frame rate.
