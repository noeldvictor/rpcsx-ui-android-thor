...
- Current verify log-row parser: `tools/parse_spu_contract_verify_log.ps1`. It accepts only `Eternal Sonata SPU contract verifier` rows, validates required fields/contract id/hit split/mismatch/overflow, and always reports `promotion_ready=false` because visuals/fatal-log gates are external.
