...
- Current refiner next action: use the newest valid-field run as the route base and run only `cpu4-stateaware-one-step-visualgate-windows` with `CleanAfterField`. Do not extend loader-control left200x2, diagonal, battle, HLE/GPU fast mode, speed claim, or 200% promotion until a valid field boundary is re-established. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
