...
- Imported slice 2026-05-12: VM reservation waiters now follow upstream-style reservation-time buckets. Waiters are keyed by address plus rtime parity, `reservation_update()` notifies the old and previous reservation timestamps, SPU/PPU reservation notify call sites pass concrete rtime values, and Android/Linux `SPU_RdEventStat` waits on the registered wait flag instead of assuming `1`. This replaces the older address-only notifier path after the Eternal Sonata `Now Loading` hang showed five TCX SPURS kernels waiting at SPU PC `0x11a8` on `MFC_LLR_LOST_EVENT` (`mask=0x400`) for `spurs_addr=0x8aa280`. Build/install succeeded on Thor; validate boot/load before stacking more SPURS scheduler changes.
- Guardrail from 2026-05-12: the earlier direct rtime notifier attempt crashed with deterministic `SIGBUS BUS_ADRALN` in `PPU[0x1000000]` at PC/fault `0xd0020d77` because it included unsafe SPU PUTLLC / 128-byte store parity changes. The active retry intentionally ports the notifier/wait generation behavior without that partial-line store rewrite. If `SIGBUS` returns, capture reservation address, rtime, waiter bucket/index, old/new hashes, and GETLLAR/PUTLLC loop identity before changing wakeup or store behavior again.
...
- A later 2026-05-12 Ghidra/SPURS stuck capture (`debug-captures/20260512-220623-eternal-sonata-stuck-ghidra-spurs`) showed the app alive with two SPURS groups: single-SPU `CellSpursKernelGroup` at `spurs=0x9d4d80` and five-SPU `TCX_CellSpursKernelGroup` at `spurs=0x8aa280`. Both had zero ready/current/pending work while `handlerDirty` and `signal1=0x8000` toggled. The five-SPU group produced repeated `Thor SPURS wait probe` samples at SPU PC `0x11a8`, `raddr=spurs_addr=0x8aa280`, `mask=0x400`, with LR/notifier waits rising and no PUTLLC waits/timeouts. Treat this as reservation wait/wake generation first; the rtime-aware notifier port is the active fix attempt.
