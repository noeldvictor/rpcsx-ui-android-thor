...
- Current refiner next action: do not rerun field or Options; run the reservation-loop first-battle TopSlot proof/repair with `BattleRoute` visual gate. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
