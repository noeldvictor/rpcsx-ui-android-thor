...
- Latest corrected-contract runtime proof `20260715-220332-cpu4-verify25cc-e379fba-extendedkey-first-battle-windows` is a valid moving-field counterproof: correct field/animation through `185s`, five external-clean host snapshots, zero targeted fatal/draw/access/Vulkan/device-lost/assertion hits, and strict verifier `732/732` accepted with `1878` hits and mismatch/overflow `0`. Scripted screenshot labels did not make it a battle; exact rebuilt Options/menu and real first-battle proofs remain required.
...
- 0x25cc descriptor/shadow verifier has clean current-format field and older Options/menu counterproofs with zero 25cc mismatches and overflow `0`. The exact source-aligned rebuilt binary now has clean moving-field proof but has not yet re-proved Options or entered battle. Treat this as verifier coverage only, not bodyfast/codegen promotion.
