...
- Current refiner next action: latest run had fatal/crash log evidence; do not extend it. Re-prove the newest clean `loader-control-left200x1` boundary with `CleanAfterField` before adding another pulse. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
