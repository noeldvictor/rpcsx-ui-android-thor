...
- Current refiner next action: do not rerun the state-aware field proof; isolate TopSlot post-field movement again with `cpu4-reservation-loop-topslot-leftonly-diagnostic-windows` before another BattleRoute retry. Do not run loader-control left200x2, diagonal, battle, HLE/GPU fast mode, speed claim, or 200% promotion until this route branch is clean. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
