...
- Current refiner next action: do not repeat `loader-control-left200x2` or the exact `0xa1c000` skip. Next useful step is a clean-route pattern/descriptor verifier for the top `0x9e4000` groups, or SPU codegen-dispatch analysis. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
...
- Broader `0x9e4000` 0x25cc pattern groups are the better CPU-pressure candidate: latest runtime-family sizing shows `437.30 MB` across `10` repeated groups, but still `0 B` RSX-local and not a proof because the source run fataled.
