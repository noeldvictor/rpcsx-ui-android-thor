...
- Current refiner next action: do not repeat `loader-control-left200x2`; it failed twice in the recent window. Repair route control or switch to focused SPU kernel HLE/codegen/verifier analysis before another movement run. Lane-2 HLE/GPU fast modes remain blocked until field, Options, and first-battle visuals are all valid.
