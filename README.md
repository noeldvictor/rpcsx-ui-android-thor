...
- **Launching a second game after closing the first can fail.** Cause found
  ...
  before the fix. What is confirmed: the core reaches Stopped in about a
  second and no emulator thread remains. The second boot into the same
  process was accepted but its render was not logged, because the thermal
  guard ended that run.
