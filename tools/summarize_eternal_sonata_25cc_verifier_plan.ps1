...
$lines.Add("")
$lines.Add("- Broad ``0x9e4000`` family coverage: ``$(Format-Bytes $totalBytes)`` across ``$totalRecords`` records, ``$($familyRows.Count)`` pattern rows, ``$($repeatRows.Count)`` repeated pattern rows, and ``$(Format-Bytes $totalRsxBytes)`` RSX-local bytes.")
...
$lines.Add("")
$lines.Add("The first code change should be verify-only and should not reuse the exact ``0xa1c000`` skip predicate as the main gate. Gate the broad family by:")
$lines.Add("")
...
$lines.Add("- main-memory EA with ``eah == 0`` and runtime family ``0x9e4000``")
...
$lines.Add("- pattern signature and command-count bucket so reports line up with ``20260526-25cc-pattern-family.csv``")
$lines.Add("")
$lines.Add("## Implementation Reading")
$lines.Add("")
$lines.Add("- Runtime code already has the useful family classifier and counters; extend that lane first.")
