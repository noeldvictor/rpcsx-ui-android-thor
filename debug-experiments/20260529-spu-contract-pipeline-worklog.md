...
Evidence:
- Followed the refiner's fatal-log guidance to back off from first-battle movement and re-prove the clean boundary. Ran `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-stable-loader-control-25cc-contract-visualgate -EternalSonataGpuProbe Profile -EternalSonataSpuHleVerify Verify25ccShadow -WindowsVisualGate CleanAfterField` with a custom no-movement macro that used `gate_title_menu`, `gate_load_target_stable`, `gate_load_complete`, and `gate_field`, then held field for 60 seconds.
- Run dir: `debug-captures\windows-lab\20260724-174440-cpu4-stable-loader-control-25cc-contract-visualgate-windows`.
- Title-menu gate passed at 48s. Stable load target gate initially saw settling/non-target frames, then confirmed two consecutive `PATH_TO_TENUTO_PRESENT` frames at 66s and 68s. Load-complete gate passed at 82s. Field gate passed at 84s.
- Visual gate status: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0084s-path-to-tenuto-field-gate-2.png` at 84s, required field-like by 120s passed, field-like frames persisted through the 30s/60s holds and late screenshots, invalid screenshots after first field-like = 0.
- Fatal scan found 0 targeted fatal/access/Vulkan/assertion/verifier-mismatch hits. This directly contrasts the previous first-battle route, which faulted at guest `0x002aedd0` after post-field movement.
- Strict contract parser was refreshed into `spu-contracts\BLUS30161\latest-verify-logrow-results.*`: rows=77, accepted_rows=77, rejected_rows=0, total_contract_hits=174, total_contract_bytes=2850816, total_contract_rejects=2439, total_output_mismatch=0, total_desc_overflow=0, strict_gate_pass=true, promotion_ready=false.
- Host contention summary was `moderate` because one sample hit CPU >=45%; external contention stayed clean. Do not use this for timing/FPS comparison.
- GPU probe summary again reported no `Eternal Sonata GPU/DMA candidate probe` records and no RSX/offload evidence under this verifier mode; PUTLLC16 analyzer still found 7 detected patterns and 11 breakage rows.

Result:
- Advances CPU/SPU bottleneck relief: yes, as route/control evidence. The 25cc/9e4000 contract verifier survives quick stable-load field entry plus a 60-second no-movement field hold with clean parser and no fatal.
- Advances GPU-feed/offload: no. This remains a CPU/SPU HLE/codegen verification lane; GPU fast paths stay parked.
- Speed increase: no. Body mode was disabled, host grade was not clean enough for timing, and there is still no Options/menu + first-battle A/B proof.

Next narrow hypothesis:
- Add exactly one post-field movement bisection before retrying first-battle proof: screenshot at field, run only the first movement segment, screenshot/hold, and scan for the `0x002aedd0` VM fault. If clean, add the second movement segment in a later run; if it faults, route repair should avoid that movement vector before any 25cc body/codegen promotion.
...
- Harness change: `tools\windows_rpcs3_lab.ps1` now supports stable load-target gate tokens `gate_load_target_stable`, `stable_load_target_gate`, and `assert_load_target_stable`. Stable mode requires `2` consecutive `PATH_TO_TENUTO_PRESENT` classifier frames and continues polling through transient `DEBUG_SAVE_PROLOGUE_PRESENT`, `MIXED_LOAD_TARGETS`, or `DAMAGED_SAVE_TARGET` until timeout instead of immediately confirming or failing on a single frame.
- Validation command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-loadlist-stable-path-gate-twoframe-target-diagnostic -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataGpuProbe Profile -WindowsVisualGate Off -InputMacro "wait:65000;shot:title-settle-before-stablegate;down:160;wait:900;shot:title-after-down160-stablegate;cross:120;wait:30000;shot:load-list-settled-before-stablegate;gate_load_target_stable:60000;shot:stable-path-gate-passed" -MaxSeconds 190 -ScreenshotEverySeconds 0 -ScreenshotStartSeconds 0 -ScreenshotMaxCount 0 -HostSampleSeconds 1 -HostSampleEverySeconds 30`.
- Run directory: `debug-captures/windows-lab/20260724-154009-cpu4-loadlist-stable-path-gate-twoframe-target-diagnostic-windows`.
- Gate result: stable target gate passed. Attempt 1 at `102s` classified `PATH_TO_TENUTO_PRESENT` with crop `190`, `GoodDiff=0.000`, lower-row cursor markers `0`, damaged-save markers `0`; attempt 2 at `104s` repeated `PATH_TO_TENUTO_PRESENT` with crop `190`, `GoodDiff=0.000`, lower-row cursor markers `0`, damaged-save markers `0`. The gate logged `PATH_TO_TENUTO_PRESENT stable frames 2/2`.
- Route safety: this was target-only and did not press the save-slot `Cross` after the gate. RPCS3 stayed in the load-list state until the `190s` wall-time cap stopped it, so there is no field, Options/menu, first-battle, movement, FPS, or 200% proof.
...
- Host verification: host contention summary was clean across `7` snapshots; external contention clean.
- GPU/counter verification: GPU probe records `1508`; total observed DMA `1,460.59 MB`; offload fit mix `too-small=801`, `spu-kernel-hle=707`; top candidates remained SPU-side at `0x451c` and `0x25cc/0x9e4000`; `RSX-local traffic=0`, `New promoted CPU/SPU=0`, SPU verifier rows `0`, contract verifier rows `0`, and reservation-loop verifier rows `0`.
- Classification: `route-tooling`, `stable-load-target-gate`, `path-to-tenuto-target-proved`, `gpu-offload-parked`, `not-speed`.
- Speed status: confirmed speed increase remains `0%`. This is not field proof, Options/menu proof, first-battle proof, HLE proof, GPU migration credit, or 200% proof.
- Next hypothesis: rerun the same route shape with `gate_load_target_stable:60000` followed by slot `Cross` and field visual gate. If field is clean, the stable token should replace single-frame `gate_load_target` in future Down160 route proofs before any bodyfast/codegen/GPU work resumes.
# 2026-06-05 16:24 ET - immediate settled-row gate races to Debug Save row (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T20:24:07.687Z`.
- Refiner: `2026-06-05T16:24:39.1934903-04:00`; decision still required `PATH_TO_TENUTO_PRESENT` before any speed/HLE/RSX experiment. This run took the previous down-map hypothesis and placed `gate_load_target` immediately after the settled load-list window, before any save-list Down pulses.
- Core identity: Windows RPCS3 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
- Command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-loadlist-settled-path-immediate-gate-field-visualgate -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataGpuProbe Profile -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 260 -InputMacro "wait:65000;shot:title-settle-before-immediategate;down:160;wait:900;shot:title-after-down160-immediategate;cross:120;wait:30000;shot:load-list-settled-before-gate;gate_load_target:30000;cross:80;wait:3000;up:80;wait:500;cross:80;wait:90000;shot:load-complete-90s;cross:600;wait:18000;shot:post-load-complete-strongdismiss600-18s;wait:45000;shot:strongdismiss600-late-check;wait:45000;shot:strongdismiss600-very-late-check" -MaxSeconds 300 -ScreenshotEverySeconds 20 -ScreenshotStartSeconds 170 -ScreenshotMaxCount 9 -HostSampleSeconds 1 -HostSampleEverySeconds 30`.
- Run directory: `debug-captures/windows-lab/20260605-162453-cpu4-loadlist-settled-path-immediate-gate-field-visualgate-windows`.
- Load-target gate: failed at `101s` and correctly aborted before slot `Cross`; marker said `expected only Path to Tenuto, got DEBUG_SAVE_PROLOGUE_PRESENT`.
- Manual screenshot verification: both `screenshot-0100s-load-list-settled-before-gate.png` and `screenshot-0101s-load-target-gate.png` show `Save File 01 / Debug Save / Prologue / Lv.00 / Time 00:00:00` selected, with visible `Save file has been damaged.` rows. This contradicts the prior down-map run at the same nominal 30s/60s timing, where `Save File 01 / Path to Tenuto` was selected, so the current route has timing/state nondeterminism rather than a stable selected-row proof.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot `none`; required field-like by `260s` failed. Only title/load-list screenshots were captured.
- Fatal/log verification: targeted scan found `0` real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verification-failed, verifier-mismatch, output-mismatch, or dynamic-fail lines. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `73` with only the MIDI-input warning.
- Host verification: clean across prelaunch, postlaunch, and postrun; no competing emulator or heavy host load.
- GPU/counter verification: GPU probe records `817`; total observed DMA `813.26 MB`; offload fit mix `too-small=431`, `spu-kernel-hle=386`; largest candidates remained SPU-side at `0x451c`. `RSX-local traffic=0`, `New promoted CPU/SPU=0`, SPU verifier rows `0`, contract verifier rows `0`, and reservation-loop verifier rows `0`; this does not advance GPU-feed/offload.
- Classification: `failed-load-target-gate`, `debug-save-prologue-route-miss`, `route-nondeterminism`, `route-tooling`, `gpu-offload-parked`, `not-speed`.
...
- Next hypothesis: do not resume acceleration work yet. Add a deterministic title-to-Load selected-row guard or a pre-gate multi-frame row classifier that requires two consecutive `Path to Tenuto` selected-row frames before pressing slot `Cross`; otherwise the route can race between `Path to Tenuto` and damaged `Debug Save / Prologue`.
# 2026-06-05 15:54 ET - load-list down-map shows Path row is present before Down pulses (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T19:54:06.923Z`.
- Refiner: `2026-06-05T15:54:27.0637354-04:00`; decision said the latest gate failed on `DEBUG_SAVE_PROLOGUE_PRESENT` and to use only a polling load-target gate requiring `PATH_TO_TENUTO_PRESENT` before speed/HLE/RSX work. To avoid duplicating the already-repeated stock route, this step kept the gate discipline but ran a target-only wait/down-map diagnostic without slot confirm.
- Core identity: Windows RPCS3 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
- Command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-loadlist-debugsave-downmap-target-diagnostic -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataGpuProbe Profile -WindowsVisualGate Off -InputMacro "wait:65000;shot:title-settle-before-downmap;down:160;wait:900;shot:title-after-down160-downmap;cross:120;wait:30000;shot:load-list-after30;wait:15000;shot:load-list-after45;wait:15000;shot:load-list-after60;down:100;wait:900;shot:load-list-down1;down:100;wait:900;shot:load-list-down2;down:100;wait:900;shot:load-list-down3;down:100;wait:900;shot:load-list-down4;gate_load_target:15000;shot:load-list-downmap-final" -MaxSeconds 175 -ScreenshotEverySeconds 0 -ScreenshotStartSeconds 0 -ScreenshotMaxCount 0 -HostSampleSeconds 1 -HostSampleEverySeconds 30`.
- Run directory: `debug-captures/windows-lab/20260605-155448-cpu4-loadlist-debugsave-downmap-target-diagnostic-windows`.
- Load-target gate: failed at `138s` and correctly aborted before slot `Cross`; marker said `expected only Path to Tenuto, got DAMAGED_SAVE_TARGET`.
- Manual screenshot verification: `screenshot-0100s-load-list-after30.png` and `screenshot-0131s-load-list-after60.png` both show the selected top row as `Save File 01 / Path to Tenuto / South Section / Ch. 1 Raindrops`, with lower rows `File does not exist.` After four `Down` pulses, `screenshot-0138s-load-target-gate.png` shows the cursor on empty `File does not exist` rows instead. This means the useful target is already present during the settled 30s-60s window; the later Down-map moves away from it.
- Visual status: visual gate was intentionally off for this load-list mapping diagnostic. No field, Options/menu, battle, or movement proof was produced.
- Fatal/log verification: targeted scan found `0` real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verification-failed, verifier-mismatch, output-mismatch, or dynamic-fail lines. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `73` with only the MIDI-input warning.
- Host verification: clean across prelaunch, postlaunch, and postrun; no competing emulator or heavy host load.
- GPU/counter verification: GPU probe records `1176`; total observed DMA `1,236.84 MB`; offload fit mix `spu-kernel-hle=646`, `too-small=530`; largest candidates remained SPU-side at `0x451c`. `RSX-local traffic=0`, `New promoted CPU/SPU=0`, SPU verifier rows `0`, contract verifier rows `0`, and reservation-loop verifier rows `0`; the run does not advance GPU-feed/offload.
- Classification: `load-list-target-map`, `path-to-tenuto-selected-before-down`, `failed-load-target-gate-after-down`, `route-tooling`, `gpu-offload-parked`, `not-speed`.
- Speed status: confirmed speed increase remains `0%`. This is not field proof, Options/menu proof, first-battle proof, 200% proof, HLE proof, or GPU migration credit.
- Next hypothesis: place `gate_load_target` immediately after the settled `Save File 01 / Path to Tenuto` window at about `30s-60s` after entering Load, and do not send save-list Down pulses before the gate. Only if that passes should the route press slot `Cross` and proceed to field visuals.

# 2026-06-05 15:24 ET - Up-repair target diagnostic stays on Debug Save damaged row (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T19:23:56.405Z`.
- Refiner: `2026-06-05T15:24:18.4302464-04:00`; decision was to stop route retries/speed toggles and run only the stable-load-list Up-repair target diagnostic. It explicitly warned not to press the slot `Cross` unless `PATH_TO_TENUTO_PRESENT` was proved.
- Core identity: Windows RPCS3 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
...
- Run directory: `debug-captures/windows-lab/20260605-152433-cpu4-hle-25cc-shadow-desc-battle-stock-down160-strongdismiss600-loadlist-uprepair-target-diagnostic-windows`.
- Load-target gate: failed at `107s` and correctly aborted before slot `Cross`; marker said `expected only Path to Tenuto, got DEBUG_SAVE_PROLOGUE_PRESENT`.
- Manual screenshot verification: `screenshot-0100s-load-list-before-uprepair.png` and `screenshot-0107s-load-target-gate.png` both show `Save File 01 / Debug Save / Prologue / Lv.00 / Time 00:00:00` selected, with two visible `Save file has been damaged.` rows. The five `Up` pulses did not recover a Path-to-Tenuto row in this timing state.
- Visual status: visual gate was intentionally off for this target-only diagnostic. No field, Options/menu, battle, or movement screenshots were produced.
- Fatal/log verification: targeted scan found `0` real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verification-failed, verifier-mismatch, output-mismatch, or dynamic-fail lines. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `73` with only the MIDI-input warning.
- Host verification: clean across prelaunch, postlaunch, and postrun; no competing emulator or heavy host load.
- GPU/counter verification: GPU probe records `900`; total observed DMA `920.29 MB`; offload fit mix `spu-kernel-hle=464`, `too-small=436`; largest DMA candidates stayed SPU-side at `0x451c` and `0x25cc/0x9e4000`. `RSX-local traffic=0`, `New promoted CPU/SPU=0`, SPU verifier rows `0`, contract verifier rows `0`, and reservation-loop verifier rows `0`; the run does not advance GPU-feed/offload.
- Classification: `failed-load-target-gate`, `debug-save-prologue-route-miss`, `damaged-save-selected-row`, `route-tooling`, `gpu-offload-parked`, `not-speed`.
- Speed status: confirmed speed increase remains `0%`. This is not field proof, Options/menu proof, first-battle proof, 200% proof, HLE proof, or GPU migration credit.
- Next hypothesis: Up-repair is not the right correction for the current timing state. The route needs a target-only selected-row diagnostic that either scrolls to the visible `Save File 04 / Path to Tenuto` row from the damaged top row, or captures enough load-list frames to map selected-row movement before any slot confirm, verifier, bodyfast/codegen-fast, Vulkan/compute, or RSX experiment resumes.
...
## 2026-06-02 11:50 EDT - heartbeat: Verify25ccShadow down120 extension regresses to Debug Save load menu

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T15:49:50.261Z`.
- Refiner: newest valid-field run was the prompt-dismissed single-left `Verify25ccShadow` counterproof, and the refiner still asked for one small state-aware movement step with `CleanAfterField`. To avoid duplicating the failed second-left extension, this step kept the verifier/counter settings and added a different tiny post-field movement pulse, `ls_down:120`, after the clean single-left route.
- Step taken: Windows-only `Verify25ccShadow + 25ccBody Verify + ReservationLoop Verify` field-route extension, label `cpu4-spu-verify25cc-dismiss-save-left200-down120-visualgate-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `CleanAfterField`, extra prompt `Cross`, save-prompt dismiss, one `ls_left:200`, then one `ls_down:120`. No Android/ADB/Thor, no fast/body/GPU behavior.
- Run dir: `debug-captures/windows-lab/20260602-115034-cpu4-spu-verify25cc-dismiss-save-left200-down120-visualgate-windows-windows`.
- Host/process verification: host contention was clean across prelaunch/postlaunch/runtime/postrun samples; RPCS3 stopped after the `220s` cap; manual process check found no remaining `rpcs3` process.
- Screenshot verification: `14` screenshots from `132s` through `212s`, byte range `827,873` to `843,391`; visual gate status `NO_FIELD_LIKE_SCREENSHOT`, first field-like none, class counts `wrong-window-or-other-small-png: 14`, gate result `failed`. Manual final-frame review of `screenshot-0212s.png` showed the Load menu on `Save File 01 / Debug Save / Prologue` with damaged/file-missing text, not Path-to-Tenuto field.
- Fatal/log verification: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification: `718` verifier rows, `718` accepted, `0` rejected, `1529` total contract hits, `25,051,136` contract bytes, `0` output mismatches, `0` descriptor overflows, `promotion_ready=false`; contract `BLUS30161-958dfe208b686622-pc025cc-CellSpursKernel0`, HLE mode `contract-25cc-9e4000`.
- Reservation-loop counters: CSV correlation present with reservation command rows `1692`, reservation command exact-PC rows `45828`, command-run MFC wait exact-PC rows `85705`, peak command snapshot hits GETLLAR/PUTLLC/Atomic/PUTLLC-fail `211744 / 162977 / 48767 / 219756 / 19585`, primary fronts `0xa70 -> 0xa74` and `0xad4 -> 0xad8`, command/read deltas GETLLAR `0`, PUTLLC `0`, command/read decision `whole-loop-recognizer-preflight`, final decision `collect-missing-proof`.
- GPU probe summary: `1518` records, `1518` SPU HLE verifier/shadow records, `719` 0x25cc family/shadow records, `718` 0x25cc body records, `21549` 0x25cc shadow descriptor records, `396` 0x451c list-seed records, `749` list-family/descriptor-batch records, `1517` MFC dynamic records, `749` MFC list transfer records, `1692` MFC wait records, `85705` MFC wait exact-PC rows, `5870` reservation-loop verify records, and `0` RSX-local traffic records.
- Classification: `failed-load-menu-route`, `debug-save-route-regression`, `spu-contract-verifier-evidence`, `reservation-csv-present`, not `valid-field-triage`, not `valid-options-counterproof`, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. The verifier/counter plumbing stayed active and clean, but the route selected the wrong load target, so all FPS/title samples and counters remain invalid for promotion.
- Narrow next step: stop extending movement from this prompt-dismissed verifier route until load-row selection is repaired or state-gated; the last valid base remains `20260602-112041-cpu4-spu-verify25cc-dismiss-save-left200-counterproof-windows-windows`.

## 2026-06-02 12:20 EDT - heartbeat: load-row Up-clamp restores verifier field route with counters

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T16:19:50.521Z`.
- Refiner: still requested using the newest valid-field run as the base and adding one small state-aware movement step with `CleanAfterField`, but the latest `down120` verifier extension had regressed to `Save File 01 / Debug Save / Prologue`. To keep the step non-duplicative and address the newest route evidence, this run repaired load-row selection by adding a stronger Up-clamp before save-slot confirm, then reused the known single-left verifier route.
- Step taken: Windows-only `Verify25ccShadow + 25ccBody Verify + ReservationLoop Verify` load-row repair, label `cpu4-spu-verify25cc-loadrow-upclamp-dismiss-save-left200-counterproof-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `CleanAfterField`, repeated Up-clamp before slot confirm, extra prompt `Cross`, save-prompt dismiss, and one `ls_left:200`. No Android/ADB/Thor, no fast/body/GPU behavior.
- Run dir: `debug-captures/windows-lab/20260602-122048-cpu4-spu-verify25cc-loadrow-upclamp-dismiss-save-left200-counterproof-windows-windows`.
- Host/process verification: RPCS3 stopped after the `220s` cap; manual process check found no remaining `rpcs3`. Host external contention was clean, but overall host contention summary was `moderate` because one runtime CPU sample hit the host CPU threshold; do not use this for FPS comparison.
- Screenshot verification: `13` screenshots from `134s` through `212s`, byte range `2,559,083` to `2,623,434`; visual gate status `FIELD_LIKE_PRESENT`, first field-like `screenshot-0134s.png` at `134s`, invalid-after-first-field-like `0`, class counts `field-like-large-png: 13`, gate result `passed-for-triage`. Manual final-frame review of `screenshot-0212s.png` confirmed clean Path-to-Tenuto field with Polka/onion visible and no load/save prompt overlay.
- Fatal/log verification: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification: `809` verifier rows, `809` accepted, `0` rejected, `1727` total contract hits, `28,295,168` contract bytes, `0` output mismatches, `0` descriptor overflows, `promotion_ready=false`; contract `BLUS30161-958dfe208b686622-pc025cc-CellSpursKernel0`, HLE mode `contract-25cc-9e4000`.
- Reservation-loop counters: CSV correlation present with reservation command rows `1565`, reservation command exact-PC rows `43167`, command-run MFC wait exact-PC rows `84972`, peak command snapshot hits GETLLAR/PUTLLC/Atomic/PUTLLC-fail `162373 / 123326 / 39047 / 167959 / 12693`, primary fronts `0xa70 -> 0xa74` and `0xad4 -> 0xad8`, command/read deltas GETLLAR `0`, PUTLLC `0`, command/read decision `whole-loop-recognizer-preflight`, final decision `collect-missing-proof`.
- GPU probe summary: `1466` records, `1466` SPU HLE verifier/shadow records, `809` 0x25cc family/shadow/body records, `24258` 0x25cc shadow descriptor records, `264` 0x451c list-seed records, `634` list-family/descriptor-batch records, `1466` MFC dynamic records, `634` MFC list transfer records, `1566` MFC wait records, `84972` MFC wait exact-PC rows, `5584` reservation-loop verify records, and `0` RSX-local traffic records.
- Classification: `valid-field-triage`, `route-tooling`, `spu-contract-verifier-evidence`, `reservation-csv-present`, `load-row-repair`, not `valid-options-counterproof`, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. The stronger Up-clamp repaired the recent Debug Save / damaged-load drift and produced a better field-valid verifier/counter base, but it still lacks Options/menu and first-battle proof and remains verify-only.
- Narrow next step: keep the Up-clamped single-left verifier route as the valid field/counter base; attempt Options/menu under the same load-row clamp and verifier settings before any fast/body/GPU behavior or further movement extension.

## 2026-06-02 12:50 EDT - heartbeat: short-pulse Options route produces valid verifier/menu counterproof

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T16:49:50.999Z`.
- Refiner: still printed the generic newest-field-plus-small-movement command, but the narrow ledger after the Up-clamp repair identified the missing proof gate as Options/menu under the same verifier/counter discipline. To avoid repeating field-only movement probes and address the 200% proof gate, this run used a short-pulse title Options macro modeled after the reliable Load selection timing.
- Step taken: Windows-only title Options/menu proof, label `cpu4-spu-verify25cc-options-shortdown-proof-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `Verify25ccShadow`, `25ccBody Verify`, and `ReservationLoop Verify`. Macro used two short `down:20` title pulses then `cross:80`. No Android/ADB/Thor, no fast/body/GPU behavior, no field movement extension.
- Run dir: `debug-captures/windows-lab/20260602-125117-cpu4-spu-verify25cc-options-shortdown-proof-windows-windows`.
- Host/process verification: host contention was clean across all `6` snapshots, external contention clean, RPCS3 stopped after the `125s` cap, and manual process check found no remaining `rpcs3` process.
- Screenshot verification: `10` screenshots from `57s` through `120s`, byte range `958,462` to `959,639`. Manual review of `screenshot-0057s.png` and `screenshot-0120s.png` confirmed the full title Options page with Battle Camera, Attack Button, Volume, Subtitles, Voice, and Language rows visible; no intro/cutscene/title-background miss.
- Fatal/log verification: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification: `432` verifier rows, `432` accepted, `0` rejected, `920` total contract hits, `15,073,280` contract bytes, `0` output mismatches, `0` descriptor overflows, `promotion_ready=false`; contract `BLUS30161-958dfe208b686622-pc025cc-CellSpursKernel0`, HLE mode `contract-25cc-9e4000`.
- Reservation-loop counters: CSV correlation present with reservation command rows `1015`, reservation command exact-PC rows `27635`, command-run MFC wait exact-PC rows `52077`, peak command snapshot hits GETLLAR/PUTLLC/Atomic/PUTLLC-fail `183909 / 141746 / 42163 / 189665 / 15483`, primary fronts `0xa70 -> 0xa74` and `0xad4 -> 0xad8`, command/read deltas GETLLAR `0`, PUTLLC `0`, command/read decision `whole-loop-recognizer-preflight`, final decision `collect-missing-proof`.
- GPU probe summary: `932` records, `932` SPU HLE verifier/shadow records, `432` 0x25cc family/shadow/body records, `12948` 0x25cc shadow descriptor records, `244` 0x451c list-seed records, `481` list-family/descriptor-batch records, `932` MFC dynamic records, `481` MFC list transfer records, `1015` MFC wait records, `52077` MFC wait exact-PC rows, `3534` reservation-loop verify records, and `0` RSX-local traffic records.
- Classification: `valid-options-counterproof`, `spu-contract-verifier-evidence`, `reservation-csv-present`, `direction-split-counterproof`, not `valid-field-triage` for this run, not first-battle proof, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. The short-pulse Options route repairs the earlier cutscene Options miss and gives a clean Options/menu verifier counterproof under the same 0x25cc discipline, but promotion remains blocked by missing first-battle proof and the absence of any fast/body/GPU behavior.
- Narrow next step: keep the Up-clamped field counterproof plus short-pulse Options counterproof as the current valid proof pair, then attempt first-battle routing under `Verify25ccShadow + 25ccBody Verify + ReservationLoop Verify` only after preserving screenshots and fatal-log gates.

## 2026-06-02 13:20 EDT - heartbeat: Up-clamped verifier first-battle route stalls on Load menu

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T17:19:51.437Z`.
- Refiner: still printed the generic newest-field-plus-small-movement command, but the narrow ledger now has valid Up-clamped field counterproof plus valid short-pulse Options/menu counterproof. The missing 200% proof gate is first battle under the same verifier/counter discipline, so this step used the repaired Up-clamped load route as a custom first-battle route attempt rather than repeating another field-only proof.
- Step taken: Windows-only first-battle route attempt, label `cpu4-spu-verify25cc-upclamp-left200-firstbattle-route-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `Verify25ccShadow`, `25ccBody Verify`, `ReservationLoop Verify`, custom Up-clamped load-row macro, save-prompt dismiss, one `ls_left:200`, then the existing battle movement tail. No Android/ADB/Thor, no fast/body/GPU behavior.
- Run dir: `debug-captures/windows-lab/20260602-132124-cpu4-spu-verify25cc-upclamp-left200-firstbattle-route-windows-windows`.
- Wrapper/process verification: outer shell timed out at `900s`, but wrapper output shows RPCS3 reached the `370s` cap, stopped PID `28648`, and wrote `RPCS3.log`. Manual process check found no remaining `rpcs3` process.
- Host verification: prelaunch/postlaunch/runtime/postrun host samples were clean in wrapper output; external contention clean.
- Screenshot verification: `30` screenshots from `134s` through `370s`, byte range `792,051` to `807,370`. No `eternal-sonata-windows-visual-gate-summary.md` was emitted before the outer timeout. Manual screenshots showed the run never reached field or battle: `screenshot-0134s.png` was still the Load menu with `Save File 01 / Debug Save / Prologue` plus a `Load data from this file. Proceed?` prompt over a lower `Save File 03 / Path to Tenuto` preview; `screenshot-0312s.png` and `screenshot-0370s.png` were still on the same Load menu/damaged-save state.
- Fatal/log verification: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification: `979` verifier rows, `979` accepted, `0` rejected, `2091` total contract hits, `34,258,944` contract bytes, `0` output mismatches, `0` descriptor overflows, `promotion_ready=false`; contract `BLUS30161-958dfe208b686622-pc025cc-CellSpursKernel0`, HLE mode `contract-25cc-9e4000`.
- Reservation-loop counters: reservation command CSV, command exact-PC CSV, and command-run MFC wait exact-PC CSV were missing; all reservation/MFC/pair rows were `0`; peak command snapshot hits all `0`; command/read decision `command-correlation-data-missing`; final decision `collect-missing-proof`.
- GPU probe summary: no `eternal-sonata-gpu-probe-summary.md` was emitted before the outer timeout, so no GPU probe summary or RSX-local accounting is available from this artifact.
- Classification: `failed-load-menu-route`, `first-battle-route-miss`, `spu-contract-verifier-evidence`, `reservation-csv-missing`, not `valid-field-triage`, not `valid-options-counterproof`, not first-battle proof, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. The first-battle route failed before field because load-row/prompt state was wrong; clean verifier rows alone do not count when visuals never leave Load.
- Narrow next step: repair first-battle load selection with an explicit Load-menu target proof/screenshot before any movement tail, or split first-battle into a state-gated two-stage run: Up-clamped field proof first, then battle movement only after a field screenshot confirms Path-to-Tenuto.

## 2026-06-02 13:50 EDT - heartbeat: load-target diagnostic crashes before menu proof

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T17:49:54.425Z`.
- Refiner: still printed the generic newest-valid-field plus one small `CleanAfterField` movement command, but the latest first-battle artifact failed earlier at Load-menu targeting. To keep the step non-duplicative and avoid another blind movement extension, this run stopped before save-slot confirm and tried to capture an explicit Up-clamped Load-menu target proof.
- Step taken: Windows-only load-target diagnostic, label `cpu4-spu-verify25cc-loadtarget-upclamp-menu-diagnostic-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `Verify25ccShadow`, `25ccBody Verify`, `ReservationLoop Verify`, two repeated Up-clamps in the Load menu, and no save-slot confirm or battle movement tail. No Android/ADB/Thor, no fast/body/GPU behavior.
- Run dir: `debug-captures/windows-lab/20260602-135218-cpu4-spu-verify25cc-loadtarget-upclamp-menu-diagnostic-windows-windows`.
- Host/process verification: host contention stayed clean across prelaunch/postlaunch/runtime/postrun samples; RPCS3 was stopped after the `110s` cap; manual process check found no remaining `rpcs3` process.
- Screenshot verification: `10` screenshots from `63s` through `103s`, byte range `33,288` to `33,850`. Manual review of `screenshot-0063s.png`, `screenshot-0080s.png`, and `screenshot-0103s.png` showed a black crash screen with the overlay `The PS3 application has likely crashed, you can close it.` No Load-menu target, Path-to-Tenuto field, Options page, or first-battle visual proof was produced.
- Fatal/log verification: invalid. The log has one real access violation, `VM: Access violation reading location 0x4 (unmapped memory)` at `0:00:49.951573` in `PPU[0x1000000] Thread (main_thread) [0x0007dccc]`. No `VK_ERROR`, no device lost, no unknown STOP, and no assert. The only `fatal` text match was still the benign config line `Show fatal error hints: false`.
- SPU verifier verification: `132` verifier rows, `132` accepted, `0` rejected, `276` total contract hits, `4,521,984` contract bytes, `0` output mismatches, `0` descriptor overflows, `promotion_ready=false`; contract `BLUS30161-958dfe208b686622-pc025cc-CellSpursKernel0`, HLE mode `contract-25cc-9e4000`.
- Reservation-loop counters: CSV correlation was present despite the fatal visual failure: reservation command rows `330`, reservation command exact-PC rows `9106`, command-run MFC wait exact-PC rows `17081`, peak command snapshot hits GETLLAR/PUTLLC/Atomic/PUTLLC-fail `209627 / 162322 / 47305 / 215927 / 18156`, primary fronts `0xa70 -> 0xa74` and `0xad4 -> 0xad8`, command/read deltas GETLLAR `0`, PUTLLC `0`, command/read decision `whole-loop-recognizer-preflight`, final decision `collect-missing-proof`.
- GPU probe summary: `313` records, `313` SPU HLE verifier/shadow records, `132` 0x25cc family/shadow/body records, `3948` 0x25cc shadow descriptor records, `89` 0x451c list-seed records, `178` 0x451c list-family/descriptor-batch records, `313` MFC dynamic records, `178` MFC list transfer records, `330` MFC wait records, `17081` MFC wait exact-PC rows, `1158` reservation-loop verify records, and `0` RSX auditor records. This is verifier/counter evidence only, not GPU migration credit.
- Classification: `fatal-invalid`, `access-violation`, `load-target-diagnostic-crash`, `spu-contract-verifier-evidence`, `reservation-csv-present`, not `valid-field-triage`, not `valid-options-counterproof`, not first-battle proof, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. The diagnostic is useful because it proves the current load-target probe is not just selecting the wrong save row; it can crash before any menu-target proof under the verifier route, so the artifact is invalid for promotion.
- Narrow next step: park movement extensions and do the SPU contract pipeline or a verifier-only crash isolation pass around the title-to-Load transition before attempting first-battle again; keep the valid proof base as `20260602-122048-cpu4-spu-verify25cc-loadrow-upclamp-dismiss-save-left200-counterproof-windows-windows` plus `20260602-125117-cpu4-spu-verify25cc-options-shortdown-proof-windows-windows`.

## 2026-06-02 14:20 EDT - heartbeat: no-movement loader control re-proves clean field after fatal

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T18:19:54.977Z`.
- Refiner: latest run had fatal/crash log evidence, so the refiner blocked further movement and requested a no-movement loader/control reproof with `CleanAfterField` before adding movement.
- Step taken: ran the exact Windows-only refiner command, label `cpu4-loader-control-visualgate-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `EternalSonataReservationLoop Verify`, `CleanAfterField`, `WindowsVisualGateFieldSeconds 160`, `MaxSeconds 190`, screenshots from `120s`, and no verifier/body fast mode, no movement extension, no Android/ADB/Thor.
- Run dir: `debug-captures/windows-lab/20260602-142032-cpu4-loader-control-visualgate-windows-windows`.
- Wrapper/process verification: outer shell timed out at `360s` during postrun handling, but wrapper output shows RPCS3 reached the `190s` cap and stopped PID `11748`. Manual process check found no remaining `rpcs3` process.
- Host verification: host contention was clean across all `6` snapshots; external contention clean.
- Screenshot verification: `10` screenshots from `117s` through `190s`, byte range `2,615,140` to `2,621,222`. No `eternal-sonata-windows-visual-gate-summary.md` was emitted before the outer timeout. Manual `screenshot-0117s.png` and `screenshot-0190s.png` review confirmed stable Path-to-Tenuto field with Polka/onion visible, no crash overlay, no Load/menu prompt, and no later invalid frame in the captured window.
- Fatal/log verification: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification: contract parser found `0` verifier rows (`no contract verifier rows found`), expected because this refiner command ran `EternalSonataSpuHleVerify Off`; not a 0x25cc counterproof.
- Reservation-loop counters: missing because postrun analysis timed out before CSV emission. Reservation command rows `0`, reservation command exact-PC rows `0`, command-run MFC wait exact-PC rows `0`, peak command snapshot hits all `0`, command/read decision `command-correlation-data-missing`, final decision `collect-missing-proof`.
- GPU probe summary: missing; no `eternal-sonata-gpu-probe-summary.md` was emitted before the outer timeout, so there is no RSX-local accounting or GPU migration evidence from this artifact.
- Classification: `valid-field-triage`, `route-tooling`, `postrun-timeout`, `reservation-csv-missing`, not `spu-contract-verifier-evidence`, not `valid-options-counterproof`, not first-battle proof, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. This repaired the immediate confidence loss from the fatal diagnostic by re-proving clean no-movement field visuals and clean fatal logs, but it produced no SPU verifier rows, no reservation CSVs, no Options/menu proof, and no first-battle proof.
- Narrow next step: keep the valid proof base as `20260602-122048-cpu4-spu-verify25cc-loadrow-upclamp-dismiss-save-left200-counterproof-windows-windows` plus `20260602-125117-cpu4-spu-verify25cc-options-shortdown-proof-windows-windows`; before any battle/movement extension, run either the SPU contract pipeline or a verifier-only title-to-Load crash isolation with bounded postrun time so counters are preserved.

## 2026-06-02 14:50 EDT - heartbeat: refiner left200 route misses field under title/cache delay

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T18:49:55.247Z`.
- Refiner: after the clean no-movement loader-control reproof, decision was to use the newest valid loader-control as the route base, add one small `ls_left:200` state-aware movement step with `CleanAfterField`, and keep lane-2 HLE/GPU dry-runs blocked.
- Step taken: ran the exact Windows-only refiner command, label `cpu4-loader-control-left200-visualgate-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `EternalSonataReservationLoop Verify`, `CleanAfterField`, one `ls_left:200`, `MaxSeconds 205`, and no verifier/body fast mode, no Android/ADB/Thor.
- Run dir: `debug-captures/windows-lab/20260602-145031-cpu4-loader-control-left200-visualgate-windows-windows`.
- Host/process verification: RPCS3 stopped after the `205s` cap; manual process check found no remaining `rpcs3` process. Host contention was invalid for FPS comparison: summary `high`, external `moderate`, with runtime/postrun CPU samples at `100%` and hot `clang++` processes during the route window.
- Screenshot verification: `14` screenshots from `123s` through `200s`, byte range `51,481` to `1,720,673`. Visual gate status `NO_FIELD_LIKE_SCREENSHOT`, first field-like none, class counts `black-overlay-small-png: 4` and `cutscene-or-nonfield-large-png: 10`, gate result `failed`. Manual review showed the route never reached Path-to-Tenuto field: `screenshot-0123s.png` was still `Building SPU Cache...`, `screenshot-0143s.png` was the Bandai Namco logo, and `screenshot-0200s.png` was the title screen. This is a timing/cache/title-route miss, not movement proof.
- Fatal/log verification: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification: contract parser found `0` verifier rows (`no contract verifier rows found`), expected because this refiner command ran `EternalSonataSpuHleVerify Off`; not a 0x25cc counterproof.
- Reservation-loop counters: CSV correlation was present but invalid for promotion because visuals failed: reservation command rows `408`, reservation command exact-PC rows `11272`, command-run MFC wait exact-PC rows `22454`, peak command snapshot hits GETLLAR/PUTLLC/Atomic/PUTLLC-fail `146896 / 83609 / 63287 / 147198 / 8291`, primary fronts `0xa70 -> 0xa74` and `0xad4 -> 0xad8`, command/read deltas GETLLAR `0`, PUTLLC `0`, command/read decision `whole-loop-recognizer-preflight`, final decision `collect-missing-proof`.
- GPU probe summary: `376` records, `376` MFC dynamic records, `117` MFC list transfer records, `408` MFC wait records, `22454` MFC wait exact-PC rows, `408` reservation-loop command records, `11272` reservation command exact-PC records, `1471` reservation-loop verify records, `0` SPU HLE verifier/shadow/25cc records, and `0` RSX auditor/local traffic records. This is route/counter telemetry only, not GPU migration credit.
- Classification: `failed-visual-gate`, `title-route-miss`, `cache-build-delay`, `contended-host`, `reservation-csv-present`, not `valid-field-triage`, not `spu-contract-verifier-evidence`, not `valid-options-counterproof`, not first-battle proof, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. The one-left step did not test field movement because the route timing fired during cache/title startup and never loaded the save; counters and FPS samples are invalid.
- Narrow next step: do not extend movement from this artifact. Re-establish a host-clean, cache-warm no-movement or single-left field route with later/poll-gated title-to-Load timing before any first-battle, verifier, or fast-path work.

## 2026-06-02 15:20 EDT - heartbeat: delayed preproof left80 stays on loading screen

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T19:19:55.676Z`.
- Refiner: latest loader-control `left200` produced non-field/cutscene frames after a clean lower boundary and blocked an automatic movement rerun. It allowed route-state repair, shrinking/changing the pulse only after pre-movement field proof, or switching to focused SPU kernel HLE/codegen/verifier analysis.
- Step taken: Windows-only delayed route repair/shrunken-pulse probe, label `cpu4-loader-control-delayload-preproof-left80-visualgate-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `EternalSonataReservationLoop Verify`, `CleanAfterField`, a longer `90s` initial title wait, explicit pre-movement screenshots, and a smaller `ls_left:80` pulse only after the intended preproof point. No verifier/body fast mode, no Android/ADB/Thor.
- Run dir: `debug-captures/windows-lab/20260602-152140-cpu4-loader-control-delayload-preproof-left80-visualgate-windows-windows`.
- Wrapper/process verification: outer shell timed out at `600s` during postrun handling, but wrapper output shows RPCS3 reached the `270s` cap and stopped PID `22676`. Manual process check found no remaining `rpcs3` process.
- Host verification: host contention was clean across all `6` snapshots; external contention clean.
- Screenshot verification: `16` screenshots from `162s` through `270s`, byte range `110,904` to `114,868`. No visual-gate summary was emitted before the outer timeout. Manual review of `screenshot-0162s.png`, `screenshot-0185s.png`, and `screenshot-0270s.png` showed the run stayed on the `Now Loading...` screen for the entire captured window, including before and after the smaller left pulse. No Path-to-Tenuto field, Options/menu, or first-battle visual proof was produced.
- Fatal/log verification: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification: contract parser found `0` verifier rows (`no contract verifier rows found`), expected because this loader-control route ran `EternalSonataSpuHleVerify Off`; not a 0x25cc counterproof.
- Reservation-loop counters: missing because postrun analysis timed out before CSV emission. Reservation command rows `0`, reservation command exact-PC rows `0`, command-run MFC wait exact-PC rows `0`, peak command snapshot hits all `0`, command/read decision `command-correlation-data-missing`, final decision `collect-missing-proof`.
- GPU probe summary: missing; no `eternal-sonata-gpu-probe-summary.md` was emitted before the outer timeout, so there is no RSX-local accounting or GPU migration evidence from this artifact.
- Classification: `failed-visual-gate`, `loading-only-route`, `delayed-load-route-miss`, `postrun-timeout`, `reservation-csv-missing`, not `valid-field-triage`, not `spu-contract-verifier-evidence`, not `valid-options-counterproof`, not first-battle proof, not `windows-micro-win`, not `gpu-migration-credit`, not 200% proof.
- Conclusion: no speed increase. The delayed/shrunken-pulse route avoided host contention and fatal logs but still never reached field, so the visible `120 FPS` samples are loading-screen/title-route samples and invalid for promotion.
- Narrow next step: stop changing movement pulses until the title-to-load path is state-gated or re-proved; prefer a target-only Load-menu/field arrival diagnostic with no movement, or switch to the SPU contract pipeline/verify-counter analysis lane while route timing is unstable.

## 2026-06-02 15:50 EDT - heartbeat: SPU contract pipeline refreshed from valid 122048 field counterproof

- Automation: `ps3-200-windows-speed-loop` heartbeat at `2026-06-02T19:49:56.458Z`.
- Refiner: blocked another automatic loader-control movement rerun after the `left200` and delayed `left80` route misses. Decision allowed route-state repair, shrinking/changing movement only after field preproof, or switching to focused SPU kernel HLE/codegen/verifier analysis.
- Step taken: Windows-only SPU contract/compiler analysis lane. Ran `tools\spu_contract_pipeline.ps1` against the last valid field verifier source run `debug-captures/windows-lab/20260602-122048-cpu4-spu-verify25cc-loadrow-upclamp-dismiss-save-left200-counterproof-windows-windows` with target PCs `0x25cc, 0x451c`, target EA `0x9e4000`, `-NoGhidra`, and `-MaxWindows 12`. No Android/ADB/Thor, no emulator fast path, no movement rerun.
- Pipeline outputs: refreshed `spu-contracts/BLUS30161` summary/index at `2026-06-02T15:50:44.8718092-04:00`; source run now points at the `122048` valid field counterproof; contracts `2`: `BLUS30161-958dfe208b686622-pc025cc-CellSpursKernel0` and `BLUS30161-958dfe208b686622-pc0451c-TCX_CellSpursKernel0`; both classify as `dma-window,spurs-kernel` with `80` hot log hits each; Ghidra headless was `missing-or-skipped`.
- Screenshot/source verification: manual review of source `screenshot-0212s.png` confirmed clean Path-to-Tenuto field with Polka/onion visible and no Load/save prompt overlay. This reuses the previously valid field visual source; no new gameplay proof was attempted.
- Fatal/log verification for source run: no access violation, no `VK_ERROR`, no device lost, no unknown STOP, no assert. The only `fatal` match was the benign config line `Show fatal error hints: false`.
- SPU verifier verification for source run: `809` verifier rows, `809` accepted, `0` rejected, `1727` total contract hits, `28,295,168` contract bytes, `0` output mismatches, `0` descriptor overflows, `promotion_ready=false`; contract `BLUS30161-958dfe208b686622-pc025cc-CellSpursKernel0`, HLE mode `contract-25cc-9e4000`.
- Reservation-loop counters for source run: CSV correlation present with reservation command rows `1565`, reservation command exact-PC rows `43167`, command-run MFC wait exact-PC rows `84972`, peak command snapshot hits GETLLAR/PUTLLC/Atomic/PUTLLC-fail `162373 / 123326 / 39047 / 167959 / 12693`, primary fronts `0xa70 -> 0xa74` and `0xad4 -> 0xad8`, command/read deltas GETLLAR `0`, PUTLLC `0`, command/read decision `whole-loop-recognizer-preflight`, final decision `collect-missing-proof`.
...
- Command: `./tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label hle-451c-size16-candidate-reproof-field-resloop-counterproof ... -EternalSonataSpuHleVerify Verify -EternalSonataSpuHleSize16Body Off -EternalSonataReservationLoop Verify -WindowsGameScreen 1 -WindowsVisualGate CleanAfterField`.
...
- Fatal/log verification: RPCS3 exited after the harness stop; fatal scan was clean after excluding the benign `Show fatal error hints: false` text. Host snapshots were clean prelaunch, postlaunch, periodic samples, and postrun.
- SPU verifier/counter verification: GPU summary recorded `1547` SPU HLE verifier records, `1546` shadow records, `1074` 0x25cc-family records, `454` 0x451c descriptor-batch records, and `0` RSX-local traffic records. The SPU contract log-row parser reported `0` accepted contract rows for this generic `Verify` run, so it is counter evidence only, not refreshed contract-row proof.
...
- Classification: `failed-visual-gate`, `loading-only-route`, `spu-hle-verifier-counter-evidence`, `reservation-loop-counter-evidence`, `not-speed`. No 200% field+Options+first-battle proof, no HLE/GPU promotion, and no speed increase.
...
## 2026-06-02T16:59:42-04:00 - black-control load-target gate false-negative on missing exemplar

- Context: heartbeat `ps3-200-windows-speed-loop`; refiner generated `2026-06-02T16:50:24.2248191-04:00`, reported repeated loading-before-field, and blocked another `loader-control-left200` movement rerun. Chosen step followed the route-control branch: a Windows-only title/load black-control diagnostic with reservation-loop verification, no slot confirm after a failed gate, no Android/ADB/Thor, and no fast/body/GPU behavior.
- Command: `./tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-titleload-blackcontrol-resloop-diagnostic-windows ... -EternalSonataReservationLoop Verify -EternalSonataGpuProbe Profile -WindowsGameScreen 1 -WindowsVisualGate Off -InputMacro wait:65000;...;gate_load_target:60000`.
- Run: `debug-captures/windows-lab/20260602-165106-cpu4-titleload-blackcontrol-resloop-diagnostic-windows-windows`.
- Route/gate verification: the macro selected title `LOAD`, entered the Load screen, then aborted before slot `Cross` because `gate_load_target` timed out at `192s` with `UNKNOWN_LOAD_TARGET`. The marker says the gate failed because its Path-to-Tenuto exemplar file is missing: `debug-captures/windows-lab/20260526-001938-cpu4-stateaware-late-load-confirm-left200-visualgate-windows-windows/screenshots/screenshot-0117s.png`.
- Screenshot verification: manual `screenshot-0068s-title-settle-before-blackgate.png` showed the title menu; manual `screenshot-0192s-load-target-gate-33.png` showed the correct top Load row, `Save File 01 / Path to Tenuto / South Section / Ch.1 Raindrops`, with lower rows saying `File does not exist.` This means the gate result was a tooling false-negative from the missing exemplar, not proof that the route selected the wrong save row.
- Fatal/log verification: RPCS3 exited cleanly after the macro abort, no process remained, and targeted fatal/access/device-lost/assert/unknown-STOP scan was clean. Host contention was clean prelaunch, postlaunch, and postrun.
- SPU verifier verification: `EternalSonataSpuHleVerify` was intentionally `Off`, so the contract parser reported `0` verifier rows and `0` accepted rows. This run is route-control evidence only, not 0x25cc contract-row proof.
- Reservation-loop/GPU counters: reservation-loop artifacts were present. GPU summary recorded `1452` records, `1452` MFC dynamic records, `942` MFC list-transfer records, `1635` MFC wait records, `78089` MFC wait exact-PC records, `1635` reservation-loop command records, `43652` reservation-loop command exact-PC records, `5467` reservation-loop verify records, `0` SPU HLE verifier records, `0` RSX-local traffic records, and offload-fit mix `spu-kernel-hle=737, too-small=715`.
- Classification: `route-tooling`, `load-target-gate-false-negative`, `missing-exemplar`, `reservation-csv-present`, `not-speed`. No field proof, no Options/menu proof, no first-battle proof, no Windows micro-win, no GPU migration credit, and no 200% proof.
- Conclusion: no speed increase. The route reached the desired Load-list target, but the current gate cannot recognize it because it depends on a missing historical exemplar. The next narrow step is a harness/tooling repair: repoint or regenerate the Path-to-Tenuto load-target exemplar from an existing valid screenshot, then rerun only the target gate before any slot confirm, movement, verifier fast path, or first-battle retry.
## 2026-06-02T17:29:24-04:00 - load-target classifier fallback repair and target-only reproof

- Context: heartbeat `ps3-200-windows-speed-loop`; refiner generated `2026-06-02T17:20:28.8711703-04:00`, reported the latest load-target-gated route aborted before save-slot `Cross`, and required polling `gate_load_target` with `PATH_TO_TENUTO_PRESENT` before any speed/HLE/RSX experiment. The prior heartbeat showed the gate was a false-negative because its good Path-to-Tenuto exemplar path was missing.
- Tooling change: updated `tools/classify_eternal_sonata_load_target.ps1` so the default Path-to-Tenuto exemplar falls back to current existing target screenshots, and so a missing Debug-Save exemplar no longer crashes the classifier with an empty `LiteralPath`. The bad/debug-save guard is now disabled when no fallback exists, rather than throwing before a Path row can be classified.
- Windows step: target-only route-control diagnostic, label `cpu4-titleload-gatefix-targetonly-reproof-windows-windows`, with `PadApi`, `-WindowsGameScreen 1`, CPU mask `0x0F`, frame/vblank `240`, `EternalSonataReservationLoop Verify`, `EternalSonataGpuProbe Profile`, no SPU verifier/body/fast path, no slot confirm after gate failure, and no Android/ADB/Thor.
- Run: `debug-captures/windows-lab/20260602-172323-cpu4-titleload-gatefix-targetonly-reproof-windows-windows`.
- Runtime gate result: still aborted before save-slot `Cross` at `112s` because the first repair left one empty-bad-exemplar `Test-Path` bug in place during the emulator run. The gate marker was `timed out after 30000ms; last status UNKNOWN_LOAD_TARGET; Cannot bind argument to parameter 'LiteralPath' because it is an empty string.` The narrow guard was fixed immediately after the run; no second emulator run was started this heartbeat.
- Screenshot verification: manual `screenshot-0068s-title-settle-gatefix.png` showed the title menu with `LOAD` available. Manual `screenshot-0112s-load-target-gate-17.png` showed the Load screen with Path-to-Tenuto visible on Save File 01 and Save File 02, and File-does-not-exist below; this is target/menu evidence only, not field gameplay.
- Offline post-fix classifier verification against the same run: `PATH_TO_TENUTO_PRESENT`; Path exemplar resolved to `20260602-165106-cpu4-titleload-blackcontrol-resloop-diagnostic-windows-windows/screenshots/screenshot-0192s-load-target-gate-33.png`; Debug-Save exemplar was disabled because no existing fallback was found; counts were `path-to-tenuto=15`, `debug-save-prologue=0`, `empty-load-slot=0`, `unknown=5`. Row `screenshot-0112s-load-target-gate-17.png` classified `path-to-tenuto` with good diff `0.000`, bad diff infinity, Path Ys `190,365`, and no lower cursor/damaged markers.
- Fatal/log verification: RPCS3 exited after the gate abort, no process remained, host contention stayed clean, and targeted fatal/access/device-lost/assert/unknown-STOP scan was clean except the benign `Show fatal error hints: false` text.
- SPU verifier verification: parser reported `0` verifier rows and `0` accepted rows, expected because `EternalSonataSpuHleVerify` was intentionally `Off`. This is route tooling, not contract-row proof.
- Reservation-loop/GPU counters: summary recorded `793` total records, `793` MFC dynamic records, `539` MFC list-transfer records, `858` MFC wait records, `41922` MFC wait exact-PC records, `858` reservation-loop command records, `23191` reservation-loop command exact-PC records, `2901` reservation-loop verify records, `0` SPU HLE verifier records, `0` RSX-local traffic records, and offload-fit mix `too-small=399, spu-kernel-hle=394`.
- Classification: `route-tooling`, `harness-fix`, `target-gate-repaired-offline`, `reservation-csv-present`, `not-speed`. No field proof, no Options/menu proof for this step, no first-battle proof, no Windows micro-win, no GPU migration credit, and no 200% proof.
- Remaining caveat: because the Debug-Save exemplar is still missing, the repaired gate can recognize visible Path-to-Tenuto rows but has weaker explicit debug-save discrimination. The next narrow step is either add/find a real Debug-Save fallback exemplar or rerun the target-only gate now that the empty bad-exemplar guard is fixed; do not proceed to slot confirm, movement, first battle, or fast paths until the live gate reports `PATH_TO_TENUTO_PRESENT` cleanly.

### 2026-06-02T17:57:57-04:00 - load-target live gate reproof passes after classifier fallback repair

- Refiner decision: latest route remained blocked by the previous live load-target gate abort, so this heartbeat stayed on a Windows-only target-gate reproof and did not run speed/HLE/RSX fast paths, slot confirmation, movement, Android, ADB, or Thor.
- Command: ./tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-titleload-livegatefix-targetonly-reproof-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -EternalSonataGpuProbe Profile -WindowsHostContentionGate ExternalFail -WindowsVisualGate Off -InputMacro "wait:65000;shot:title-settle-livegatefix;down:160;wait:900;shot:title-after-down160-livegatefix;cross:120;wait:12000;shot:pre-livegatefix;gate_load_target:30000;shot:path-target-after-livegatefix" -MaxSeconds 135 -ScreenshotEverySeconds 20 -ScreenshotStartSeconds 70 -ScreenshotMaxCount 5 -HostSampleSeconds 1 -HostSampleEverySeconds 30.
- Run: debug-captures/windows-lab/20260602-175130-cpu4-titleload-livegatefix-targetonly-reproof-windows-windows. The outer shell timed out after the emulator had already been stopped at the 135s run limit, so the normal post-run reservation/GPU summary files were not emitted; counters below are from a manual single-pass parse of RPCS3.log.
- Screenshot verification: screenshots/screenshot-0068s-title-settle-livegatefix.png is the title screen with the cursor on LOAD; screenshots/screenshot-0089s-path-target-after-livegatefix.png is the Load screen showing Save File 03 at Path to Tenuto / South Section / Ch. 1 Raindrops and empty lower slots. This is target/menu evidence only, not field or battle evidence.
- Load-target classifier: gate passed live on attempt 3 with status PATH_TO_TENUTO_PRESENT; counts path-to-tenuto=1, debug-save-prologue=0, empty-load-slot=0, unknown=5; lower-row cursor markers  ; damaged-save text markers  . Debug-Save exemplar remains disabled/missing, so explicit debug-save discrimination is still weaker than desired.
- Fatal/log verification: exact fatal/access-violation/unhandled-exception/crash scan all  . Warning-only SPURS export lines and _sys_ppu_thread_exit aborted LOAD_ENUM warnings were present but not fatal crash evidence. Host contention snapshots stayed clean.
- SPU verifier rows:   because EternalSonataSpuHleVerifier was off for this route-tooling reproof.
- Reservation/GPU counters from RPCS3.log: MFC dynamic 961; MFC list transfer 624; MFC wait 1059; MFC wait exact-PC rows 51118; reservation command rows 1059; reservation command PC rows 28392; reservation verify rows 1059; reservation verify lane rows 2494; RSX-local rows  ; GPU fit summary rows   because post-run summary generation was clipped by the shell timeout.
- Classification: oute-tooling, 	arget-gate-live-pass, eservation-counters-present, 
ot-speed. No 200% claim: this does not include field+Options+first-battle proof and did not exercise a fast path.
- Next narrow step: now that live target gate reports PATH_TO_TENUTO_PRESENT, rerun the poll-gated slot-confirm/movement route from the refiner, still with visual gates and no speed/HLE/RSX optimization until field visuals are valid; alternatively add a real Debug-Save exemplar first if we want stronger bad-save discrimination.

### 2026-06-02T18:32:11-04:00 - live gate false-positive selects Debug Save instead of Path-to-Tenuto

- Refiner decision: blocked another loader-control-left200 movement rerun and required route-state control, movement shrink/change, or SPU verifier/codegen analysis before more movement. Chosen step was a Windows-only route-control/no-movement slot-confirm reproof: live load-target gate, then slot confirm, with CleanAfterField visual gate. No Android, ADB, Thor, speed/HLE fast path, or movement pulse was run.
- Command: ./tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-livegate-slotconfirm-nomove-field-reproof-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -EternalSonataGpuProbe Profile -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -InputMacro "wait:65000;shot:title-settle-livegate-slotfield;down:160;wait:900;shot:title-load-selected-livegate-slotfield;cross:120;wait:12000;shot:pre-livegate-slotfield;gate_load_target:30000;shot:path-target-before-slotfield;cross:120;wait:600;cross:600;wait:85000;shot:field-arrival-livegate-slotfield;wait:45000;shot:field-late-livegate-slotfield" -MaxSeconds 230 -ScreenshotEverySeconds 20 -ScreenshotStartSeconds 100 -ScreenshotMaxCount 10 -HostSampleSeconds 1 -HostSampleEverySeconds 30.
- Run: debug-captures/windows-lab/20260602-182050-cpu4-livegate-slotconfirm-nomove-field-reproof-windows-windows.
- Load-target classifier result: reported PATH_TO_TENUTO_PRESENT on attempt 2; summary counts path-to-tenuto=1, debug-save-prologue=0, empty-load-slot=0, unknown=4; lower-row cursor markers 0; damaged-save text markers 0. This result is invalid as a target proof because manual screenshot review contradicts it.
- Screenshot verification: screenshot-0068s-title-settle-livegate-slotfield.png showed the title menu; screenshot-0085s-load-target-gate-2.png and screenshot-0086s-path-target-before-slotfield.png both visibly show Save File 01 / Debug Save / Prologue with File does not exist below and a Save file has been damaged row, not Path to Tenuto. The late screenshots, including screenshot-0219s-field-late-livegate-slotfield.png, remained on the same Debug Save load menu. No Path-to-Tenuto field, Options/menu, or first-battle visual proof was produced.
- Visual gate: NO_FIELD_LIKE_SCREENSHOT; first field-like screenshot none; gate failed with no field-like screenshot at or before 160s.
- Fatal/log verification: exact fatal, access violation, VK_ERROR, device lost, unhandled exception, assertion, and crash scans were all 0. Host contention snapshots stayed clean.
- SPU verifier rows: 0 because EternalSonataSpuHleVerify was off for this route-tooling reproof.
- Reservation/GPU counters: GPU summary emitted MFC dynamic 1695, MFC list transfer 1064, MFC wait 1926, MFC wait exact-PC 91877, reservation-loop command CSV present, reservation-loop command exact-PC CSV present, reservation-loop verify CSV present, SPU HLE verifier records 0, RSX-local traffic records 0, offload fit mix spu-kernel-hle=886 and too-small=809. Manual RPCS3.log pass counted reservation command rows 1925, reservation command exact-PC rows 51285, reservation verify rows 1925, reservation verify lane rows 4513. These counters are invalid for speed/HLE promotion because visuals and target selection failed.
- Classification: route-tooling, load-target-classifier-false-positive, debug-save-selected, damaged-save-marker-missed, failed-visual-gate, reservation-counters-present, not-speed. No 200 percent claim and no windows micro-win.
- Conclusion: no speed increase. The live gate is unsafe: missing Debug-Save and damaged-save exemplars let a Debug Save / damaged row pass as Path-to-Tenuto. Do not slot-confirm again until the classifier gets an explicit Debug Save / damaged-save guard or a real bad exemplar from this run.
- Next narrow step: repair tools/classify_eternal_sonata_load_target.ps1 using this run's Debug Save and damaged-save screenshots as bad/guard exemplars, then rerun only the target gate before any slot confirm, movement, first-battle route, or fast path.

### 2026-06-02T18:52:55-04:00 - load-target classifier bad-exemplar guard repair

- Refiner decision: still blocked automatic loader-control-left200 movement reruns and required route-state control, movement shrink/change, or SPU verifier/codegen analysis before more movement. The narrowest current route-state issue was the prior live gate false-positive, so this heartbeat stayed on a Windows-only harness/classifier repair and did not launch Android, ADB, Thor, movement, first-battle, HLE fast paths, or RSX/GPU behavior.
- Tooling change: updated `tools/classify_eternal_sonata_load_target.ps1` so the missing Debug-Save bad exemplar falls back to `debug-captures/windows-lab/20260602-182050-cpu4-livegate-slotconfirm-nomove-field-reproof-windows-windows/screenshots/screenshot-0085s-load-target-gate-2.png`, the exact prior false-positive frame. The damaged-save guard now also falls back to that frame and scans exemplar rows `190, 365, 535` instead of only `190, 365`.
- Offline false-positive recheck: `./tools/classify_eternal_sonata_load_target.ps1 -RunDir debug-captures/windows-lab/20260602-182050-cpu4-livegate-slotconfirm-nomove-field-reproof-windows-windows -NoWriteSummary` now returns `DEBUG_SAVE_PROLOGUE_PRESENT`, with counts `path-to-tenuto=0, debug-save-prologue=11, empty-load-slot=0, unknown=4`. Key rows `screenshot-0085s-load-target-gate-2.png` and `screenshot-0086s-path-target-before-slotfield.png` now classify `debug-save-prologue` with `BadDiff=0.000` instead of passing as Path-to-Tenuto.
- Offline good-target recheck: `./tools/classify_eternal_sonata_load_target.ps1 -RunDir debug-captures/windows-lab/20260602-175130-cpu4-titleload-livegatefix-targetonly-reproof-windows-windows -RequirePathToTenuto -NoWriteSummary` still returns `PATH_TO_TENUTO_PRESENT`, with counts `path-to-tenuto=6, debug-save-prologue=0, empty-load-slot=0, unknown=5`. Key rows `screenshot-0088s-load-target-gate-3.png` and `screenshot-0089s-path-target-after-livegatefix.png` remain `path-to-tenuto` with GoodDiff `0.001` and BadDiff `17.759`.
- Screenshot verification: reused the prior manual screenshot evidence. The repaired bad exemplar visibly shows `Save File 01 / Debug Save / Prologue` plus a damaged-save row; the good target artifact visibly shows `Path to Tenuto / South Section / Ch. 1 Raindrops`. No new field, Options/menu, or first-battle screenshots were produced this heartbeat.
- Fatal/log/counter verification from the prior false-positive artifact: visual status `NO_FIELD_LIKE_SCREENSHOT`; exact fatal/access/VK_ERROR/device-lost/unhandled/assert/crash counts all `0`; SPU verifier rows `0` because verifier was off; reservation command rows `1925`, reservation command exact-PC rows `51285`, reservation verify rows `1925`, reservation verify lane rows `4513`; GPU summary MFC dynamic `1695`, MFC list transfer `1064`, MFC wait `1926`, MFC wait exact-PC `91877`, RSX-local `0`, offload fit `spu-kernel-hle=886, too-small=809`.
- Classification: `route-tooling`, `harness-fix`, `load-target-classifier-guard-repaired`, `debug-save-false-positive-fixed`, `not-speed`. Counters remain invalid for speed/HLE promotion because the repaired artifact was a bad target/menu run, not field gameplay.
- Conclusion: no speed increase. The next narrow route step may rerun only a target gate first; do not slot-confirm, move, or attempt first battle until the live gate reports Path-to-Tenuto with the repaired Debug-Save guard active.

### 2026-06-02T19:51:55-04:00 - repaired live target gate stays unknown on black crash overlay

- Refiner decision: latest route-state work still blocked movement and speed/HLE/RSX experiments until the polling load-target gate reports `PATH_TO_TENUTO_PRESENT`. Chosen step was a Windows-only target-gate reproof with the repaired Debug-Save bad-exemplar guard active. No slot confirm, movement, first-battle route, Android, ADB, Thor, HLE fast path, or RSX/GPU behavior was run.
- Command: `./tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-titleload-guardlive-targetonly-reproof-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -EternalSonataGpuProbe Profile -WindowsHostContentionGate ExternalFail -WindowsVisualGate Off -InputMacro "wait:65000;shot:title-settle-guardlive;down:160;wait:900;shot:title-load-selected-guardlive;cross:120;wait:12000;shot:pre-guardlive;gate_load_target:45000;shot:path-target-after-guardlive" -MaxSeconds 150 -ScreenshotEverySeconds 20 -ScreenshotStartSeconds 70 -ScreenshotMaxCount 5 -HostSampleSeconds 1 -HostSampleEverySeconds 30`.
- Run: `debug-captures/windows-lab/20260602-192041-cpu4-titleload-guardlive-targetonly-reproof-windows-windows`.
- Screenshot verification: `screenshots/screenshot-0068s-title-settle-guardlive.png` showed the title menu with `LOAD` selected. All gate frames after entering load were black-overlay/small PNGs; manual `screenshots/screenshot-0125s-load-target-gate-10.png` showed a black screen with the RPCS3 overlay text `The PS3 application has likely crashed, you can close it.` No Load-list Path-to-Tenuto, field, Options/menu, or first-battle visual proof was produced.
- Load-target classifier: status `UNKNOWN_LOAD_TARGET`; counts `path-to-tenuto=0, debug-save-prologue=0, empty-load-slot=0, unknown=13`. Gate marker: `Load target gate failed at 125s: timed out after 45000ms; last status UNKNOWN_LOAD_TARGET; Load target gate failed: expected only Path to Tenuto, got UNKNOWN_LOAD_TARGET.` This confirms the repaired guard did not repeat the prior Debug-Save false-positive, but the live route still did not reach a valid target.
- Fatal/log verification: real fatal present: `VM: Access violation reading location 0x4 (unmapped memory)` at `PPU main_thread [0x0007dccc]`. Exact fatal count `0` by the line-prefix scan, but targeted access-violation count `1`; `VK_ERROR`, device lost, unhandled exception, assertion, and crash counts were `0`. Host contention stayed clean.
- SPU verifier rows: `0` because `EternalSonataSpuHleVerify` was off for this route-control reproof.
- Reservation/GPU counters: GPU summary emitted MFC dynamic `509`, MFC list transfer `331`, MFC wait `526`, MFC wait exact-PC `26473`, reservation-loop CSVs present, SPU HLE verifier records `0`, RSX-local traffic records `0`, and offload fit `spu-kernel-hle=255, too-small=254`. Manual RPCS3.log pass counted reservation command rows `526`, reservation command exact-PC rows `14393`, reservation verify rows `526`, reservation verify lane rows `1283`. These counters are invalid for promotion because visuals failed and the run hit a fatal access violation.
- Refiner after verification: latest run is now `failed-fatal-log` with load target `UNKNOWN_LOAD_TARGET`; it repeats `load-target-gate-failed-before-slot-cross` and says not to run speed/HLE/RSX experiments until a polling gate reaches `PATH_TO_TENUTO_PRESENT`.
- Classification: `failed-fatal-log`, `route-tooling`, `black-overlay-crash`, `load-target-unknown`, `reservation-counters-invalid`, `not-speed`. No 200 percent claim and no Windows micro-win.
- Conclusion: no speed increase. Next narrow step should inspect route/checkpoint state or add black-overlay crash-state handling before another slot confirm; do not stack movement or fast paths on this artifact.

### 2026-06-02T20:22:22-04:00 - black-overlay crash state added to load-target classifier

- Refiner decision: latest load-target gate still aborted before save-slot `Cross` with `UNKNOWN_LOAD_TARGET`, and the refiner again blocked speed/HLE/RSX experiments until a polling gate reaches `PATH_TO_TENUTO_PRESENT`. The prior ledger also required black-overlay crash-state handling before another slot confirm. This heartbeat therefore took a Windows-only harness/classifier step and did not launch Android, ADB, Thor, movement, first-battle, HLE fast paths, or RSX/GPU behavior.
- Tooling change: updated `tools/classify_eternal_sonata_load_target.ps1` with a black-overlay/crash guard. Unknown load-target screenshots with PNG size <= `150000` bytes now classify as `black-overlay-or-crash`, and summary status becomes `BLACK_OVERLAY_OR_CRASH_PRESENT` when no Path-to-Tenuto or Debug-Save rows are present.
- Offline latest-crash recheck: `./tools/classify_eternal_sonata_load_target.ps1 -RunDir debug-captures/windows-lab/20260602-192041-cpu4-titleload-guardlive-targetonly-reproof-windows-windows` now reports `BLACK_OVERLAY_OR_CRASH_PRESENT`; counts `path-to-tenuto=0, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=11, unknown=2`. Key frames `screenshot-0083s-load-target-gate.png` and `screenshot-0125s-load-target-gate-10.png` classify `black-overlay-or-crash` with PNG sizes around `33 KB`.
- Offline good-target recheck: `./tools/classify_eternal_sonata_load_target.ps1 -RunDir debug-captures/windows-lab/20260602-175130-cpu4-titleload-livegatefix-targetonly-reproof-windows-windows -RequirePathToTenuto -NoWriteSummary` still reports `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=6, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=5`. Key rows `screenshot-0088s-load-target-gate-3.png` and `screenshot-0089s-path-target-after-livegatefix.png` remain `path-to-tenuto`.
- Offline bad-target recheck: `./tools/classify_eternal_sonata_load_target.ps1 -RunDir debug-captures/windows-lab/20260602-182050-cpu4-livegate-slotconfirm-nomove-field-reproof-windows-windows -NoWriteSummary` still reports `DEBUG_SAVE_PROLOGUE_PRESENT`; counts `path-to-tenuto=0, debug-save-prologue=11, empty-load-slot=0, black-overlay-or-crash=0, unknown=4`. The Debug-Save bad exemplar guard remains intact.
- Screenshot/log/counter verification source: reused the verified latest crash artifact. Title screenshot showed `LOAD`; final load-gate screenshot was black overlay with RPCS3 crash text. Fatal verification had real `VM: Access violation reading location 0x4`; SPU verifier rows `0`; reservation command rows `526`, reservation command exact-PC rows `14393`, reservation verify rows `526`, reservation verify lane rows `1283`; GPU summary MFC dynamic `509`, MFC list transfer `331`, MFC wait `526`, MFC wait exact-PC `26473`, RSX-local `0`, offload fit `spu-kernel-hle=255, too-small=254`.
- Classification: `route-tooling`, `harness-fix`, `black-overlay-crash-state-classifier`, `not-speed`. This does not convert the failed artifact into proof; it only prevents the route machinery/refiner from treating black crash frames as generic unknown target screens.
- Conclusion: no speed increase. Next narrow step can inspect save/checkpoint state or rerun only a guarded target gate that must reach `PATH_TO_TENUTO_PRESENT`; do not stack movement, first-battle, or fast paths on black/crash artifacts.

### 2026-06-02T20:54:36-04:00 - refiner polling route rejects Debug Save before slot confirm

- Refiner decision: latest load-target gate was `BLACK_OVERLAY_OR_CRASH_PRESENT`, so the refiner again blocked speed/HLE/RSX experiments and required a polling load-target gate that must reach `PATH_TO_TENUTO_PRESENT` before continuing. This heartbeat ran the refiner-proposed Windows-only polling route once. No Android, ADB, Thor, HLE fast path, or RSX/GPU behavior was run.
- Command: `./tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-stateaware-loadtarget-pollgated-doubleconfirm-dismisssave-left200-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 215 -InputMacro "wait:45000;down:20;wait:500;cross:80;wait:12000;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:500;gate_load_target:30000;cross:80;wait:3000;up:80;wait:500;cross:80;wait:32000;cross:120;wait:18000;shot:100;up:80;wait:300;cross:120;wait:1200;cross:120;wait:35000;shot:100;down:80;wait:300;cross:120;wait:1500;ls_left:200;wait:1000;shot:100;wait:10000;shot:100" -MaxSeconds 260 -ScreenshotEverySeconds 10 -ScreenshotStartSeconds 155 -ScreenshotMaxCount 10`.
- Run: `debug-captures/windows-lab/20260602-205050-cpu4-stateaware-loadtarget-pollgated-doubleconfirm-dismisssave-left200-visualgate-windows-windows`.
- Route/gate result: the macro aborted before save-slot `Cross` and before any movement because the load-target gate returned `DEBUG_SAVE_PROLOGUE_PRESENT` at `63s`, not `PATH_TO_TENUTO_PRESENT`. Gate marker: `Load target gate failed at 63s: Load target gate failed: expected only Path to Tenuto, got DEBUG_SAVE_PROLOGUE_PRESENT.`
- Screenshot verification: the only screenshot, `screenshots/screenshot-0063s-load-target-gate.png`, visibly shows `Save File 01 / Debug Save / Prologue` with damaged-save rows. No Path-to-Tenuto target, field, Options/menu, or first-battle visual proof was produced.
- Load-target classifier: status `DEBUG_SAVE_PROLOGUE_PRESENT`; counts `path-to-tenuto=0, debug-save-prologue=1, empty-load-slot=0, black-overlay-or-crash=0, unknown=0`. This confirms the repaired bad-save guard rejected the wrong target cleanly instead of false-passing.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot none; expected because the route aborted at the load gate.
- Fatal/log verification: exact fatal/access-violation/VK_ERROR/device-lost/unhandled-exception/assert/crash scans were all `0`; host snapshots were clean prelaunch, postlaunch, and postrun.
- SPU verifier rows: `0` because `EternalSonataSpuHleVerify` was off for this route-control run.
- Reservation/GPU counters: GPU probe summary emitted MFC dynamic `436`, MFC list transfer `292`, MFC wait `469`, MFC wait exact-PC `23113`, reservation-loop command records `469`, reservation-loop command exact-PC records `12705`, reservation-loop verify records `1596`, RDCH join `3`, lane join `3`, raw-lane `8`, RSX-local traffic `0`, offload fit `spu-kernel-hle=220, too-small=216`. Manual RPCS3.log pass counted reservation command rows `469`, reservation command exact-PC rows `12705`, reservation verify rows `469`, reservation verify lane rows `1127`. These counters are invalid for speed/HLE promotion because visuals and target selection failed.
- Classification: `route-tooling`, `load-target-gate-correct-reject`, `debug-save-selected`, `failed-visual-gate`, `reservation-counters-invalid`, `not-speed`. No 200 percent proof, no Windows micro-win, and no first-battle proof.
- Conclusion: no speed increase. Next narrow step should inspect why the current title-to-Load route lands on Debug Save / damaged rows, or add a save-list/checkpoint-state repair that proves Path-to-Tenuto before any slot confirm, movement, or fast path.

### 2026-06-02T21:28:58-04:00 - save-list inventory shows initial Path row and Down drift to empty/damaged rows

- Refiner decision: latest state-aware polling repair selected `Debug Save / Prologue`, so the refiner said not to repeat the same polling macro and instead to inventory current save-list rows with repeated Down screenshots and no slot `Cross`. This heartbeat followed that Windows-only route-state diagnostic. No Android, ADB, Thor, slot confirm, movement, first-battle route, HLE fast path, or RSX/GPU behavior was run.
- Command: `./tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-save-list-inventory-after-debugsave-reject-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -EternalSonataGpuProbe Profile -WindowsVisualGate Off -InputMacro "wait:65000;shot:title-settle-before-inventory;down:160;wait:900;shot:title-after-down160-inventory;cross:120;wait:60000;shot:load-list-initial-after60;down:120;wait:900;shot:load-list-after-down1;down:120;wait:900;shot:load-list-after-down2;down:120;wait:900;shot:load-list-after-down3;down:120;wait:900;shot:load-list-after-down4;down:120;wait:900;shot:load-list-after-down5;down:120;wait:900;shot:load-list-after-down6" -MaxSeconds 165 -ScreenshotEverySeconds 0 -ScreenshotStartSeconds 0 -ScreenshotMaxCount 0 -HostSampleSeconds 1 -HostSampleEverySeconds 30`.
- Run: `debug-captures/windows-lab/20260602-212046-cpu4-save-list-inventory-after-debugsave-reject-windows-windows`.
- Screenshot verification: `screenshots/screenshot-0068s-title-settle-before-inventory.png` and `screenshots/screenshot-0069s-title-after-down160-inventory.png` were title/menu setup. `screenshots/screenshot-0130s-load-list-initial-after60.png` showed the correct top row: `Save File 01 / Path to Tenuto / South Section / Ch. 1 Raindrops`, with lower rows `File does not exist`. `screenshots/screenshot-0132s-load-list-after-down1.png` showed the cursor on the first lower empty row while the top preview still displayed Path-to-Tenuto. `screenshots/screenshot-0140s-load-list-after-down6.png` showed only empty `File does not exist` rows with the cursor on a lower row. No field, Options/menu, or first-battle visual proof was produced.
- Offline classifier inventory: status `DAMAGED_SAVE_TARGET`; counts `path-to-tenuto=3, debug-save-prologue=0, empty-load-slot=4, black-overlay-or-crash=0, unknown=2`. Rows `load-list-initial-after60`, `after-down1`, and `after-down2` classified as Path preview, but `after-down1` had lower cursor marker `365` and `after-down2` had lower cursor marker `535`; rows `after-down3` through `after-down6` classified `empty-load-slot`. This confirms the important route gotcha: Path preview alone is not enough once Down inputs drift the cursor to lower rows.
- Fatal/log verification: exact fatal/access-violation/VK_ERROR/device-lost/unhandled-exception/assert/crash scans were all `0`. Host contention stayed clean across `5` snapshots.
- SPU verifier rows: `0` because `EternalSonataSpuHleVerify` was off for this route-state inventory.
...
## 2026-06-03T01:07:56-04:00 - repaired post-load dismiss route survives left400 one-step field movement

- Automation: ps3-200-windows-speed-loop heartbeat 2026-06-03T04:50:08.058Z.
- Refiner run: tools\ps3_harness_refiner.ps1 -MaxRuns 8; decision still said to use the newest valid-field run as the route base and add one small state-aware movement step with CleanAfterField.
- Non-duplicative Windows-only step: kept the repaired post-Load-complete dismiss route and replaced the prior left200 movement with one ls_left:400 pulse. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Command label: cpu4-postload-dismiss-left400-one-step-visualgate-windows; Windows-only, PadApi backend, game screen 1, CPU affinity 0x0F, frame/vblank 240, reservation loop Verify, host contention gate ExternalFail, visual gate CleanAfterField.
- Capture: debug-captures/windows-lab/20260603-005109-cpu4-postload-dismiss-left400-one-step-visualgate-windows-windows.
- Visual gate: FIELD_LIKE_PRESENT; first field-like screenshot was screenshot-0309s-field-before-left400-postload.png at 309s; required field-like by 335s passed; invalid screenshots after first field-like = 0; class counts were field-like-large-png=21, wrong-window-or-other-small-png=5, cutscene-or-nonfield-large-png=2.
- Screenshot verification: screenshot-0309s-field-before-left400-postload.png showed a clean Path-to-Tenuto field before movement; screenshot-0311s-field-after-left400-postload.png showed Polka shifted left after the ls_left:400 pulse with clean field visuals; screenshot-0326s-field-late-left400-postload.png stayed clean late after movement.
- Load target verifier: PATH_TO_TENUTO_PRESENT; counts were path-to-tenuto=2, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2, lower-row cursor markers=0, damaged-save text markers=2.
- Fatal/log verification: no fatal/access-violation/VK_ERROR/device-lost/unhandled/assert/unknown STOP/crash lines after excluding the harmless Show fatal error hints config line; stdout and stderr were empty; no RPCS3 process remained after the run; host contention stayed clean across 5 snapshots.
- SPU verifier rows: 0.
- Reservation/GPU counters: MFC dynamic=2415, MFC list=1388, MFC wait=2674, MFC wait exact-PC=134150, reservation command=2673, reservation command exact-PC=72143, reservation verify=9138 summary records / 2673 exact log rows, reservation lane=6465, total observed DMA=3140.22 MB, SPU HLE verifier records=0, RSX-local traffic records=0, offload fit mix spu-kernel-hle=1523 and too-small=892.
- Classification: valid-field-triage, route-tooling, stateaware-route-repaired, post-load-dismiss-left400, clean-small-movement, reservation-counters-present, not-speed.
- Conclusion: no speed increase. The repaired route now survives a larger single left pulse than the previous left200 proof, but this remains route-control evidence only because there is still no same-run Options/menu proof and no first-battle proof. The refiner's stock printed state-aware macro is still stale and should be updated to emit the repaired post-load dismiss route, or the next heartbeat should continue using the explicit repaired route for the next narrow proof.

## 2026-06-03T01:39:16-04:00 - repaired post-load dismiss route survives left800 one-step field movement

- Automation: ps3-200-windows-speed-loop heartbeat 2026-06-03T05:20:08.347Z.
- Refiner run: tools\ps3_harness_refiner.ps1 -MaxRuns 8; decision still said to use the newest valid-field run as the route base and add one small state-aware movement step with CleanAfterField.
- Non-duplicative Windows-only step: kept the repaired post-Load-complete dismiss route and replaced the prior left400 movement with one ls_left:800 pulse. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Command label: cpu4-postload-dismiss-left800-one-step-visualgate-windows; Windows-only, PadApi backend, game screen 1, CPU affinity 0x0F, frame/vblank 240, reservation loop Verify, host contention gate ExternalFail, visual gate CleanAfterField.
- Capture: debug-captures/windows-lab/20260603-012138-cpu4-postload-dismiss-left800-one-step-visualgate-windows-windows.
- Visual gate: FIELD_LIKE_PRESENT; first field-like screenshot was screenshot-0309s-field-before-left800-postload.png at 309s; required field-like by 335s passed; invalid screenshots after first field-like = 0; class counts were field-like-large-png=21, wrong-window-or-other-small-png=5, cutscene-or-nonfield-large-png=2.
- Screenshot verification: screenshot-0309s-field-before-left800-postload.png showed a clean Path-to-Tenuto field before movement; screenshot-0312s-field-after-left800-postload.png showed Polka shifted farther left after the ls_left:800 pulse with clean field visuals; screenshot-0327s-field-late-left800-postload.png stayed clean late after movement.
- Load target verifier: PATH_TO_TENUTO_PRESENT; counts were path-to-tenuto=2, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2, lower-row cursor markers=0, damaged-save text markers=2.
- Fatal/log verification: no fatal/access-violation/VK_ERROR/device-lost/unhandled/assert/unknown STOP/crash lines after excluding the harmless Show fatal error hints config line; stdout and stderr were empty; no RPCS3 process remained after the run; host contention stayed clean across 5 snapshots per run output.
- SPU verifier rows: 0.
- Reservation/GPU counters: MFC dynamic=2634, MFC list=1566, MFC wait=2949, MFC wait exact-PC=145449, reservation command=9964 summary records / 2949 exact cmd rows, reservation command exact-PC=79138, reservation verify=9964 summary records / 2948 exact log rows, reservation lane=7016, total observed DMA=3319.45 MB, SPU HLE verifier records=0, RSX-local traffic records=0, offload fit mix spu-kernel-hle=1575 and too-small=1059.
- Classification: valid-field-triage, route-tooling, stateaware-route-repaired, post-load-dismiss-left800, clean-small-movement, reservation-counters-present, not-speed.
- Conclusion: no speed increase. The repaired route now survives a single left800 pulse, expanding the clean movement envelope beyond left400. This is still only route-control evidence because there is no same-run Options/menu proof and no first-battle proof. The refiner's stock printed state-aware macro is still stale and should be updated to emit the repaired post-load dismiss route, or the next heartbeat should continue using the explicit repaired route for the next narrow proof.

## 2026-06-03T02:08:14-04:00 - repaired post-load dismiss route survives left1200 one-step field movement

- Automation: ps3-200-windows-speed-loop heartbeat 2026-06-03T05:50:10.140Z.
- Refiner run: tools\ps3_harness_refiner.ps1 -MaxRuns 8; decision still said to use the newest valid-field run as the route base and add one small state-aware movement step with CleanAfterField.
- Non-duplicative Windows-only step: kept the repaired post-Load-complete dismiss route and replaced the prior left800 movement with one ls_left:1200 pulse. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Command label: cpu4-postload-dismiss-left1200-one-step-visualgate-windows; Windows-only, PadApi backend, game screen 1, CPU affinity 0x0F, frame/vblank 240, reservation loop Verify, host contention gate ExternalFail, visual gate CleanAfterField.
- Capture: debug-captures/windows-lab/20260603-015128-cpu4-postload-dismiss-left1200-one-step-visualgate-windows-windows.
- Visual gate: FIELD_LIKE_PRESENT; first field-like screenshot was screenshot-0309s-field-before-left1200-postload.png at 309s; required field-like by 335s passed; invalid screenshots after first field-like = 0; class counts were field-like-large-png=21, wrong-window-or-other-small-png=5, cutscene-or-nonfield-large-png=2.
- Screenshot verification: screenshot-0309s-field-before-left1200-postload.png showed a clean Path-to-Tenuto field before movement; screenshot-0312s-field-after-left1200-postload.png showed Polka shifted left onto the path after the ls_left:1200 pulse with clean field visuals; screenshot-0327s-field-late-left1200-postload.png stayed clean late after movement.
- Load target verifier: PATH_TO_TENUTO_PRESENT; counts were path-to-tenuto=2, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2, lower-row cursor markers=0, damaged-save text markers=2.
- Fatal/log verification: no fatal/access-violation/VK_ERROR/device-lost/unhandled/assert/unknown STOP/crash lines after excluding the harmless Show fatal error hints config line; stdout and stderr were empty; no RPCS3 process remained after the run; host contention stayed clean across 5 snapshots per run output.
- SPU verifier rows: 0.
- Reservation/GPU counters: MFC dynamic=2561, MFC list=1472, MFC wait=2892, MFC wait exact-PC=141951, reservation command=2892 summary records / 2892 exact cmd rows, reservation command exact-PC=77387, reservation verify=9766 summary records / 2891 exact log rows, reservation lane=6875, total observed DMA=3122.87 MB, SPU HLE verifier records=0, RSX-local traffic records=0, offload fit mix spu-kernel-hle=1530 and too-small=1031.
- Classification: valid-field-triage, route-tooling, stateaware-route-repaired, post-load-dismiss-left1200, clean-field-movement, reservation-counters-present, not-speed.
- Conclusion: no speed increase. The repaired route now survives a single left1200 pulse and gets close to the historical left1317 clean lower bound without hitting the old left1318 fatal bracket. This remains route-control evidence only because there is no same-run Options/menu proof and no first-battle proof. The refiner's stock printed state-aware macro is still stale and should be updated to emit the repaired post-load dismiss route, or the next heartbeat should continue using the explicit repaired route for the next narrow proof.

## 2026-06-03T02:38:38-04:00 - repaired post-load dismiss left1317 attempt stalls on Load screen

- Automation: ps3-200-windows-speed-loop heartbeat 2026-06-03T06:20:11.274Z.
- Refiner run: tools\ps3_harness_refiner.ps1 -MaxRuns 8; decision still said to use the newest valid-field run as the route base and add one small state-aware movement step with CleanAfterField. The printed command remains the stale short state-aware macro without the repaired post-Load-complete dismiss, so this step used the explicit repaired route.
- Non-duplicative Windows-only step: kept the repaired post-Load-complete dismiss route and tested one ls_left:1317 pulse to reconnect the repaired route with the historical left1317-clean / left1318-fatal bracket. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Command label: cpu4-postload-dismiss-left1317-one-step-visualgate-windows; Windows-only, PadApi backend, game screen 1, CPU affinity 0x0F, frame/vblank 240, reservation loop Verify, host contention gate ExternalFail, visual gate CleanAfterField.
- Capture: debug-captures/windows-lab/20260603-022143-cpu4-postload-dismiss-left1317-one-step-visualgate-windows-windows.
- Visual gate: NO_FIELD_LIKE_SCREENSHOT; first field-like screenshot none; required field-like by 335s failed; class counts were wrong-window-or-other-small-png=26 and cutscene-or-nonfield-large-png=2.
- Screenshot verification: screenshot-0309s-field-before-left1317-postload.png, screenshot-0312s-field-after-left1317-postload.png, and screenshot-0327s-field-late-left1317-postload.png all visibly stayed on the Load menu with Load complete shown, not Path-to-Tenuto field gameplay. The ls_left:1317 movement was therefore not actually tested in field.
- Load target verifier: initial live gate passed PATH_TO_TENUTO_PRESENT; counts were path-to-tenuto=2, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2, lower-row cursor markers=0, damaged-save text markers=2. Later screenshots nevertheless remained on the Load menu, so the route failed after the target gate/load-complete stage.
- Fatal/log verification: no fatal/access-violation/VK_ERROR/device-lost/unhandled/assert/unknown STOP/crash lines after excluding the harmless Show fatal error hints config line; stdout and stderr were empty; no RPCS3 process remained after the run; host contention stayed clean across 5 snapshots per run output.
- SPU verifier rows: 0.
- Reservation/GPU counters: MFC dynamic=2569, MFC list=1676, MFC wait=2977, MFC wait exact-PC=139357, reservation command=2977, reservation command exact-PC=78853, reservation verify=9860 summary records / 2977 exact log rows, reservation lane=6883, total observed DMA=2481.14 MB, SPU HLE verifier records=0, RSX-local traffic records=0, offload fit mix too-small=1307 and spu-kernel-hle=1262.
- Classification: route-tooling, failed-visual-gate, load-complete-stuck, repaired-left1317-not-field-tested, reservation-counters-invalid, not-speed.
- Conclusion: no speed increase. The repaired route is clean through left1200, but the left1317 repaired-route attempt did not reach field after Load complete and cannot be treated as movement, speed, Options/menu, first-battle, HLE, or GPU evidence. This is not the old left1318 fatal/corrupt-field pattern because fatal logs were clean; the next narrow action should either bisect between left1200 and left1317 with the repaired route or repair the post-Load-complete dismiss state before retrying upper movement.

## 2026-06-03T03:09:07-04:00 - repaired post-load dismiss route survives left1258 bisection movement

- Automation: ps3-200-windows-speed-loop heartbeat 2026-06-03T06:50:11.950Z.
- Refiner run: tools\ps3_harness_refiner.ps1 -MaxRuns 8; decision still said to use the newest valid-field run as the route base and add one small state-aware movement step with CleanAfterField. The printed command remains the stale short state-aware macro without the repaired post-Load-complete dismiss, so this step used the explicit repaired route.
- Non-duplicative Windows-only step: bisected the repaired-route boundary between left1200 clean and left1317 Load-complete stuck by testing one ls_left:1258 pulse. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Command label: cpu4-postload-dismiss-left1258-one-step-visualgate-windows; Windows-only, PadApi backend, game screen 1, CPU affinity 0x0F, frame/vblank 240, reservation loop Verify, host contention gate ExternalFail, visual gate CleanAfterField.
- Capture: debug-captures/windows-lab/20260603-025159-cpu4-postload-dismiss-left1258-one-step-visualgate-windows-windows.
- Visual gate: FIELD_LIKE_PRESENT; first field-like screenshot was screenshot-0309s-field-before-left1258-postload.png at 309s; required field-like by 335s passed; invalid screenshots after first field-like = 0; class counts were field-like-large-png=21, wrong-window-or-other-small-png=5, cutscene-or-nonfield-large-png=2.
- Screenshot verification: screenshot-0309s-field-before-left1258-postload.png showed a clean Path-to-Tenuto field before movement; screenshot-0312s-field-after-left1258-postload.png showed Polka shifted left onto the path after the ls_left:1258 pulse with clean field visuals; screenshot-0327s-field-late-left1258-postload.png stayed clean late after movement.
- Load target verifier: PATH_TO_TENUTO_PRESENT; counts were path-to-tenuto=2, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2, lower-row cursor markers=0, damaged-save text markers=2.
- Fatal/log verification: no fatal/access-violation/VK_ERROR/device-lost/unhandled/assert/unknown STOP/crash lines after excluding the harmless Show fatal error hints config line; stdout and stderr were empty; no RPCS3 process remained after the run; host contention stayed clean across 5 snapshots per run output.
- SPU verifier rows: 0.
- Reservation/GPU counters: MFC dynamic=2495, MFC list=1432, MFC wait=2853, MFC wait exact-PC=139400, reservation command=2853, reservation command exact-PC=76228, reservation verify=9662 summary records / 2853 exact log rows, reservation lane=6809, total observed DMA=3036.48 MB, SPU HLE verifier records=0, RSX-local traffic records=0, offload fit mix spu-kernel-hle=1492 and too-small=1003.
- Classification: valid-field-triage, route-tooling, repaired-boundary-bisection, post-load-dismiss-left1258, clean-field-movement, reservation-counters-present, not-speed.
- Conclusion: no speed increase. The repaired route now has a clean bisection proof at left1258, narrowing the repaired-route boundary to left1258 clean versus left1317 stuck on the Load-complete menu. This remains route-control evidence only because there is no same-run Options/menu proof and no first-battle proof. The next narrow action should continue bisection above left1258 or repair/refine post-Load-complete dismissal near the upper movement range before any speed/HLE/RSX/GPU claim.

## 2026-06-03T03:40:00-04:00 - repaired post-load dismiss left1287 reaches field then fatal-crashes
- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T07:20:13.305Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` again selected the newest valid-field route base and one small state-aware movement step with `CleanAfterField`; the printed stock macro still lacks the repaired post-load dismiss, so this run used the repaired load confirmation route explicitly.
- Non-duplicative Windows-only step: bisected the repaired post-load boundary between clean `ls_left:1258` and stuck `ls_left:1317` using one `ls_left:1287` movement after the field screenshot.
- Capture: `debug-captures/windows-lab/20260603-032202-cpu4-postload-dismiss-left1287-one-step-visualgate-windows-windows`.
- Visual gate: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0309s-field-before-left1287-postload.png` at `309s` (`2.50 MB`); required field-like by `335s` passed; invalid after first field-like `0`; class counts `field-like-large-png=21`, `wrong-window-or-other-small-png=5`, `cutscene-or-nonfield-large-png=2`; gate result `passed-for-triage`.
- Screenshot verification: `screenshot-0309s-field-before-left1287-postload.png` is clean field gameplay before movement. `screenshot-0312s-field-after-left1287-postload.png` and `screenshot-0327s-field-late-left1287-postload.png` remain field-like visually, but both show RPCS3's likely-crashed overlay after the `ls_left:1287` step, so this is not a clean movement proof.
- Load target verification: `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=2`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=2`; lower-row cursor markers `0`; damaged-save text markers `2`.
- Fatal/log verification: RPCS3 was no longer running. Fatal scan found a PPU access violation in `RPCS3.log` line `244432` and mirrored in `rpcs3.stderr.txt` (`RPCS3: PPU[0x100000c] Thread () [0x002aedd0]: VM: Access violation reading location 0x40 (unmapped memory)`). `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `108`.
- SPU verifier rows: `0`; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic `2395`, MFC list `1413`, MFC wait `2724`, MFC wait exact-PC `132618`, reservation command `2724`, reservation command exact-PC `72759`, reservation verify `9181`, reservation lane-join `3`, reservation raw-lane `8`, total observed DMA `2754.27 MB`, RSX-local `0`, offload fit mix `spu-kernel-hle=1378`, `too-small=1017`.
- Classification: `valid-field-triage`, `route-tooling`, `repaired-boundary-bisection`, `post-load-dismiss-left1287`, `field-entry-then-fatal`, `access-violation`, `reservation-counters-present-but-invalid-for-speed`, `not-speed`.
- Conclusion: no speed increase. The repaired route boundary is now sharper: `left1258` is clean field movement, `left1287` reaches field but fatal-crashes after movement, and `left1317` remains Load-complete stuck. Continue from `left1258` or bisect below `left1287`; do not use this fatal run for 200% proof, Options proof, first-battle proof, or counter-derived speed claims.

## 2026-06-03T04:07:40-04:00 - repaired no-movement control is clean after left1287 fatal
- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T07:50:13.768Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T03:50:39.2790354-04:00` and changed the next action after the `left1287` access violation to a no-movement loader/control with `CleanAfterField` before adding more movement. The printed stock macro is still the short loader-control route, so this run kept the explicit repaired post-load dismiss route and removed movement.
- Non-duplicative Windows-only step: repaired post-load dismiss no-movement control after the `left1287` fatal boundary. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, first-battle route, or movement pulse was run.
- Capture: `debug-captures/windows-lab/20260603-035102-cpu4-postload-dismiss-nomove-after-left1287-fatal-visualgate-windows-windows`.
- Visual gate: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0309s-field-before-nomove-after-left1287-fatal.png` at `309s` (`2.49 MB`); required field-like by `335s` passed; invalid after first field-like `0`; class counts `field-like-large-png=19`, `wrong-window-or-other-small-png=5`, `cutscene-or-nonfield-large-png=2`; gate result `passed-for-triage`.
- Screenshot verification: `screenshot-0309s-field-before-nomove-after-left1287-fatal.png`, `screenshot-0324s-field-late-nomove-after-left1287-fatal.png`, and `screenshot-0340s.png` all show clean Path-to-Tenuto field gameplay with no crash overlay, no visible field corruption, and no Load-menu stall. The harness stopped RPCS3 at the `340s` max wall-time after clean field frames, not because of a crash.
- Load target verification: `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=2`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=2`; lower-row cursor markers `0`; damaged-save text markers `2`.
- Fatal/log verification: no RPCS3 process remained after the harness stop. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Host contention stayed clean across `5` snapshots with the external gate clean.
- SPU verifier rows: `0`; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic `2419`, MFC list `1389`, MFC wait `2776`, MFC wait exact-PC `135200`, reservation command `2776`, reservation command exact-PC `73996`, reservation verify `9332`, reservation lane-join `3`, reservation raw-lane `8`, total observed DMA `3016.75 MB`, RSX-local `0`, offload fit mix `spu-kernel-hle=1457`, `too-small=962`.
- Classification: `valid-field-triage`, `route-tooling`, `repaired-no-movement-control`, `post-left1287-fatal-control-clean`, `reservation-counters-present-but-route-only`, `not-speed`.
- Conclusion: no speed increase. The repaired route itself remains clean after the `left1287` fatal, so the fatal appears tied to the upper movement step rather than basic load/field stability. Continue movement bisection from the clean side below `left1287` only; this run is not Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence.

## 2026-06-03T04:39:20-04:00 - repaired left1272 bisection stalls on Now Loading
- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T08:20:14.252Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T04:20:45.5389806-04:00` and selected the newest valid-field run as the route base with one small state-aware movement step under `CleanAfterField`. The printed macro remains the stale short state-aware route, so this run kept the explicit repaired post-load dismiss route.
- Non-duplicative Windows-only step: bisected below the `left1287` fatal movement with one `ls_left:1272` attempt, between clean `left1258` and fatal `left1287`. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Capture: `debug-captures/windows-lab/20260603-042106-cpu4-postload-dismiss-left1272-one-step-visualgate-windows-windows`.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot none; required field-like by `335s` failed; class counts `loading-like-small-png=21`, `wrong-window-or-other-small-png=5`, `cutscene-or-nonfield-large-png=2`; gate result `failed`.
- Screenshot verification: `screenshot-0309s-field-before-left1272-postload.png`, `screenshot-0312s-field-after-left1272-postload.png`, and `screenshot-0327s-field-late-left1272-postload.png` all visibly show the `Now Loading...` screen, not Path-to-Tenuto field gameplay. The `ls_left:1272` pulse therefore was not actually tested in field.
- Load target verification: `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=2`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=2`; lower-row cursor markers `0`; damaged-save text markers `2`.
- Fatal/log verification: no RPCS3 process remained after the harness stop. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Host contention stayed clean across `5` snapshots with the external gate clean.
- SPU verifier rows: `0`; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic `2695`, MFC list `1196`, MFC wait `3000`, MFC wait exact-PC `155058`, reservation command `3000`, reservation command exact-PC `81566`, reservation verify `10520`, reservation lane-join `3`, reservation raw-lane `8`, total observed DMA `3112.55 MB`, RSX-local `0`, offload fit mix `spu-kernel-hle=1770`, `too-small=925`.
- Classification: `route-tooling`, `failed-visual-gate`, `now-loading-stall`, `post-load-dismiss-left1272-not-field-tested`, `reservation-counters-invalid`, `not-speed`.
- Conclusion: no speed increase. The repaired movement boundary is now more nuanced: `left1258` is the last clean field movement, `left1272` stalls on `Now Loading...` before field, `left1287` reaches field then fatal-crashes after movement, and `left1317` stalls on the Load-complete menu. Continue by repairing or stabilizing the post-load transition before more upper movement, or bisect closer to `left1258`; do not use this run for Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed claims.

## 2026-06-03T05:05:45-04:00 - repaired left1265 attempt loses RPCS3 window after apparent field point
- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T08:50:14.653Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T04:50:52.5958233-04:00` and still selected the newest valid-field route base with one small state-aware movement step under `CleanAfterField`. The printed macro remains the stale short state-aware route, so this run kept the explicit repaired post-load dismiss route.
- Non-duplicative Windows-only step: retreated from the `left1272` Now Loading stall toward the last clean movement proof by testing one `ls_left:1265` pulse. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Capture: `debug-captures/windows-lab/20260603-045119-cpu4-postload-dismiss-left1265-one-step-visualgate-windows-windows`.
- Visual gate summary: `FIELD_LIKE_PRESENT_WITH_LATER_INVALID_SCREENSHOTS`; first field-like according to the byte classifier was `screenshot-0309s-field-before-left1265-postload.png` at `309s` (`2.49 MB`); invalid screenshots after first field-like `1`, first `screenshot-0312s-field-after-left1265-postload.png` at `312s` (`76.96 KB`); required field-like by `335s` passed; class counts `field-like-large-png=1`, `wrong-window-or-other-small-png=6`, `cutscene-or-nonfield-large-png=2`; gate result `failed`.
- Screenshot verification: manual viewing overrode the byte classifier. `screenshot-0309s-field-before-left1265-postload.png` and `screenshot-0312s-field-after-left1265-postload.png` both show the GitHub browser/repo page, not RPCS3 Path-to-Tenuto field gameplay. The run output also skipped the late screenshot because the game window was not found and the process had exited, so the `ls_left:1265` pulse was not proven in field.
- Load target verification: `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=2`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=2`; lower-row cursor markers `0`; damaged-save text markers `2`.
- Fatal/log verification: no RPCS3 process remained after the run. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. Broad fatal/crash/access/assert scan excluding label/path/config noise also found no matches. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Host contention stayed clean across `3` snapshots with the external gate clean.
- SPU verifier rows: `0`; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic `2357`, MFC list `1432`, MFC wait `2622`, MFC wait exact-PC `128959`, reservation command `2621`, reservation command exact-PC `70510`, reservation verify `8877`, reservation lane-join `3`, reservation raw-lane `8`, total observed DMA `2730.64 MB`, RSX-local `0`, offload fit mix `spu-kernel-hle=1353`, `too-small=1004`.
- Classification: `route-tooling`, `failed-visual-gate`, `wrong-window-capture`, `window-lost-after-load`, `post-load-dismiss-left1265-not-field-proven`, `reservation-counters-invalid`, `not-speed`.
- Conclusion: no speed increase. `left1265` is not a clean movement proof: the capture lost RPCS3 and manual screenshots show the browser window. The only clean repaired movement proof remains `left1258`; `left1272` stalls on `Now Loading...`, `left1287` reaches field then fatal-crashes after movement, and `left1317` stalls on the Load-complete menu. Next work should stabilize/repair window capture or post-load transition before any higher movement, Options/menu, first-battle, HLE/GPU, or 200% claim.

## 2026-06-03T05:39:30-04:00 - repaired left1261 bisection restores clean field movement
- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T09:20:15.147Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T05:20:59.8614973-04:00` and again selected the newest valid-field route base with one small state-aware movement step under `CleanAfterField`. The printed macro remains the stale short state-aware route, so this run kept the explicit repaired post-load dismiss route.
- Non-duplicative Windows-only step: bisected below the invalid `left1265`/`left1272` results with one `ls_left:1261` pulse, using `left1258` as the last clean movement proof. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Capture: `debug-captures/windows-lab/20260603-052151-cpu4-postload-dismiss-left1261-one-step-visualgate-windows-windows`.
- Visual gate: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0309s-field-before-left1261-postload.png` at `309s` (`2.49 MB`); required field-like by `335s` passed; invalid after first field-like `0`; class counts `field-like-large-png=21`, `wrong-window-or-other-small-png=5`, `cutscene-or-nonfield-large-png=2`; gate result `passed-for-triage`.
- Screenshot verification: `screenshot-0309s-field-before-left1261-postload.png` shows clean Path-to-Tenuto field before movement. `screenshot-0312s-field-after-left1261-postload.png` shows Polka shifted left onto the path after `ls_left:1261` with clean field visuals. `screenshot-0327s-field-late-left1261-postload.png` stays clean after movement with no crash overlay, loading screen, or wrong-window capture.
- Load target verification: `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=2`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=2`; lower-row cursor markers `0`; damaged-save text markers `2`.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. Broad fatal/crash/access/assert scan excluding label/path/config noise also found no matches. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Host contention stayed clean across `5` snapshots with the external gate clean.
- SPU verifier rows: `0`; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic `2408`, MFC list `1420`, MFC wait `2753`, MFC wait exact-PC `133995`, reservation command `2753`, reservation command exact-PC `73436`, reservation verify `9234`, reservation lane-join `3`, reservation raw-lane `8`, total observed DMA `2844.07 MB`, RSX-local `0`, offload fit mix `spu-kernel-hle=1417`, `too-small=991`.
- Classification: `valid-field-triage`, `route-tooling`, `repaired-boundary-bisection`, `post-load-dismiss-left1261`, `clean-field-movement`, `reservation-counters-present`, `not-speed`.
- Conclusion: no speed increase. The repaired movement boundary now has a fresh clean proof at `left1261`, while `left1265` remains invalid/window-lost, `left1272` stalls on `Now Loading...`, `left1287` reaches field then fatal-crashes after movement, and `left1317` stalls on the Load-complete menu. This is not Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence.

## 2026-06-03T06:05:25-04:00 - repaired left1263 bisection loses RPCS3 window before movement proof
- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T09:50:15.589Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T05:50:57.8121937-04:00` and still selected the newest valid-field route base with one small state-aware movement step under `CleanAfterField`. The printed macro remains the stale short state-aware route, so this run kept the explicit repaired post-load dismiss route.
- Non-duplicative Windows-only step: bisected between clean `left1261` and invalid/window-lost `left1265` with one `ls_left:1263` pulse. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Capture: `debug-captures/windows-lab/20260603-055129-cpu4-postload-dismiss-left1263-one-step-visualgate-windows-windows`.
- Visual gate summary: `FIELD_LIKE_PRESENT_WITH_LATER_INVALID_SCREENSHOTS`; first field-like according to the byte classifier was `screenshot-0309s-field-before-left1263-postload.png` at `309s` (`2.50 MB`); invalid screenshots after first field-like `1`, first `screenshot-0312s-field-after-left1263-postload.png` at `312s` (`76.97 KB`); required field-like by `335s` passed; class counts `field-like-large-png=1`, `wrong-window-or-other-small-png=6`, `cutscene-or-nonfield-large-png=2`; gate result `failed`.
- Screenshot verification: manual viewing overrode the byte classifier again. `screenshot-0309s-field-before-left1263-postload.png` and `screenshot-0312s-field-after-left1263-postload.png` both show the GitHub browser/repo page, not RPCS3 Path-to-Tenuto field gameplay. The harness skipped the late screenshot because the game window was not found and RPCS3 had exited, so `ls_left:1263` was not proven in field.
- Load target verification: `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=2`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=2`; lower-row cursor markers `0`; damaged-save text markers `2`.
- Fatal/log verification: no RPCS3 process remained after the run. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. Broad fatal/crash/access/assert scan excluding label/path/config noise also found no matches. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Host contention stayed clean across `3` snapshots with the external gate clean.
- SPU verifier rows: `0`; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic `2398`, MFC list `1426`, MFC wait `2713`, MFC wait exact-PC `132363`, reservation command `2713`, reservation command exact-PC `72608`, reservation verify `9149`, reservation lane-join `3`, reservation raw-lane `8`, total observed DMA `2791.72 MB`, RSX-local `0`, offload fit mix `spu-kernel-hle=1377`, `too-small=1021`.
- Classification: `route-tooling`, `failed-visual-gate`, `wrong-window-capture`, `window-lost-after-load`, `post-load-dismiss-left1263-not-field-proven`, `reservation-counters-invalid`, `not-speed`.
- Conclusion: no speed increase. `left1263` joins `left1265` as an invalid window-loss/capture result, not a movement proof. The only fresh clean repaired movement proof remains `left1261`; `left1272` stalls on `Now Loading...`, `left1287` reaches field then fatal-crashes after movement, and `left1317` stalls on the Load-complete menu. Next work should stabilize window capture or retry from the clean `left1261` side before any higher movement, Options/menu, first-battle, HLE/GPU, or 200% claim.

## 2026-06-03T06:39:55-04:00 - repaired left1262 reaches field then fatal-crashes after movement
- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T10:20:16.040Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T06:21:06.3548285-04:00` and still selected the newest valid-field route base with one small state-aware movement step under `CleanAfterField`. The printed macro remains the stale short state-aware route, so this run kept the explicit repaired post-load dismiss route.
- Non-duplicative Windows-only step: bisected the clean/invalid split with one `ls_left:1262` pulse, between clean `left1261` and invalid/window-lost `left1263`. No Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, or first-battle route was run.
- Capture: `debug-captures/windows-lab/20260603-062146-cpu4-postload-dismiss-left1262-one-step-visualgate-windows-windows`.
- Visual gate: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0309s-field-before-left1262-postload.png` at `309s` (`2.50 MB`); required field-like by `335s` passed; invalid after first field-like `0`; class counts `field-like-large-png=21`, `wrong-window-or-other-small-png=5`, `cutscene-or-nonfield-large-png=2`; gate result `passed-for-triage`.
- Screenshot verification: `screenshot-0309s-field-before-left1262-postload.png` shows clean Path-to-Tenuto field before movement. `screenshot-0312s-field-after-left1262-postload.png` and `screenshot-0327s-field-late-left1262-postload.png` are real field-like RPCS3 frames after `ls_left:1262`, but both show RPCS3's likely-crashed overlay and corrupted/warped field output, so this is not clean movement proof.
- Load target verification: `PATH_TO_TENUTO_PRESENT`; counts `path-to-tenuto=2`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=2`; lower-row cursor markers `0`; damaged-save text markers `2`.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan found a PPU access violation in `RPCS3.log` line `238793` and mirrored in `rpcs3.stderr.txt` line `1`: `RPCS3: PPU[0x100000c] Thread () [0x002aedd0]: VM: Access violation reading location 0x40 (unmapped memory)`. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `108`. Host contention stayed clean across `5` snapshots with the external gate clean.
- SPU verifier rows: `0`; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic `2334`, MFC list `1417`, MFC wait `2667`, MFC wait exact-PC `128809`, reservation command `2667`, reservation command exact-PC `71218`, reservation verify `8977`, reservation lane-join `3`, reservation raw-lane `8`, total observed DMA `2698.23 MB`, RSX-local `0`, offload fit mix `spu-kernel-hle=1318`, `too-small=1016`.
- Classification: `valid-field-triage`, `route-tooling`, `repaired-boundary-bisection`, `post-load-dismiss-left1262`, `field-entry-then-fatal`, `access-violation`, `reservation-counters-present-but-invalid-for-speed`, `not-speed`.
- Conclusion: no speed increase. The fresh repaired boundary is now tight: `left1261` is clean field movement, `left1262` reaches field then fatal-crashes after movement, `left1263` and `left1265` are invalid/window-lost, `left1272` stalls on `Now Loading...`, `left1287` reaches field then fatal-crashes after movement, and `left1317` stalls on the Load-complete menu. This is not Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence.

## 2026-06-03T07:08:51-04:00 - repaired no-movement control after left1262 fatal stays clean field

- Scope: Windows-only follow-up to the refiner fatal blocker after `20260603-062146-cpu4-postload-dismiss-left1262-one-step-visualgate-windows-windows`; executed one no-movement loader/control pass with `CleanAfterField`, not a speed pass.
- Capture: `debug-captures/windows-lab/20260603-065251-cpu4-postload-dismiss-nomove-after-left1262-fatal-visualgate-windows-windows`.
- Visual proof: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0309s-field-before-nomove-after-left1262-fatal.png` at 309s; late field screenshots `screenshot-0325s-field-late-nomove-after-left1262-fatal.png` and `screenshot-0340s.png`; invalid screenshots after first field-like: 0.
- Route/load proof: load target gate reported `PATH_TO_TENUTO_PRESENT`; counts were path-to-tenuto=2, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2; host contention gate stayed clean.
- Fatal/log proof: RPCS3 process stopped only by the 340s harness timeout; targeted scan found no `VM: Access violation`, `VK_ERROR`, device-lost, assertion, mismatch, or dynamic-fail lines; stdout/stderr were both 0 bytes; broad scan only showed known boot/export strings.
- SPU/counter evidence: SPU HLE verifier records 0; MFC dynamic records 2359; MFC list transfer records 1375; MFC wait records 2659; MFC wait exact-PC records 130729; reservation-loop command records 2659; reservation-loop command exact-PC records 71315; reservation-loop verify records 9015; RDCH join records 3; lane-join records 3; raw-lane records 8; total observed DMA 2809.91 MB; RSX-local traffic records 0; offload fit mix `spu-kernel-hle=1378`, `too-small=981`.
- Classification: `valid-field-triage`, `repaired-no-movement-control`, `post-left1262-fatal-control-clean`, `reservation-counters-present-but-route-only`, `not-speed`.
- Conclusion: no speed increase and no 200% proof. The repaired no-movement control did not reproduce the `left1262` fatal, so the movement step remains the likely crash boundary candidate; next Windows-only non-duplicative work should either compare a tiny movement boundary from this clean baseline or inspect the `0x002aedd0` / `0x40` fatal site before fast paths.
## 2026-06-03T07:38:39-04:00 - post-control left1261 reproof stalls on lower-slot Load-complete screen

- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T11:20:17.164Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T07:20:41.8258101-04:00` and selected the newest valid-field run as route base with one small state-aware movement step under `CleanAfterField`. Because the fatal-log blocker still applies, this step tried to reprove the last clean boundary (`ls_left:1261`) from the post-`left1262` clean no-movement control rather than pushing higher movement.
- Non-duplicative Windows-only step: `cpu4-postcontrol-reproof-left1261-after-left1262-fatal-visualgate-windows`; no Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, first-battle route, or 200% speed pass was run.
- Capture: `debug-captures/windows-lab/20260603-072108-cpu4-postcontrol-reproof-left1261-after-left1262-fatal-visualgate-windows-windows`.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot none; required field-like by 335s failed; invalid screenshots after first field-like 0; class counts `cutscene-or-nonfield-large-png=2`, `wrong-window-or-other-small-png=21`; gate result `failed`.
- Screenshot verification: `screenshot-0309s-field-before-left1261-after-left1262-control.png`, `screenshot-0311s-field-after-left1261-after-left1262-control.png`, and `screenshot-0327s-field-late-left1261-after-left1262-control.png` all show the Load menu on `Save File 05 / Path to Tenuto` with `Save file has been damaged.` and `Load complete.` overlays, not Path-to-Tenuto field gameplay. The `ls_left:1261` pulse therefore happened while stuck on the Load-complete screen and does not reprove movement.
- Load target verification: load target gate reported `PATH_TO_TENUTO_PRESENT`, but the rows show lower-row selection evidence: counts path-to-tenuto=2, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2; lower-row cursor markers 2; damaged-save text markers 0. This is a route-selection failure despite the loose Path-to-Tenuto gate pass.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. `rpcs3.stdout.txt` length 0; `rpcs3.stderr.txt` length 0. Broad fatal/error scan only found known boot/module/probe strings, not a new crash. Host contention stayed clean across 5 snapshots with the external gate clean.
- SPU verifier rows: 0; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic 2577, MFC list 1661, MFC wait 2971, MFC wait exact-PC 139569, reservation command 2971, reservation command exact-PC 78699, reservation verify 9836, RDCH join 3, lane-join 3, raw-lane 8, total observed DMA 2523.40 MB, RSX-local 0, offload fit mix `spu-kernel-hle=1292`, `too-small=1285`.
- Classification: `route-tooling`, `failed-visual-gate`, `load-menu-stall`, `lower-slot-loadcomplete`, `post-control-left1261-not-reproved`, `reservation-counters-invalid`, `not-speed`.
- Conclusion: no speed increase. The post-control `left1261` reproof failed before field because the route drifted to the lower Save File 05 load-complete/damaged-save screen; the latest clean movement boundary remains the earlier `left1261` proof, while the latest clean route base is the no-movement control. Next Windows-only work should harden the load target gate to require top-row Path-to-Tenuto/lower-row cursor markers 0 before any movement, Options, battle, HLE/GPU, or speed claim.
## 2026-06-03T08:07:22-04:00 - up-repair top-row left1261 reaches field then loses RPCS3 window

- Automation heartbeat: ps3-200-windows-speed-loop at 2026-06-03T11:50:17.387Z.
- Refiner: 	ools/ps3_harness_refiner.ps1 -MaxRuns 8 generated 2026-06-03T07:50:47.0140612-04:00 and selected the newest valid-field route base with one small state-aware movement step under CleanAfterField. Because the latest route blocker was lower-slot Load-complete drift, this run added an explicit Load-list up-repair before the same known-clean ls_left:1261 pulse.
- Non-duplicative Windows-only step: cpu4-uprepair-toprow-left1261-after-loadcomplete-stall-visualgate-windows; no Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, first-battle route, or 200% speed pass was run.
- Capture: debug-captures/windows-lab/20260603-075126-cpu4-uprepair-toprow-left1261-after-loadcomplete-stall-visualgate-windows-windows.
- Visual gate: FIELD_LIKE_PRESENT_WITH_LATER_INVALID_SCREENSHOTS; first field-like screenshot screenshot-0313s-field-before-uprepair-left1261.png at 313s (2.49 MB); invalid screenshots after first field-like: 1, first screenshot-0315s-field-after-uprepair-left1261.png at 315s (76.84 KB); required field-like by 335s passed; class counts ield-like-large-png=1, wrong-window-or-other-small-png=7; gate result ailed.
- Screenshot verification: screenshot-0133s-load-list-after-uprepair-left1261.png shows the Load list back on the top Path-to-Tenuto rows (Save File 01 / Save File 02) rather than the prior lower Save File 05 damaged-save state. screenshot-0313s-field-before-uprepair-left1261.png shows clean Path-to-Tenuto field before movement. screenshot-0315s-field-after-uprepair-left1261.png shows the GitHub/browser page, not RPCS3 gameplay, so the ls_left:1261 pulse is not re-proven.
- Load target verification: load target gate reported PATH_TO_TENUTO_PRESENT; counts path-to-tenuto=3, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2; lower-row cursor markers 0; damaged-save text markers 3. Manual view confirms top-row Path-to-Tenuto was restored, but the classifier still treats the lower File-does-not-exist row as damaged-save-like text.
- Fatal/log verification: no RPCS3 process remained after the run. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. pcs3.stdout.txt length 0; pcs3.stderr.txt length 0. Broad fatal/error scan only found known boot/module/probe strings, not a new crash. Host contention stayed clean across 3 snapshots with the external gate clean.
- SPU verifier rows: 0; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic 2394, MFC list 1488, MFC wait 2666, MFC wait exact-PC 129786, reservation command 2666, reservation command exact-PC 71538, reservation verify 8984, RDCH join 5, lane-join 3, raw-lane 8, total observed DMA 2766.92 MB, RSX-local 0, offload fit mix spu-kernel-hle=1301, 	oo-small=1093.
- Classification: oute-tooling, ailed-window-lost-after-field, uprepair-toprow-restored, post-left1261-window-loss, eservation-counters-invalid, 
ot-speed.
- Conclusion: no speed increase. The up-repair fixed the lower-slot Load-complete drift enough to reach clean field, but the run lost the RPCS3 window/process immediately after ls_left:1261; this is not movement proof, Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence. Next work should stabilize post-field capture/process retention or add a no-movement post-uprepair control before another movement boundary attempt.

## 2026-06-03T08:40:30-04:00 - up-repair no-movement retention control stays clean field

- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T12:20:17.990Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T08:20:48.9000849-04:00` and selected the newest valid-field route base with one small state-aware movement step under `CleanAfterField`. Because the latest movement attempt lost RPCS3 immediately after `ls_left:1261`, this run used the same top-row up-repaired route with no movement to isolate route/process retention before another movement boundary attempt.
- Non-duplicative Windows-only step: `cpu4-uprepair-toprow-nomove-retention-after-left1261-windowloss-visualgate-windows`; no Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, first-battle route, movement, or 200% speed pass was run.
- Capture: `debug-captures/windows-lab/20260603-082144-cpu4-uprepair-toprow-nomove-retention-after-left1261-windowloss-visualgate-windows-windows`.
- Visual gate: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0313s-field-before-uprepair-nomove-retention.png` at 313s (`2.49 MB`); invalid screenshots after first field-like 0; required field-like by 335s passed; class counts `field-like-large-png=23`, `wrong-window-or-other-small-png=6`; gate result `passed-for-triage`.
- Screenshot verification: `screenshot-0313s-field-before-uprepair-nomove-retention.png`, `screenshot-0328s-field-late1-uprepair-nomove-retention.png`, and `screenshot-0344s-field-late2-uprepair-nomove-retention.png` all show real Path-to-Tenuto field with Polka stationary and no crash overlay, wrong-window capture, or loading/menu stall. Later automatic frames through `screenshot-0358s-01.png` also stayed field-like before the harness timeout stopped RPCS3.
- Load target verification: load target gate reported `PATH_TO_TENUTO_PRESENT`; counts path-to-tenuto=3, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=0, unknown=2; lower-row cursor markers 0; damaged-save text markers 3. Manual view and path rows indicate top Path-to-Tenuto was restored; damaged-save markers remain a classifier artifact from lower visible rows.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan for access violation, VK/device lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, and dynamic fail found no matches. `rpcs3.stdout.txt` length 0; `rpcs3.stderr.txt` length 0. Broad fatal/error scan only found known boot/module/probe strings, not a new crash. Host contention stayed clean across 5 snapshots with the external gate clean.
- SPU verifier rows: 0; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic 2757, MFC list 1521, MFC wait 3089, MFC wait exact-PC 154268, reservation command 3089, reservation command exact-PC 83165, reservation verify 10557, RDCH join 5, lane-join 3, raw-lane 8, total observed DMA 3554.15 MB, RSX-local 0, offload fit mix `spu-kernel-hle=1730`, `too-small=1027`.
- Classification: `valid-field-triage`, `route-tooling`, `uprepair-toprow-restored`, `no-movement-retention-control-clean`, `reservation-counters-present-but-route-only`, `not-speed`.
- Conclusion: no speed increase. The up-repaired route itself is stable when no movement is issued, so the previous `left1261` failure is now narrowed to the post-field movement/window-retention boundary rather than the top-row load repair. This is not movement proof, Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence. Next Windows-only work should retry one smaller post-field movement pulse from this up-repaired clean control or inspect why PadApi movement coincides with window/process loss.

## 2026-06-03T08:57:32-04:00 - up-repair left630 attempt aborts on early black-overlay fatal before load target

- Automation heartbeat: `ps3-200-windows-speed-loop` at `2026-06-03T12:50:18.251Z`.
- Refiner: `tools/ps3_harness_refiner.ps1 -MaxRuns 8` generated `2026-06-03T08:50:55.1188413-04:00` and selected the newest valid-field route base with one small state-aware movement step under `CleanAfterField`. Because the previous up-repaired no-movement route stayed clean through field, this run attempted a smaller non-duplicative `ls_left:630` pulse from the same up-repaired route.
- Non-duplicative Windows-only step: `cpu4-uprepair-toprow-left630-after-nomove-retention-visualgate-windows`; no Android, ADB, Thor, HLE fast path, RSX/GPU fast path, Options route, first-battle route, or 200% speed pass was run.
- Capture: `debug-captures/windows-lab/20260603-085126-cpu4-uprepair-toprow-left630-after-nomove-retention-visualgate-windows-windows`.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot none; required field-like by 335s failed; invalid screenshots after first field-like 0; class counts `black-overlay-small-png=9`; gate result `failed`.
- Screenshot verification: `screenshot-0130s-load-list-initial-uprepair-left630.png`, `screenshot-0133s-load-target-gate.png`, and `screenshot-0161s-load-target-gate-7.png` all show a black RPCS3 crash overlay saying the PS3 application likely crashed. The macro aborted at the load-target gate before pressing slot `Cross`, so `ls_left:630` was never issued and no movement was tested.
- Load target verification: load target gate timed out at 161s with `BLACK_OVERLAY_OR_CRASH_PRESENT`; counts path-to-tenuto=0, debug-save-prologue=0, empty-load-slot=0, black-overlay-or-crash=9, unknown=2; lower-row cursor markers 0; damaged-save text markers 0. `load-target-gate-failed.txt` records the timeout and black-overlay status.
- Fatal/log verification: no RPCS3 process remained after the aborted run. Targeted scan found `RPCS3.log` line 61941 and `rpcs3.stderr.txt` line 1: `RPCS3: PPU[0x1000000] Thread (main_thread) [0x0007dccc]: VM: Access violation reading location 0x4 (unmapped memory)`. `rpcs3.stdout.txt` length 0; `rpcs3.stderr.txt` length 118. Host contention stayed clean across 3 snapshots with the external gate clean.
- SPU verifier rows: 0; no verifier mismatch/dynamic-fail rows found.
- Counter verification: MFC dynamic 505, MFC list 356, MFC wait 530, MFC wait exact-PC 26158, reservation command 530, reservation command exact-PC 14433, reservation verify 1801, RDCH join 3, lane-join 3, raw-lane 8, total observed DMA 503.70 MB, RSX-local 0, offload fit mix `too-small=267`, `spu-kernel-hle=238`.
- Classification: `failed-fatal-log`, `failed-visual-gate`, `black-overlay-before-load-target`, `pre-slot-route-fatal`, `left630-not-tested`, `reservation-counters-invalid`, `not-speed`.
- Conclusion: no speed increase. The run failed before field and before the intended smaller movement pulse, with a new early main-thread `0x0007dccc` / `0x4` access violation. The latest valid route proof remains the up-repaired no-movement retention control; next Windows-only work should treat this early fatal as newer evidence and either re-prove the no-movement route or inspect the `0x0007dccc` fatal site before additional movement/Options/battle/HLE/GPU attempts.

## 2026-06-03T09:34:26.1332090-04:00 - cpu4 pollgated top-row left200 after left630 early fatal
- Automation: `ps3-200-windows-speed-loop` heartbeat `2026-06-03T13:20:19.007Z`.
- Refiner decision followed: do not run speed/HLE/RSX experiments after the prior `BLACK_OVERLAY_OR_CRASH_PRESENT` / early-fatal route; re-prove the polling load-target lane and require `PATH_TO_TENUTO_PRESENT` first.
- Windows-only command: `tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-pollgated-toprow-left200-after-left630-earlyfatal-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 215 ...`.
- Run directory: `debug-captures/windows-lab/20260603-092224-cpu4-pollgated-toprow-left200-after-left630-earlyfatal-visualgate-windows-windows`.
- Load-target verification: `PATH_TO_TENUTO_PRESENT` passed on attempt 1; both `screenshot-0063s-load-list-uprepair-after-left630-earlyfatal.png` and `screenshot-0063s-load-target-gate.png` classified as `path-to-tenuto`, with no black-overlay/crash or unknown load-target screenshots.
- Screenshot verification: `screenshot-0120s-field-entry-after-left630-earlyfatal.png` is a real field-like frame at 120s, but `screenshot-0158s-field-before-left200-after-left630-earlyfatal.png`, `screenshot-0162s-field-after-left200-after-left630-earlyfatal.png`, and later frames show the Save/create-new-file prompt rather than field movement.
- Visual gate: `FIELD_LIKE_PRESENT_WITH_LATER_INVALID_SCREENSHOTS`; failed because invalid non-field screenshots appeared after the first field-like screenshot. This is route-tooling progress only, not a valid field-movement proof.
- Fatal/log verification: targeted fatal scan found no `VM: Access violation`, `VK_ERROR`, device-lost, assertion, mismatch, or dynamic-fail lines; stdout/stderr were both 0 bytes; RPCS3 was stopped after the run.
- SPU/reservation counters: `SPU HLE verifier records=0`; reservation-loop command records `2265`; exact-PC records `56057`; verify records `7144`; RDCH join records `5`; lane-join records `3`; raw-lane records `8`; PUTLLC16 analyzer records `43`; no runtime PUTLLC16 pair fast path was enabled.
- Classification: `valid-load-target`, `field-entry-recovered`, `invalid-post-field-save-menu`, `left200-not-field-movement`, `reservation-counters-present`, `not-speed`.
- Speed accounting: confirmed speed increase remains `0%`; there is still no field + Options/menu + first-battle 200% proof.

## 2026-06-03T10:02:26.1343865-04:00 - loader-control left200x2 reproof after save-menu invalidation
- Automation: `ps3-200-windows-speed-loop` heartbeat `2026-06-03T13:50:20.915Z`.
- Refiner decision followed: back off from the latest non-field/save-menu route and re-prove the last clean loader-control `left200x2` route under `CleanAfterField` before diagonal, HLE, GPU, Options, battle, or speed work.
- Windows-only command: `tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-loader-control-left200x2-reproof-after-save-menu-invalid-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 160 ...`.
- Run directory: `debug-captures/windows-lab/20260603-095109-cpu4-loader-control-left200x2-reproof-after-save-menu-invalid-visualgate-windows-windows`.
- Visual gate: `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0117s.png` at 117s (`2.49 MB`); required field-like by 160s passed; invalid screenshots after first field-like `0`; class counts `field-like-large-png=16`; gate result `passed-for-triage`.
- Screenshot verification: `screenshot-0117s.png` shows clean Path-to-Tenuto field at the save point; `screenshot-0136s.png` and `screenshot-0138s.png` show the two `ls_left:200` pulses still in valid field gameplay with Polka moved left of the save point; `screenshot-0210s.png` remains valid field after the late hold. No Save/create-file prompt, blue/starry miss, loading stall, black overlay, or wrong-window capture appeared.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan found no `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail lines. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Broad scan only found expected reservation/probe rows.
- Host verification: `ExternalFail` gate stayed clean across `6` snapshots, with no competing emulator or heavy host load detected.
- SPU/reservation counters: `SPU HLE verifier records=0`; reservation-loop command records `1814`; exact-PC records `49677`; verify records `6365`; RDCH join records `3`; lane-join records `3`; raw-lane records `8`; PUTLLC16 analyzer records `43`; PUTLLC16 runtime and pair-verify records `0`; total observed DMA `2753.95 MB`; offload fit mix `spu-kernel-hle=1273`, `too-small=419`; direct RSX-local scout traffic `0`.
- Classification: `valid-field-triage`, `route-tooling`, `loader-control-left200x2-reproved`, `movement-baseline-clean`, `reservation-counters-present-but-route-only`, `not-speed`.
- Speed accounting: confirmed speed increase remains `0%`; this restores a clean small-movement field baseline, but it is not Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence.

## 2026-06-03T10:32:17.0823794-04:00 - loader-control left200x2 diag200 reproof route miss after save-menu invalidation
- Automation: `ps3-200-windows-speed-loop` heartbeat `2026-06-03T14:20:23.132Z`.
- Refiner decision followed: extend the newest valid loader-control `left200x2` field route by exactly one tiny diagonal micro-pulse under `CleanAfterField`; keep HLE/GPU dry-runs blocked.
- Windows-only command: `tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-loader-control-left200x2-diag200-reproof-after-save-menu-invalid-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 160 ...`.
- Run directory: `debug-captures/windows-lab/20260603-102120-cpu4-loader-control-left200x2-diag200-reproof-after-save-menu-invalid-visualgate-windows-windows`.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot none; required field-like by 160s failed; class counts `wrong-window-or-other-small-png=18`; gate result `failed`.
- Screenshot verification: `screenshot-0117s.png`, `screenshot-0141s.png`, and `screenshot-0220s.png` all show the Load screen stuck on `Save File 01 / Debug Save / Prologue` with `Save file has been damaged`, not Path-to-Tenuto field. The two `ls_left:200` pulses and the `combo:ls_left+ls_down:200` diagonal pulse therefore were not tested as field gameplay movement.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan found no `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail lines. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Broad scan only found expected reservation/probe rows.
- Host verification: `ExternalFail` gate stayed clean across `6` snapshots, with no competing emulator or heavy host load detected.
- SPU/reservation counters: `SPU HLE verifier records=0`; reservation-loop command records `1882`; exact-PC records `50055`; verify records `6287`; RDCH join records `3`; lane-join records `3`; raw-lane records `8`; PUTLLC16 analyzer records `43`; PUTLLC16 runtime and pair-verify records `0`; total observed DMA `1631.71 MB`; offload fit mix `spu-kernel-hle=856`, `too-small=795`; direct RSX-local scout traffic `0`.
- Classification: `failed`, `failed-visual-gate`, `debug-save-prologue-route-miss`, `diag200-not-field-tested`, `reservation-counters-invalid`, `not-speed`.
- Speed accounting: confirmed speed increase remains `0%`; the previous clean `left200x2` field baseline remains the latest valid small-movement proof, and this failed diagonal route is not Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence.

## 2026-06-03T10:58:10.7242110-04:00 - state-aware one-step after diag route miss stalls on top-row load list
- Automation: `ps3-200-windows-speed-loop` heartbeat `2026-06-03T14:50:23.664Z`.
- Refiner decision followed: use the newest valid-field run as the route base and add only one small state-aware movement step under `CleanAfterField`; keep HLE/GPU dry-runs blocked.
- Windows-only command: `tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-stateaware-one-step-after-diag-debugsave-route-miss-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 160`.
- Run directory: `debug-captures/windows-lab/20260603-105115-cpu4-stateaware-one-step-after-diag-debugsave-route-miss-visualgate-windows-windows`.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot none; required field-like by 160s failed; class counts `wrong-window-or-other-small-png=3`; gate result `failed`.
- Screenshot verification: `screenshot-0117s.png` and `screenshot-0133s.png` show the Load screen on `Save File 01 / Path to Tenuto / South Section / Ch. 1 Raindrops` with lower rows reading `File does not exist`. This recovered the Path-to-Tenuto top-row display versus the prior damaged Debug Save miss, but it never reached field within the default 120s run window. The state-aware movement step was not tested in gameplay.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan found no `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail lines. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Broad scan only found expected reservation/probe rows.
- Host verification: `ExternalFail` gate stayed clean across `4` snapshots, with no competing emulator or heavy host load detected.
- SPU/reservation counters: `SPU HLE verifier records=0`; reservation-loop command records `1070`; exact-PC records `28398`; verify records `3541`; RDCH join records `3`; lane-join records `3`; raw-lane records `8`; PUTLLC16 analyzer records `43`; PUTLLC16 runtime and pair-verify records `0`; total observed DMA `918.03 MB`; offload fit mix `too-small=482`, `spu-kernel-hle=459`; direct RSX-local scout traffic `0`.
- Classification: `failed`, `failed-visual-gate`, `path-to-tenuto-load-list-restored`, `load-list-stall-before-field`, `stateaware-step-not-tested`, `reservation-counters-invalid`, `not-speed`.
- Speed accounting: confirmed speed increase remains `0%`; the latest valid small-movement field proof remains `20260603-095109-cpu4-loader-control-left200x2-reproof-after-save-menu-invalid-visualgate-windows-windows`, and this run is not movement proof, Options/menu proof, first-battle proof, 200% proof, HLE/GPU credit, or counter-derived speed evidence.

## 2026-06-03T11:30:34.2535412-04:00 - damaged-confirm left200 after load-list stall goes black overlay
- Automation: `ps3-200-windows-speed-loop` heartbeat `2026-06-03T15:20:24.159Z`.
- Refiner decision followed: latest state-aware one-step repair reached the load-confirm/list state and never accepted field; do not rerun the default macro, instead add an explicit post-prompt `Cross`, delay screenshots, then test one `ls_left:200` pulse under `CleanAfterField`.
- Windows-only command: `tools/eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-stateaware-damaged-confirm-left200-after-loadlist-stall-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 160 ...`.
- Run directory: `debug-captures/windows-lab/20260603-112127-cpu4-stateaware-damaged-confirm-left200-after-loadlist-stall-visualgate-windows-windows`.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot none; required field-like by 160s failed; class counts `black-overlay-small-png=8`; gate result `failed`.
- Screenshot verification: `screenshot-0132s.png`, `screenshot-0135s.png`, and `screenshot-0175s.png` all show a black gameplay area with the RPCS3 FPS/performance overlay still visible. No Path-to-Tenuto field, load list, Options/menu, battle, or movement frame appeared; the intended `ls_left:200` pulse was not proven as field gameplay.
- Fatal/log verification: no RPCS3 process remained after the harness timeout stop. Targeted scan found no `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail lines. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` length `0`. Broad scan only found expected reservation/probe rows, not a new fatal.
- Host verification: `ExternalFail` gate stayed clean across `5` snapshots, with no competing emulator or heavy host load detected.
...
## 2026-06-03T12:24:49.3673940-04:00 - Hardware acceleration RAG: CPU/SPU feed first, GPU offload only after bulk-contract proof

- Automation: `ps3-200-windows-speed-loop` heartbeat `2026-06-03T16:20:25.075Z`.
- Refiner context: newest valid run is `20260603-115211-cpu4-loader-control-left200x2-reconfirm-after-blackoverlay-visualgate-windows-windows` with clean field triage, but recent summaries still report `0 B` RSX-local and the refiner's direction says broad SPU-to-Vulkan compute remains parked until a new scout proves RSX-consumed data.
- Local evidence anchor: latest clean route has reservation-loop command/exact-PC/verify rows, but `New promoted CPU/SPU -> GPU replacement` is still `0 B` / `0.000%`; `Direct RSX-local scout traffic` is still `0 B`. Banked bodyfast evidence is CPU-pressure only (`42.60%` to `37.10%` process CPU on capped BattleRoute), not FPS, GPU migration, or 200% proof.
- Source, PS3 architecture: IBM Research describes Cell/B.E. as one conventional PowerPC core plus eight SPEs, each with a small fast Local Store, and says accesses outside LS require explicitly programmed DMA. Source: https://research.ibm.com/publications/techniques-for-code-and-data-management-in-the-local-stores-of-the-cell-processor
- Source, PS3 hardware split: Sony's PS3 launch technical outline describes Cell as the processor and RSX as the NVIDIA/SCEI graphics processor, with XDR memory also separate in the platform. Source: https://sonyinteractive.com/en/press-releases/2005/sony-computer-entertainment-inc-to-launch-its-next-generation-computer-entertainment-system-playstation-3-in-spring-2006/
- Source, x86 SIMD: Intel documents AVX2 as extending AVX with 256-bit integer operations, FMA, and gather. Source: https://edc.intel.com/content/www/us/en/design/products/platforms/details/raptor-lake-s/13th-generation-core-processors-datasheet-volume-1-of-2/002/002/intel-advanced-vector-extensions-2-intel-avx2/
- Source, ARM/NEON: Arm's official Neon Intrinsics Reference covers Advanced SIMD intrinsics for AArch64/AArch32 and notes Dot Product intrinsics for Armv8.2-A and newer. Source: https://arm-software.github.io/acle/neon_intrinsics/advsimd.html
- Source, Thor-class SoC: Qualcomm's Snapdragon 8 Gen 2 product page/brief reports a Kryo CPU up to 3.36 GHz and an Adreno GPU marketed for faster graphics rendering; this supports a Thor strategy that uses CPU SIMD and GPU/Vulkan selectively rather than assuming unlimited GPU compute bandwidth. Sources: https://www.qualcomm.com/smartphones/products/8-series/snapdragon-8-gen-2-mobile-platform and https://www.qualcomm.com/content/dam/qcomm-martech/dm-assets/documents/Snapdragon-8-Gen-2-Product-Brief.pdf
- Source, Vulkan compute constraint: Khronos documents that compute pipelines contain a single compute shader stage, and Vulkan synchronization/barriers must be explicit. This makes GPU offload plausible for stable bulk jobs, but expensive or risky for tiny SPURS/reservation control loops that need immediate guest-visible synchronization. Sources: https://docs.vulkan.org/spec/latest/chapters/pipelines.html and https://docs.vulkan.org/spec/latest/chapters/shaders.html
- Source, RPCS3 setting anchor: RPCS3's configuration wiki lists SPU decoder backends including static/dynamic interpreters and ASMJIT/LLVM recompilers. This supports focusing on SPU codegen/contract specialization before treating underused GPU as the root cause. Source: https://wiki.rpcs3.net/index.php?title=Help%3AConfigurations
- RAG conclusion: low GPU usage is probably a symptom of CPU/SPU feed stalls, not automatically proof that RSX/Vulkan work is waiting to be offloaded. The next acceleration lane should be `contract -> verifier -> SIMD/codegen/HLE fast path`, with GPU compute only for a stable bulk LS/DMA job that has repeated input/output shape, low readback pressure, and either RSX-consumed destination evidence or a verifier-clean byte/hash comparison.
- Candidate ranking for next work:
  - Priority 1: `0x25cc / 0x9e4000` broader runtime-family contract counters and reject buckets, because local atlas sizes are GB-scale and prior exact skip was only `0.10%` of refreshed 0x25cc bytes.
  - Priority 2: reservation-loop exact-PC/RDCH/lane joins into verifier-safe reduced-loop or branchstate lowering, because latest clean runs repeatedly show thousands of reservation rows and zero RSX-local bytes.
  - Priority 3: host SIMD mapping for the same contracts: x86 AVX2/AVX-512 where available on Windows, and AArch64 NEON/DOTPROD-friendly lowering for Thor. Do not assume SVE/SVE2 on Snapdragon 8 Gen 2 without feature reports.
...
## 2026-06-03 15:20 ET - left200x2 clean field boundary restored after fatal route crash (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane.
- Refiner: `2026-06-03T15:20:50.4585872-04:00`; decision was to avoid extending the latest fatal state-aware route and re-prove the newest clean loader-control `left200x2` boundary with `CleanAfterField` before adding another pulse or HLE/GPU mode.
- Command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-loader-control-left200x2-reconfirm-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 160 -InputMacro "wait:45000;down:20;wait:500;cross:80;wait:12000;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:500;cross:80;wait:3000;up:80;wait:500;cross:80;wait:32000;cross:120;wait:18000;shot:100;wait:15000;shot:100;wait:1000;ls_left:200;wait:1000;shot:100;wait:1000;ls_left:200;wait:1000;shot:100;wait:10000;shot:100" -MaxSeconds 215 -ScreenshotEverySeconds 10 -ScreenshotStartSeconds 110 -ScreenshotMaxCount 11`.
- Run directory: `debug-captures/windows-lab/20260603-152104-cpu4-loader-control-left200x2-reconfirm-visualgate-windows-windows`.
- Harness note: the wrapper reached the 215s run window and stopped RPCS3, then the shell command timed out during postrun analysis. Manual verification was run on the completed artifact.
- Visual gate: manual `check_eternal_sonata_windows_visual_gate.ps1 -RunDir ... -RequireFieldLike -RequireNoInvalidAfterFirstField -RequireFieldAtOrBeforeSeconds 160` passed. Status `FIELD_LIKE_PRESENT`; first field-like screenshot `screenshot-0117s.png` at `117s`; 16/16 screenshots field-like through `screenshot-0210s.png`; invalid screenshots after first field-like `0`; required field by `160s` passed.
- Manual screenshot check: `screenshot-0117s.png` shows clean Path-to-Tenuto field at the save point; `screenshot-0210s.png` remains clean after the two left-200 pulses.
- Fatal/log verification: targeted scan found no real crash/access/device-lost/assertion/verification failure. The only fatal-pattern hits in `RPCS3.log` were startup/config/VFS path lines; stdout/stderr did not report a crash.
- Host verification: clean across 7 snapshots (`prelaunch`, `postlaunch`, `sample-0149s`, `sample-0152s`, `sample-0180s`, `sample-0210s`, `postrun`); no competing emulator or heavy host load.
- Counter verification: normal GPU/counter summarizer did not finish before timeout, so direct log counts were used. SPU HLE verifier rows `0`; reservation-loop verify rows `6120`; RSX auditor rows `0`; direct `RSX-local traffic records` strings `0`; direct `New promoted CPU/SPU` strings `0`.
- GPU/offload evidence: no promoted CPU/SPU-to-GPU replacement evidence and no RSX-local scout evidence in direct log scan. This restores route validity only; it does not advance Vulkan compute or GPU offload.
...
## 2026-06-05 12:24 ET - RPCS3 0.0.41 Verify25cc prompt-dismiss attempt remains load-list blocked (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T16:23:48.537Z`.
- Refiner: `2026-06-05T12:24:25.4986071-04:00`; decision was the newest valid-field route plus one small state-aware movement step. Because the latest `Verify25ccShadow` run was parser-clean but visually blocked on the load prompt, this heartbeat used one non-duplicative verify-only route repair instead of repeating a route-only proof.
- Core identity: Windows RPCS3 executable was the freshly integrated 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
- Command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-rpcs3-0041-verify25cc-promptdismiss-field-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -EternalSonataSpuHleVerify Verify25ccShadow -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 160 -InputMacro "wait:45000;down:20;wait:500;cross:80;wait:12000;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:500;cross:80;wait:3000;up:80;wait:500;cross:80;wait:32000;cross:120;wait:18000;shot:100;wait:15000;shot:100;wait:10000;shot:100" -MaxSeconds 205 -ScreenshotEverySeconds 10 -ScreenshotStartSeconds 110 -ScreenshotMaxCount 10`.
- Run directory: `debug-captures/windows-lab/20260605-122452-cpu4-rpcs3-0041-verify25cc-promptdismiss-field-visualgate-windows-windows`.
- Harness note: RPCS3 stopped at the requested `205s`, then the wrapper hit the known postrun-analysis stall and the shell timed out. Manual verification was run against the completed artifact.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot `none`; required field-like by `160s` failed; `13/13` screenshots classified `wrong-window-or-other-small-png`.
- Screenshot verification: `screenshot-0117s.png` shows the Load screen on `Save File 01 / Path to Tenuto / South Section / Ch. 1 Raindrops` with lower rows saying `File does not exist.` It is not Path-to-Tenuto field gameplay, not Options/menu, and not battle. Later screenshots through `screenshot-0200s.png` stayed in the same load-list class.
- Fatal/log verification: targeted scan found no real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail line. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` contained only `MidiInWinMM::initialize: no MIDI input devices currently available.`
- Host verification: host contention stayed clean across `6` snapshots. GPU-engine samples reached about `10.7%` to `11.4%`, but this is load-list telemetry and does not prove useful GPU offload.
- SPU verifier/parser result: strict parser rows `477`; accepted rows `477`; rejected rows `0`; total contract hits `1010`; total contract bytes `16,547,840`; output mismatch `0`; descriptor overflow `0`; strict parser gate passed; `promotion_ready=false` because external visual/fatal gates are not satisfied.
- Counter verification: CSV summarizer reported command-correlation data missing because postrun analysis did not finish. Direct log counts showed `SPU HLE verifier` rows `1303`, `Eternal Sonata SPU contract verifier` rows `477`, `Eternal Sonata reservation loop` strings `44298`, `PUTLLC16` strings `8`, `RDCH` strings `62`, `RSX-local traffic` strings `0`, and `New promoted CPU/SPU` strings `0`.
- Classification: `verify-logrow-progress`, `strict-parser-pass`, `failed-visual-gate`, `load-list-stall`, `rpcs3-0.0.41-core-smoke`, `stackable-cpu-pressure-plan`, `gpu-offload-parked`, `not-speed`.
- Speed status: confirmed speed increase remains `0%`. The observed `40-50 FPS` title samples are load-list samples, not field + Options/menu + first-battle A/B proof.
- Next hypothesis: the blocker changed from load-prompt dismissal to load-list acceptance/transition. Add a target-row-to-field guard or longer post-`Cross` load transition proof before another `Verify25ccShadow` capture. Do not enable bodyfast/codegen-fast/Vulkan/GPU modes from this evidence.

## 2026-06-05 12:54 ET - RPCS3 0.0.41 Verify25cc cross-guard selects Debug Save row (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T16:53:49.281Z`.
- Refiner: `2026-06-05T12:54:16.0897833-04:00`; decision stayed generic state-aware field repair. This heartbeat instead took the narrower non-duplicative route-control step from the newest failure: a repeated `Cross` guard on the 0.0.41 `Verify25ccShadow` path to test whether top-row load-list acceptance could transition to field.
- Core identity: Windows RPCS3 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
- Command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-rpcs3-0041-verify25cc-loadlist-crossguard-field-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -EternalSonataSpuHleVerify Verify25ccShadow -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 170 -InputMacro "wait:45000;down:20;wait:500;cross:80;wait:12000;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:500;cross:80;wait:3000;up:80;wait:500;cross:80;wait:25000;cross:120;wait:5000;cross:120;wait:5000;cross:120;wait:25000;shot:100;wait:15000;shot:100;wait:20000;shot:100" -MaxSeconds 220 -ScreenshotEverySeconds 10 -ScreenshotStartSeconds 110 -ScreenshotMaxCount 11`.
- Run directory: `debug-captures/windows-lab/20260605-125437-cpu4-rpcs3-0041-verify25cc-loadlist-crossguard-field-visualgate-windows-windows`.
- Harness note: RPCS3 stopped at the requested `220s`, then the wrapper hit the known postrun-analysis stall and the shell timed out. Manual verification was run against the completed artifact.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot `none`; required field-like by `170s` failed; `14` screenshots captured and no field-like frame was found.
- Screenshot verification: `screenshot-0128s.png` shows the Load screen on `Save File 01 / Debug Save / Prologue / Lv.00 / Time 00:00:00` with lower rows saying `File does not exist.` The repeated cross guard did not load Path-to-Tenuto field; it moved/accepted the wrong save-list state. Later title FPS samples up to `56.00 FPS` are therefore load-list/menu samples only.
- Fatal/log verification: targeted scan found no real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail line. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` contained only `MidiInWinMM::initialize: no MIDI input devices currently available.`
- Host verification: host contention stayed clean. Sample GPU-engine sums were about `10.2%` to `12.8%`, but this is invalid load-list telemetry and not proof of useful GPU offload.
- SPU verifier/parser result: strict parser rows `549`; accepted rows `549`; rejected rows `0`; total contract hits `1182`; total contract bytes `19,365,888`; output mismatch `0`; descriptor overflow `0`; `promotion_ready=false`.
- Counter verification: direct log counts showed `SPU HLE verifier` rows `1334`, `Eternal Sonata SPU contract verifier` rows `549`, `Eternal Sonata reservation loop` strings `46167`, `PUTLLC16` strings `8`, `RDCH` strings `62`, `RSX-local traffic` strings `0`, and `New promoted CPU/SPU` strings `0`.
- Classification: `verify-logrow-progress`, `strict-parser-pass`, `failed-visual-gate`, `debug-save-prologue-route-miss`, `crossguard-wrong-row`, `rpcs3-0.0.41-core-smoke`, `stackable-cpu-pressure-plan`, `gpu-offload-parked`, `not-speed`.
- Speed status: confirmed speed increase remains `0%`. This is not field proof, Options/menu proof, first-battle proof, 200% proof, or GPU migration credit.
- Next hypothesis: route selection must be repaired before more contract captures. The right next step is a load-list target-row guard/inventory that proves the selected row text is `Path to Tenuto`, not `Debug Save / Prologue`, immediately before the confirm/load transition. Do not enable bodyfast/codegen-fast/Vulkan/GPU modes from this evidence.

## 2026-06-05 13:24 ET - RPCS3 0.0.41 Verify25cc load-target gate aborts before save-row proof (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T17:23:49.594Z`.
- Refiner: `2026-06-05T13:24:11.8839336-04:00`; decision remained generic state-aware movement from the newest valid field. The selected non-duplicative action was the prior section's next hypothesis: use the harness `gate_load_target` token before slot confirm so the run either proves `PATH_TO_TENUTO_PRESENT` or aborts before wasting a full `Verify25ccShadow` capture.
- Core identity: Windows RPCS3 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
- Command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-rpcs3-0041-verify25cc-loadtarget-gated-field-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -EternalSonataSpuHleVerify Verify25ccShadow -WindowsHostContentionGate ExternalFail -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 170 -InputMacro "wait:65000;shot:title-settle;down:20;wait:500;shot:title-after-down;cross:80;wait:12000;shot:post-title-cross;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:500;shot:pre-load-target-gate;gate_load_target:30000;cross:120;wait:45000;shot:post-load-field-check;wait:20000;shot:post-load-field-late" -MaxSeconds 210 -ScreenshotEverySeconds 10 -ScreenshotStartSeconds 110 -ScreenshotMaxCount 10`.
- Run directory: `debug-captures/windows-lab/20260605-132434-cpu4-rpcs3-0041-verify25cc-loadtarget-gated-field-visualgate-windows-windows`.
- Load-target gate: failed and correctly aborted the macro before the final slot-confirm `Cross`. Marker: `Load target gate failed at 112s: timed out after 30000ms; last status BLACK_OVERLAY_OR_CRASH_PRESENT; expected only Path to Tenuto`.
- Load-target classifier: status `BLACK_OVERLAY_OR_CRASH_PRESENT`; counts `path-to-tenuto=0`, `debug-save-prologue=0`, `empty-load-slot=0`, `black-overlay-or-crash=2`, `unknown=8`; lower-row cursor markers `0`; damaged-save text markers `0`.
- Visual gate: `FIELD_LIKE_PRESENT_WITH_LATER_INVALID_SCREENSHOTS`; first field-like-sized frame `screenshot-0106s-load-target-gate-5.png` at `106s`, but invalid screenshot after first field-like `screenshot-0112s-load-target-gate-6.png`; gate result `failed`.
- Manual screenshot verification: `screenshot-0106s-load-target-gate-5.png` is a story/cutscene frame with subtitle text, not Path-to-Tenuto gameplay field and not a Load-list row. The visual-gate field-like label is a byte-size false positive. This run did not prove save-row selection, field, Options/menu, first battle, or movement.
- Fatal/log verification: targeted scan found no real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail line. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` contained only `MidiInWinMM::initialize: no MIDI input devices currently available.`
- Host verification: host contention stayed clean across `3` snapshots; no competing emulator or heavy host load.
- SPU verifier/parser result: strict parser rows `268`; accepted rows `268`; rejected rows `0`; total contract hits `577`; total contract bytes `9,453,568`; output mismatch `0`; descriptor overflow `0`; `promotion_ready=false`.
- Counter verification: direct log counts showed `SPU HLE verifier` rows `667`, `Eternal Sonata SPU contract verifier` rows `268`, `Eternal Sonata reservation loop` strings `22704`, `PUTLLC16` strings `8`, `RDCH` strings `62`, `RSX-local traffic` strings `0`, and `New promoted CPU/SPU` strings `0`.
- Classification: `verify-logrow-progress`, `strict-parser-pass`, `failed-load-target-gate`, `cutscene-route-miss`, `visual-byte-size-false-positive`, `rpcs3-0.0.41-core-smoke`, `stackable-cpu-pressure-plan`, `gpu-offload-parked`, `not-speed`.
- Speed status: confirmed speed increase remains `0%`. This is not field proof, Options/menu proof, first-battle proof, 200% proof, or GPU migration credit.
- Next hypothesis: the route is now failing before a stable Load-list row is even visible. Add a title-to-Load state gate or longer title-menu settle that proves the menu entered `Load` and shows `Path to Tenuto` before any load-target gate, `Verify25ccShadow`, bodyfast/codegen-fast, or Vulkan/GPU experiment.

## 2026-06-05 13:54 ET - polling load-target gate proves Debug Save damaged top row (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T17:53:54.879Z`.
- Refiner: `2026-06-05T13:54:17.6585176-04:00`; decision was to stop speed/HLE/RSX work and require `PATH_TO_TENUTO_PRESENT` from the polling load-target gate before continuing.
- Core identity: Windows RPCS3 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
- Command: `tools\eternal_sonata_speed_sprint.ps1 -Action WindowsScene -Scene field -Label cpu4-stateaware-loadtarget-pollgated-doubleconfirm-dismisssave-left200-visualgate-windows -WindowsInputBackend PadApi -WindowsGameScreen 1 -WindowsCpuAffinityMask 0x0F -WindowsFrameLimit 240 -WindowsVblankRate 240 -EternalSonataReservationLoop Verify -WindowsVisualGate CleanAfterField -WindowsVisualGateFieldSeconds 215 -InputMacro "wait:45000;down:20;wait:500;cross:80;wait:12000;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:160;up:80;wait:500;gate_load_target:30000;cross:80;wait:3000;up:80;wait:500;cross:80;wait:32000;cross:120;wait:18000;shot:100;up:80;wait:300;cross:120;wait:1200;cross:120;wait:35000;shot:100;down:80;wait:300;cross:120;wait:1500;ls_left:200;wait:1000;shot:100;wait:10000;shot:100" -MaxSeconds 260 -ScreenshotEverySeconds 10 -ScreenshotStartSeconds 155 -ScreenshotMaxCount 10`.
- Run directory: `debug-captures/windows-lab/20260605-135434-cpu4-stateaware-loadtarget-pollgated-doubleconfirm-dismisssave-left200-visualgate-windows-windows`.
- Load-target gate: failed at `65s` and correctly aborted before pressing slot `Cross`; marker says `expected only Path to Tenuto, got DEBUG_SAVE_PROLOGUE_PRESENT`.
- Load-target classifier: status `DEBUG_SAVE_PROLOGUE_PRESENT`; counts `path-to-tenuto=0`, `debug-save-prologue=1`, `empty-load-slot=0`, `black-overlay-or-crash=0`, `unknown=1`; damaged-save text markers `1`.
- Manual screenshot verification: `screenshot-0065s-load-target-gate-2.png` shows `Save File 01 / Debug Save / Prologue / Lv.00 / Time 00:00:00` and two visible `Save file has been damaged.` rows. This proves the current top-row route state is corrupted/wrong-row before any field load.
- Visual gate: `NO_FIELD_LIKE_SCREENSHOT`; first field-like screenshot `none`; required field-like by `215s` failed; only two load-gate screenshots were captured.
- Fatal/log verification: targeted scan found no real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail line. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` contained only `MidiInWinMM::initialize: no MIDI input devices currently available.`
- Host verification: clean across `3` snapshots; no competing emulator or heavy host load.
- Counter verification: `Verify25ccShadow` was intentionally off for this route-control gate. Direct log counts showed `SPU HLE verifier` rows `0`, `Eternal Sonata SPU contract verifier` rows `0`, `Eternal Sonata reservation loop` strings `13588`, `PUTLLC16` strings `8`, `RDCH` strings `62`, `RSX-local traffic` strings `0`, and `New promoted CPU/SPU` strings `0`.
- Classification: `failed-load-target-gate`, `debug-save-prologue-route-miss`, `damaged-save-top-row`, `route-control-blocker`, `gpu-offload-parked`, `not-speed`.
- Speed status: confirmed speed increase remains `0%`. This is not field proof, Options/menu proof, first-battle proof, 200% proof, HLE proof, or GPU migration credit.
- Next hypothesis: stop pressing through the current save-list state. The narrow next step is a save-list inventory/repair that restores or selects a clean `Path to Tenuto` row before re-enabling `Verify25ccShadow`; otherwise all parser/GPU/HLE evidence remains invalid because it starts from the damaged Debug Save row.

## 2026-06-05 14:24 ET - save-list inventory finds Path-to-Tenuto on Save File 04 middle row (no speed)

- Automation: `ps3-200-windows-speed-loop` hardware-acceleration lane, heartbeat `2026-06-05T18:23:55.329Z`.
- Refiner: `2026-06-05T14:24:18.2024406-04:00`; decision was to inventory current save-list rows without pressing the slot, because the previous polling repair selected damaged `Debug Save / Prologue`.
- Core identity: Windows RPCS3 0.0.41 build from `rpcs3-upstream\build-msvc\bin\rpcs3.exe`; title samples reported `0.0.41-595`.
...
- Run directory: `debug-captures/windows-lab/20260605-142433-cpu4-hle-25cc-shadow-desc-battle-stock-down160-strongdismiss600-save-list-inventory-after-pregate-debugsave-windows`.
- Inventory result: the load-target classifier reported `DAMAGED_SAVE_TARGET` overall, but found `path-to-tenuto=7`, `debug-save-prologue=0`, `unknown=2`, with damaged-save text markers in `7` screenshots.
- Manual screenshot verification: `screenshot-0130s-load-list-initial-after60.png` shows `Save File 04 / Path to Tenuto / South Section / Ch. 1 Raindrops` as the middle visible row. The top and bottom visible rows say `File does not exist`, and the classifier still detects damaged-save markers, so top-row targeting remains unsafe.
- Cursor/row implication: repeated `Down` screenshots kept the Path-to-Tenuto preview at crop row `365` while cursor markers drifted to lower rows. The stable useful target is the visible middle `Save File 04` row, not the current top-row assumption.
- Fatal/log verification: targeted scan found no real `VM: Access violation`, `VK_ERROR`, device-lost, unhandled, unknown STOP, assertion, verifier mismatch, output mismatch, or dynamic-fail line. `rpcs3.stdout.txt` length `0`; `rpcs3.stderr.txt` contained only `MidiInWinMM::initialize: no MIDI input devices currently available.`
- Host verification: clean across `5` snapshots; no competing emulator or heavy host load.
- Counter/GPU verification: this was an inventory run with SPU verifier and reservation-loop verify off. GPU-probe records `1364`; total observed DMA `1,347.62 MB`; offload fit mix `too-small=724`, `spu-kernel-hle=640`; direct RSX-local traffic `0`; indirect RSX overlap `0`; promoted CPU/SPU-to-GPU replacement `0`.
- Top CPU-pressure rows remained SPU-side, not GPU-side: the largest rows were `0x451c` TCX CellSpursKernel traffic and `0x25cc` / `0x9e4000` at `2.90 MB` in this inventory run, all with `0 B` RSX-local.
