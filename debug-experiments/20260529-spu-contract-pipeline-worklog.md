...
- SPU/counter evidence: SPU HLE verifier records 0; MFC dynamic records 2359; MFC list transfer records 1375; MFC wait records 2659; MFC wait exact-PC records 130729; reservation-loop command records 2659; reservation-loop command exact-PC records 71315; reservation-loop verify records 9015; RDCH join records 3; lane-join records 3; raw-lane records 8; total observed DMA 2809.91 MB; RSX-local traffic records 0; offload fit mix spu-kernel-hle=1378, 	oo-small=981.
- Classification: alid-field-triage, epaired-no-movement-control, post-left1262-fatal-control-clean, eservation-counters-present-but-route-only, 
ot-speed.
- Conclusion: no speed increase and no 200% proof. The repaired no-movement control did not reproduce the left1262 fatal, so the movement step remains the likely crash boundary candidate; next Windows-only non-duplicative work should either compare a tiny movement boundary from this clean baseline or inspect the  x002aedd0 /  x40 fatal site before fast paths.
