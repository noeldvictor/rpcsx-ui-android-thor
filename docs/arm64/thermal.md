...
device watchdog polls every 0.25 seconds and holds the app process at 68 C. The
controller adopts that process hold and waits until fixed silicon is below
60 C before it resumes a later slice. A value of 72 C still force-stops the app.
This margin absorbs a fast subsystem-sensor rise while the host is busy. It does
not change the cold-start rule: a value below 70 C can start immediately.
