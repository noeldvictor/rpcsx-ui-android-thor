...
- Update after the cache storage picker patch: Settings lets users choose app-owned internal compiled cache or app-owned SD-card compiled cache when Android exposes it. The core still sees `RPCSX.rootDirectory/cache/cache`; SD selection redirects that path with a symlink and warns that removable storage may slow cache-heavy boot/shader work.
- Update after the curated GPU driver patch: the custom driver screen keeps `Default` as fallback, adds Thor/Adreno 740 guidance, and exposes curated Turnip GitHub sources with notes. Release fetching now shows each ZIP asset separately so users can avoid OneUI and A8xx/Gen8 packages on Thor.
