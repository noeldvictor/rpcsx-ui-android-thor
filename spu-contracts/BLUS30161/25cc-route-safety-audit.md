...
- Safety classification: `failed-menu-window-missing / no-speed / no-gpu-migration-credit`. Do not promote 25cc HLE/body/codegen/GPU paths from this run; fix menu window/title-menu gating and require real Options/menu pixels before trying full payload.
## 2026-07-24 20:31 ET - counts-mode one-pulse route passed field triage
- New evidence: `debug-captures/windows-lab/20260724-202616-cpu4-loader-control-left200-25cc-counts-visualgate-windows-windows` combined one `ls_left:200` pulse with `Verify25ccShadow` and `EternalSonataSpuHle25ccShadowPayload Counts`; CleanAfterField passed (`FIELD_LIKE_PRESENT`, first field-like `117s`, required-by-160s passed, `14/14` field-like screenshots, invalid-after-first `0`).
- Strict counts parser passed: rows `80`, accepted `80`, rejected `0`, total hits `160`, total bytes `2,621,440`, rejects `2,243`, `payload_mode_counts={counts:80}`, mismatch `0`, descriptor overflow `0`, strict gate pass `true`, promotion_ready `false`.
- Safety interpretation: counts mode is now route-safe on both loader-control and one-pulse movement bases. It remains route-repair-only; `payload_mode=full`, Options/menu, first-battle visuals, and zero mismatch/overflow/fatal hits are still required before any promotion or fast path.
