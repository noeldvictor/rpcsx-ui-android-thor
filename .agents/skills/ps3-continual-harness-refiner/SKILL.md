...
HLE, RSX, GPU, or speed work.
If that `ls_left:1331` midpoint passes `PATH_TO_TENUTO_PRESENT` and reaches a
clean pre-movement Path-to-Tenuto field, but the immediate and later post-left
screenshots show RPCS3's likely-crashed overlay with corrupt/frozen field
visuals and stderr/RPCS3.log reports a PPU `VM: Access violation reading
location 0x40`, classify it as
...
Do not count it as clean movement, first-battle, speed, GPU migration, or 200%
evidence even if the byte-size visual gate says field-like. Keep `left1312` as
the clean lower boundary, `left1331` as the fatal/corrupt upper boundary, and
try the `ls_left:1321` midpoint with immediate post-movement screenshots before
verifier, battle, HLE, RSX, GPU, or speed work.
If that `ls_left:1321` midpoint also passes `PATH_TO_TENUTO_PRESENT` and reaches
a clean pre-movement Path-to-Tenuto field, but the immediate and later post-left
screenshots show RPCS3's likely-crashed overlay with corrupt/frozen field
visuals and stderr/RPCS3.log reports a PPU `VM: Access violation reading
location 0x40`, classify it as
...
Do not count it as clean movement, first-battle, speed, GPU migration, or 200%
evidence even if the byte-size visual gate says field-like. Keep `left1312` as
the clean lower boundary, `left1321` as the fatal/corrupt upper boundary, and
try the `ls_left:1316` midpoint with immediate post-movement screenshots before
verifier, battle, HLE, RSX, GPU, or speed work.
If that `ls_left:1316` proof passes `PATH_TO_TENUTO_PRESENT`, reaches clean
Path-to-Tenuto field, visibly accepts the left movement pulse, and stays
field-clean through late screenshots, classify it as
`hle-25cc-shadow-desc-battle-stock-down160-strongdismiss600-left1316-field-clean`.
This is still a clean movement boundary only: not first-battle proof, not speed,
not GPU migration, and not 200% evidence. Bank `left1316` as the clean lower
boundary below the `left1321` fatal/corrupt upper boundary, then try the
`ls_left:1318` midpoint with immediate screenshots before verifier, battle,
HLE, RSX, GPU, or speed work.
If that `ls_left:1318` midpoint also passes `PATH_TO_TENUTO_PRESENT` and reaches
a clean pre-movement Path-to-Tenuto field, but the immediate and later post-left
screenshots show RPCS3's likely-crashed overlay with corrupt/frozen field
visuals and stderr/RPCS3.log reports a PPU `VM: Access violation reading
location 0x40`, classify it as
...
Do not count it as clean movement, first-battle, speed, GPU migration, or 200%
evidence even if the byte-size visual gate says field-like. Keep `left1316` as
