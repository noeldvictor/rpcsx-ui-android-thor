...
| [`docs/arm64/armsx3-comparison.md`](docs/arm64/armsx3-comparison.md) | **ARMSX3 diffed against upstream RPCS3**, so their work is separated from what they inherited. They do not touch the wait/spin problem at all; we hold a measured 13.8% on the DMA threshold and have ARM AES they lack. They beat us on a NEON `copy_data_swap_u32`, and two of their changes challenge our config: they un-pinned the JIT CPU, and their affinity actually applies because they enable `thread_scheduler_mode::alt`. |
