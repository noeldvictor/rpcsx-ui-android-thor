...
#define ARCH_ARM64 1
// v8.4a+ gives us atomic 16 byte ld/st. There is still no ACLE feature macro
// for LSE2, so it has to be inferred or asserted from outside.
//
// The probe below is kept for toolchains that define the __ARM_ARCH_8_x__
// family, but be aware that clang on AArch64 defines none of them at any
// -march, so it never fires there. The Android build therefore passes
// -DARM_FEATURE_LSE2=1 explicitly from CMakeLists.txt, gated on the configured
// ISA baseline being Armv8.4-A or newer. If you are wondering why the 16-byte
// atomic fast paths in util/atomic.hpp look dead, this is why: without the
// macro, load/store/release all fall back to an LDAXP/STLXP retry loop that
// takes the line exclusive even to read.
