...
namespace rsx
{
	...
	struct registers_decoder<NV4097_SET_COLOR_MASK>
	{
		...
		static void dump(std::string& out, const decoded_type& decoded)
		{
			...
			fmt::append(out, "Surface: color mask A = %s R = %s G = %s B = %s", print_boolean(decoded.color_a()), print_boolean(decoded.color_r()), print_boolean(decoded.color_g()), print_boolean(decoded.color_b()));
		}
	};
	...
	struct registers_decoder<NV4097_SET_SHADER_WINDOW>
	{
		...
		private:
			...
			{
				return bf_decoder<12, 4>(value);
			}
			u8 window_shader_pixel_center_raw() const
			{
				return bf_decoder<16, 4>(value);
			}
	...
	struct registers_decoder<NV4097_SET_USER_CLIP_PLANE_CONTROL>
	{
		...
		private:
			...
			u8 clip_plane0_raw() const
			{
				return bf_decoder<0, 4>(value);
			}
			u8 clip_plane1_raw() const
			{
				return bf_decoder<4, 4>(value);
			}
			u8 clip_plane2_raw() const
			{
				return bf_decoder<8, 4>(value);
			}
			u8 clip_plane3_raw() const
			{
				return bf_decoder<12, 4>(value);
			}
			u8 clip_plane4_raw() const
			{
				return bf_decoder<16, 4>(value);
			}
			u8 clip_plane5_raw() const
			{
				return bf_decoder<20, 4>(value);
			}
		...
		static void dump(std::string& out, const decoded_type& decoded)
		{
			fmt::append(out, "User clip: UC0: %s UC1: %s UC2: %s UC3: %s UC4: %s", decoded.clip_plane0(), decoded.clip_plane1(), decoded.clip_plane2(), decoded.clip_plane3(), decoded.clip_plane4(), decoded.clip_plane5());
		}
	};
	...
	struct registers_decoder<NV4097_SET_SURFACE_FORMAT>
	{
		...
		private:
			...
			u8 color_fmt_raw() const
			{
				return bf_decoder<0, 5>(value);
			}
			u8 depth_fmt_raw() const
			{
				return bf_decoder<5, 3>(value);
			}
			u8 type_raw() const
			{
				return bf_decoder<8, 4>(value);
			}
			u8 antialias_raw() const
			{
				return bf_decoder<12, 4>(value);
			}
		...
		static void dump(std::string& out, const decoded_type& decoded)
		{
			fmt::append(out, "Surface: Color format: %s DepthStencil format: %s Anti aliasing: %s w: %u h: %u", decoded.color_fmt(), decoded.depth_fmt(), decoded.antialias(), decoded.log2width(), decoded.log2height());
		}
	};
	...
	struct registers_decoder<NV3089_IMAGE_IN_FORMAT>
	{
		...
		private:
			...
			u8 transfer_origin_raw() const
			{
				return bf_decoder<16, 8>(value);
			}
			u8 transfer_interpolator_raw() const
			{
				return bf_decoder<24, 8>(value);
			}
		...
		static void dump(std::string& out, const decoded_type& decoded)
		{
			fmt::append(out, "NV3089: input fmt: %u origin: %s interp: %s", decoded.format(), decoded.transfer_origin(), decoded.transfer_interpolator());
		}
	};
	...
	struct registers_decoder<NV309E_SET_FORMAT>
	{
		...
		private:
			...
			u32 transfer_destination_fmt() const
			{
				return bf_decoder<0, 16, u32>(value);
			}
	...
	struct registers_decoder<NV4097_SET_BLEND_COLOR>
	{
		...
		static void dump(std::string& out, const decoded_type& decoded)
		{
			fmt::append(out, "Blend color: 8b BGRA = %u, %u, %u, %u 16b RG = %u , %u", decoded.blue8(), decoded.green8(), decoded.red8(), decoded.alpha8(), decoded.red16(), decoded.green16());
		}
	};
...
#define TRANSFORM_CONSTANT(index)                                                                             \
	template <>                                                                                               \
	struct registers_decoder<NV4097_SET_TRANSFORM_CONSTANT + index> : public transform_constant_helper<index> \
	{                                                                                                         \
	};
...
#define TRANSFORM_PROGRAM(index)                                                                            \
	template <>                                                                                             \
	struct registers_decoder<NV4097_SET_TRANSFORM_PROGRAM + index> : public transform_program_helper<index> \
	{                                                                                                       \
	};
...
#define VERTEX_DATA_ARRAY_FORMAT(index)                                                                       \
	template <>                                                                                               \
	struct registers_decoder<NV4097_SET_VERTEX_DATA_ARRAY_FORMAT + index> : public vertex_array_helper<index> \
	{                                                                                                         \
	};
...
#define VERTEX_DATA_ARRAY_OFFSET(index)                                                                              \
	template <>                                                                                                      \
	struct registers_decoder<NV4097_SET_VERTEX_DATA_ARRAY_OFFSET + index> : public vertex_array_offset_helper<index> \
	{                                                                                                                \
	};
...
#define VERTEX_DATA4UB(index)                                                                                   \
	template <>                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_DATA4UB_M + index> : public register_vertex_helper<index, u8, 4> \
	{                                                                                                           \
	};
#define VERTEX_DATA1F(index)                                                                                    \
	template <>                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_DATA1F_M + index> : public register_vertex_helper<index, f32, 1> \
	{                                                                                                           \
	};
#define VERTEX_DATA2F(index)                                                                                    \
	template <>                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_DATA2F_M + index> : public register_vertex_helper<index, f32, 2> \
	{                                                                                                           \
	};
#define VERTEX_DATA3F(index)                                                                                    \
	template <>                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_DATA3F_M + index> : public register_vertex_helper<index, f32, 3> \
	{                                                                                                           \
	};
#define VERTEX_DATA4F(index)                                                                                    \
	template <>                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_DATA4F_M + index> : public register_vertex_helper<index, f32, 4> \
	{                                                                                                           \
	};
#define VERTEX_DATA2S(index)                                                                                    \
	template <>                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_DATA2S_M + index> : public register_vertex_helper<index, u16, 2> \
	{                                                                                                           \
	};
#define VERTEX_DATA4S(index)                                                                                    \
	template <>                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_DATA4S_M + index> : public register_vertex_helper<index, u16, 4> \
	{                                                                                                           \
	};
...
#define DECLARE_VERTEX_DATA4S(index) \
	...
	struct texture_control0_helper
	{
		...
		static void dump(std::string& out, const decoded_type& decoded)
		{
			fmt::append(out, "Texture %u: %s, Min/Max LOD: %g/%g, Max Aniso: %s, AKill: %s", index, print_boolean(decoded.enabled()), decoded.min_lod(), decoded.max_lod(), decoded.max_aniso(), print_boolean(decoded.alpha_kill_enabled()));
		}
	};
...
#define TEXTURE_OFFSET(index)                                                                                 \
	template <>                                                                                               \
	struct registers_decoder<NV4097_SET_TEXTURE_OFFSET + ((index) * 8)> : public texture_offset_helper<index> \
	{                                                                                                         \
	};
#define TEXTURE_FORMAT(index)                                                                                 \
	template <>                                                                                               \
	struct registers_decoder<NV4097_SET_TEXTURE_FORMAT + ((index) * 8)> : public texture_format_helper<index> \
	{                                                                                                         \
	};
#define TEXTURE_IMAGE_RECT(index)                                                                                     \
	template <>                                                                                                       \
	struct registers_decoder<NV4097_SET_TEXTURE_IMAGE_RECT + ((index) * 8)> : public texture_image_rect_helper<index> \
	{                                                                                                                 \
	};
#define TEXTURE_CONTROL0(index)                                                                                   \
	template <>                                                                                                   \
	struct registers_decoder<NV4097_SET_TEXTURE_CONTROL0 + ((index) * 8)> : public texture_control0_helper<index> \
	{                                                                                                             \
	};
#define TEXTURE_CONTROL3(index)                                                                           \
	template <>                                                                                           \
	struct registers_decoder<NV4097_SET_TEXTURE_CONTROL3 + index> : public texture_control3_helper<index> \
	{                                                                                                     \
	};
...
#define DECLARE_TEXTURE_CONTROL3(index) \
	...
	struct vertex_texture_control0_helper
	{
		...
		static void dump(std::string& out, const decoded_type& decoded)
		{
			fmt::append(out, "VTexture %u: %s, Min/Max LOD: %g/%g", index, print_boolean(decoded.enabled()), decoded.min_lod(), decoded.max_lod());
		}
	};

#define VERTEX_TEXTURE_CONTROL0(index)                                                                                          \
	template <>                                                                                                                 \
	struct registers_decoder<NV4097_SET_VERTEX_TEXTURE_CONTROL0 + ((index) * 8)> : public vertex_texture_control0_helper<index> \
	{                                                                                                                           \
	};
