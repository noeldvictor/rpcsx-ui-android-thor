...
// Value 4 selects the shared title wait after the libnet load.
...
// Default-off, title-gated LLVM probe for Eternal Sonata's draw-command
...
void ppu_thor_es_command9_probe(ppu_thread& ppu, u64 stream_start, u64 object,
	u64 argument, u32 cia);
void ppu_thor_es_command61_probe(ppu_thread& ppu, u64 stream_pointer, u64 source_pointer,
	u64 arg0, u64 arg1, u64 arg2, u32 cia);
void ppu_thor_es_template_probe(ppu_thread& ppu, u64 stream_start, u64 object, u32 cia);
...
// Default-off, title-gated verifier for Eternal Sonata's asynchronous draw-job
// drain. It observes the exact generated-command range passed to the async job
// builder after the game's drain returns and never mutates guest memory.
...
// Compatibility resolver for stale diagnostic objects; current codegen does
// not emit this hook and the implementation is intentionally a no-op.
