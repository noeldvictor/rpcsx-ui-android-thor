...
namespace
{
	...
	constexpr u32 thor_es_async_draw_stable_intervals = 2;
	constexpr u32 thor_es_async_draw_consume_window = 64;
	...
	constexpr std::array<u32, 2> thor_es_command9_arg_stores{
		0x002eda84,
		0x002ee6e8,
	};
	constexpr std::array<u32, 6> thor_es_command61_complete_stores{
		0x002caac0,
		0x002e13c8,
		0x002e80d8,
		0x002e8a88,
	...
	constexpr std::array<u32, 2> thor_es_template_complete_stores{
		0x00313420,
		0x00319158,
	};
	constexpr u32 thor_es_draw_stream_size = 0x18'0000;
	constexpr usz thor_es_command60_event_capacity = 4096;
	constexpr usz thor_es_command9_event_capacity = 4096;
	constexpr usz thor_es_command61_event_capacity = 4096;
	constexpr usz thor_es_template_event_capacity = 4096;
	...
	static_assert((thor_es_command9_event_capacity & (thor_es_command9_event_capacity - 1)) == 0);
	static_assert((thor_es_command61_event_capacity & (thor_es_command61_event_capacity - 1)) == 0);
	static_assert((thor_es_template_event_capacity & (thor_es_template_event_capacity - 1)) == 0);
	...
	std::array<atomic_t<u64>, thor_es_command9_event_capacity> thor_es_command9_events{};
	std::array<atomic_t<u64>, thor_es_command9_event_capacity> thor_es_command9_objects_ppus{};
	std::array<atomic_t<u64>, thor_es_command9_event_capacity> thor_es_command9_arguments{};
	std::array<atomic_t<u64>, thor_es_command9_event_capacity> thor_es_command9_orders{};
	std::array<atomic_t<u64>, thor_es_command9_event_capacity> thor_es_command9_sequences{};
	std::array<atomic_t<u64>, thor_es_command61_event_capacity> thor_es_command61_events{};
	std::array<atomic_t<u64>, thor_es_command61_event_capacity> thor_es_command61_sources{};
	std::array<atomic_t<u64>, thor_es_command61_event_capacity> thor_es_command61_payloads{};
	std::array<atomic_t<u64>, thor_es_command61_event_capacity> thor_es_command61_sequences{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_events{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_words01{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_completion_words01{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_words23{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_words45{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_word6_ppus{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_objects{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_orders{};
	std::array<atomic_t<u64>, thor_es_template_event_capacity> thor_es_template_sequences{};
	...
	std::array<atomic_t<u64>, thor_es_async_draw_event_capacity> thor_es_async_draw_event_sequences{};
	...
	std::array<atomic_t<u64>, thor_es_async_draw_event_capacity> thor_es_async_draw_snapshot_sequences{};
	std::array<atomic_t<u32>, thor_es_async_draw_event_capacity> thor_es_async_draw_snapshot_readers{};
	std::array<atomic_t<bool>, thor_es_async_draw_event_capacity> thor_es_async_draw_snapshot_writers{};
	atomic_t<u64> thor_es_command60_event_cursor = 0;
	atomic_t<u64> thor_es_command9_event_cursor = 0;
	atomic_t<u64> thor_es_command61_event_cursor = 0;
	atomic_t<u64> thor_es_template_event_cursor = 0;
	atomic_t<u64> thor_es_provenance_order_cursor = 0;
	atomic_t<u64> thor_es_publish_event_cursor = 0;
	atomic_t<u64> thor_es_async_draw_event_cursor = 0;
	atomic_t<u64> thor_es_async_draw_event_published_cursor = 0;
	atomic_t<u64> thor_es_async_draw_barrier_cursor = 0;

	using thor_es_async_draw_snapshot_store =
		std::array<std::array<u8, thor_es_async_draw_target_max_size>, thor_es_async_draw_event_capacity>;

	thor_es_async_draw_snapshot_store& get_thor_es_async_draw_snapshot_store()
	{
		// Repair mode owns this 4 MiB rollback store. Keep it lazy so normal
		// probe-off and verify-only runs do not commit the memory.
		static const auto store = std::make_unique<thor_es_async_draw_snapshot_store>();
		return *store;
	}

	void lock_thor_es_async_draw_snapshot(usz index)
	{
		while (thor_es_async_draw_snapshot_writers[index].exchange(true))
		{
			thread_ctrl::wait_for(1, false);
		}
		std::atomic_thread_fence(std::memory_order_acquire);
	}

	void invalidate_locked_thor_es_async_draw_snapshot(usz index)
	{
		thor_es_async_draw_snapshot_sequences[index].release(0);
		while (thor_es_async_draw_snapshot_readers[index].load())
		{
			thread_ctrl::wait_for(1, false);
		}
		std::atomic_thread_fence(std::memory_order_acquire);
	...
	void unlock_thor_es_async_draw_snapshot(usz index)
	{
		std::atomic_thread_fence(std::memory_order_release);
		thor_es_async_draw_snapshot_writers[index].release(false);
	}
	...
	bool hash_thor_es_async_draw_target(u32 address, u32 size, u64& hash)
	{
		...
		std::array<u64, chunk_size / sizeof(u64)> chunk{};
		...
		for (u32 offset = 0; offset < size; offset += chunk_size)
		{
			...
			const u32 word_count = count / sizeof(u64);
			for (u32 index = 0; index < word_count; index++)
			{
				result = rpcs3::hash64(result, chunk[index]);
			}

			const auto* const tail = reinterpret_cast<const u8*>(chunk.data()) + word_count * sizeof(u64);
			for (u32 index = word_count * sizeof(u64); index < count; index++)
			{
				result = rpcs3::hash64(result, tail[index - word_count * sizeof(u64)]);
			}
		}
	...
	if (contains_probe_site)
	{
		...
		if (!logged.exchange(true))
		{
			ppu_log.notice("Thor Eternal Sonata PPU dispatch probe v5 enabled: loads=[0x%x,0x%x] "
				...
				static_cast<u32>(thor_es_command9_arg_stores.size()),
				static_cast<u32>(thor_es_command60_stores.size()),
				static_cast<u32>(thor_es_command61_complete_stores.size()),
				static_cast<u32>(thor_es_template_complete_stores.size()));
		}
	}
...
bool ppu_thor_es_dispatch_provenance_range(u32 address, u32 size)
{
	...
	for (const u32 cia : thor_es_command9_arg_stores)
	{
		if (address <= cia && cia < end)
		{
			return true;
	...
	for (const u32 cia : thor_es_command61_complete_stores)
	{
		if (address <= cia && cia < end)
		{
			return true;
	...
	for (const u32 cia : thor_es_template_complete_stores)
	{
		if (address <= cia && cia < end)
		{
			return true;
...
void ppu_thor_es_command9_probe(ppu_thread& ppu, u64 stream_start64, u64 object64,
	u64 argument64, u32 cia)
{
	const bool known_emitter = std::find(thor_es_command9_arg_stores.begin(),
		thor_es_command9_arg_stores.end(), cia) != thor_es_command9_arg_stores.end();
	if (Emu.GetTitleID() != "BLUS30161" || !get_thor_es_dispatch_probe_enabled() ||
		!known_emitter || stream_start64 > 0xffff'ffffu - 7 || object64 > 0xffff'ffffu)
	{
		return;
	}

	const u32 stream_start = static_cast<u32>(stream_start64);
	const u32 argument = static_cast<u32>(argument64);
	std::array<be_t<u32>, 2> words{};
	if (!vm::try_access(stream_start, words.data(), sizeof(words), false) ||
		static_cast<u32>(words[0]) != 9 || static_cast<u32>(words[1]) != argument)
	{
		return;
	}

	const u64 order = thor_es_provenance_order_cursor.fetch_add(1) + 1;
	const u64 cursor = thor_es_command9_event_cursor.fetch_add(1);
	const usz index = cursor & (thor_es_command9_event_capacity - 1);
	thor_es_command9_sequences[index].release(0);
	thor_es_command9_objects_ppus[index].release(
		static_cast<u64>(static_cast<u32>(object64)) << 32 | ppu.id);
	thor_es_command9_arguments[index].release(argument);
	thor_es_command9_orders[index].release(order);
	thor_es_command9_events[index].release(static_cast<u64>(stream_start) << 32 | cia);
	thor_es_command9_sequences[index].release(cursor + 1);
}

void ppu_thor_es_command61_probe(ppu_thread&, u64 stream_pointer, u64 source_pointer,
	u64 arg0, u64 arg1, u64 arg2, u32 cia)
{
	const bool known_emitter = std::find(thor_es_command61_complete_stores.begin(),
		thor_es_command61_complete_stores.end(), cia) != thor_es_command61_complete_stores.end();
	if (Emu.GetTitleID() != "BLUS30161" || !get_thor_es_dispatch_probe_enabled() || !known_emitter)
	{
		return;
	}
	...
	const usz index = cursor & (thor_es_command61_event_capacity - 1);
	thor_es_command61_sequences[index].release(0);
	thor_es_command61_sources[index].release(
		static_cast<u64>(static_cast<u32>(arg0)) << 32 | static_cast<u32>(source_pointer));
	thor_es_command61_payloads[index].release(
		static_cast<u64>(static_cast<u32>(arg1)) << 32 | static_cast<u32>(arg2));
	// Publish the sequence after the complete snapshot. Clearing it first and
	// checking it twice lets a reader reject a slot being overwritten in place.
	thor_es_command61_events[index]
		.release(static_cast<u64>(address) << 32 | cia);
	thor_es_command61_sequences[index].release(cursor + 1);
}

void ppu_thor_es_template_probe(ppu_thread& ppu, u64 stream_start64, u64 object64, u32 cia)
{
	const bool known_emitter = std::find(thor_es_template_complete_stores.begin(),
		thor_es_template_complete_stores.end(), cia) != thor_es_template_complete_stores.end();
	if (Emu.GetTitleID() != "BLUS30161" || !get_thor_es_dispatch_probe_enabled() ||
		!known_emitter || stream_start64 > 0xffff'ffffu - 0x1b || object64 > 0xffff'ffffu)
	{
		return;
	}

	const u32 stream_start = static_cast<u32>(stream_start64);
	std::array<be_t<u32>, 7> words{};
	if (!vm::try_access(stream_start, words.data(), sizeof(words), false) ||
		static_cast<u32>(words[3]) != 0x1e || static_cast<u32>(words[4]) != 0x009c4798 ||
		static_cast<u32>(words[5]) != 0x1f || static_cast<u32>(words[6]) != 3)
	{
		return;
	}

	const u64 order = thor_es_provenance_order_cursor.fetch_add(1) + 1;
	const u64 cursor = thor_es_template_event_cursor.fetch_add(1);
	const usz index = cursor & (thor_es_template_event_capacity - 1);
	thor_es_template_sequences[index].release(0);
	// Ghidra proves both completion sites intend this fixed header. Retain it
	// even when another writer has already replaced the header before the final
	// pointer store, and keep the completion-time memory separately.
	thor_es_template_words01[index].release(
		static_cast<u64>(0x1c) << 32);
	thor_es_template_completion_words01[index].release(
		static_cast<u64>(static_cast<u32>(words[0])) << 32 | static_cast<u32>(words[1]));
	thor_es_template_words23[index].release(
		static_cast<u64>(static_cast<u32>(words[2])) << 32 | static_cast<u32>(words[3]));
	thor_es_template_words45[index].release(
		static_cast<u64>(static_cast<u32>(words[4])) << 32 | static_cast<u32>(words[5]));
	thor_es_template_word6_ppus[index].release(
		static_cast<u64>(static_cast<u32>(words[6])) << 32 | ppu.id);
	thor_es_template_objects[index].release(static_cast<u32>(object64));
	thor_es_template_orders[index].release(order);
	thor_es_template_events[index].release(static_cast<u64>(stream_start) << 32 | cia);
	thor_es_template_sequences[index].release(cursor + 1);
}
	...
	u64 object, u64 parser_mode, u32 cia)
{
	...
	u32 command61_source_pointer = 0;
	u32 command61_emitted_arg0 = 0;
	u32 command61_emitted_arg1 = 0;
	u32 command61_emitted_arg2 = 0;
	if (command61_handler)
	{
		...
		for (u64 age = 0; age < event_count; age++)
		{
			const u64 expected_sequence = cursor - age;
			const usz index = (cursor - age - 1) & (thor_es_command61_event_capacity - 1);
			if (thor_es_command61_sequences[index].load() != expected_sequence)
			{
				continue;
			}

			const u64 event = thor_es_command61_events[index].load();
			if (static_cast<u32>(event >> 32) == address)
			{
				const u64 source = thor_es_command61_sources[index].load();
				const u64 payload = thor_es_command61_payloads[index].load();
				if (thor_es_command61_sequences[index].load() != expected_sequence)
				{
					continue;
				}
				...
				command61_source_pointer = static_cast<u32>(source);
				command61_emitted_arg0 = static_cast<u32>(source >> 32);
				command61_emitted_arg1 = static_cast<u32>(payload >> 32);
				command61_emitted_arg2 = static_cast<u32>(payload);
				break;
			}
		}
	}
	const char* const command61_payload_relation = !command61_emitter_cia || !command61_args_valid
		? "unknown"
		: command61_emitted_arg0 == command61_arg0 &&
			command61_emitted_arg1 == command61_arg1 &&
			command61_emitted_arg2 == command61_arg2
			? "stable_since_emit"
			: "changed_since_emit";

	u32 command9_start = 0;
	u32 command9_emitter_cia = 0;
	u32 command9_emitter_ppu = 0;
	u32 command9_object = 0;
	u32 command9_argument = 0;
	u32 command9_event_age = umax;
	u64 command9_order = 0;
	if (address >= 12)
	{
		const u32 wanted_start = address - 12;
		const u64 cursor = thor_es_command9_event_cursor.load();
		const u64 event_count = std::min<u64>(cursor, thor_es_command9_event_capacity);
		for (u64 age = 0; age < event_count; age++)
		{
			const u64 expected_sequence = cursor - age;
			const usz index = (cursor - age - 1) & (thor_es_command9_event_capacity - 1);
			if (thor_es_command9_sequences[index].load() != expected_sequence)
			{
				continue;
			}

			const u64 event = thor_es_command9_events[index].load();
			if (static_cast<u32>(event >> 32) != wanted_start)
			{
				continue;
			}

			const u64 object_ppu = thor_es_command9_objects_ppus[index].load();
			const u32 argument = static_cast<u32>(thor_es_command9_arguments[index].load());
			const u64 order = thor_es_command9_orders[index].load();
			if (thor_es_command9_sequences[index].load() != expected_sequence)
			{
				continue;
			}

			command9_start = wanted_start;
			command9_emitter_cia = static_cast<u32>(event);
			command9_emitter_ppu = static_cast<u32>(object_ppu);
			command9_object = static_cast<u32>(object_ppu >> 32);
			command9_argument = argument;
			command9_event_age = static_cast<u32>(age);
			command9_order = order;
			break;
		}
	}
	u32 command9_current_command = 0;
	u32 command9_current_argument = 0;
	const bool command9_current_valid = command9_start &&
		read_u32(command9_start, command9_current_command) &&
		read_u32(command9_start + sizeof(u32), command9_current_argument);
	const bool command9_stable = command9_current_valid && command9_current_command == 9 &&
		command9_current_argument == command9_argument;
	const char* const command9_object_relation = !command9_start ? "unknown" :
		command9_object == object_address ? "same_as_parser" : "different_from_parser";

	u32 template_start = 0;
	u32 template_emitter_cia = 0;
	u32 template_emitter_ppu = 0;
	u32 template_object = 0;
	u32 template_event_age = umax;
	u32 template_fault_word = umax;
	u64 template_order = 0;
	u32 template_completion_word0 = 0;
	u32 template_completion_word1 = 0;
	std::array<u32, 7> template_emitted{};
	{
		const u64 cursor = thor_es_template_event_cursor.load();
		const u64 event_count = std::min<u64>(cursor, thor_es_template_event_capacity);
		for (u64 age = 0; age < event_count; age++)
		{
			const u64 expected_sequence = cursor - age;
			const usz index = (cursor - age - 1) & (thor_es_template_event_capacity - 1);
			if (thor_es_template_sequences[index].load() != expected_sequence)
			{
				continue;
			}

			const u64 event = thor_es_template_events[index].load();
			const u32 start = static_cast<u32>(event >> 32);
			if (address < start || address - start >= 0x1c || (address - start) % sizeof(u32))
			{
				continue;
			}

			const u64 words01 = thor_es_template_words01[index].load();
			const u64 completion_words01 = thor_es_template_completion_words01[index].load();
			const u64 words23 = thor_es_template_words23[index].load();
			const u64 words45 = thor_es_template_words45[index].load();
			const u64 word6_ppu = thor_es_template_word6_ppus[index].load();
			const u32 object = static_cast<u32>(thor_es_template_objects[index].load());
			const u64 order = thor_es_template_orders[index].load();
			if (thor_es_template_sequences[index].load() != expected_sequence)
			{
				continue;
			}
			...
			template_emitter_cia = static_cast<u32>(event);
			template_emitter_ppu = static_cast<u32>(word6_ppu);
			template_object = object;
			template_event_age = static_cast<u32>(age);
			template_fault_word = (address - start) / sizeof(u32);
			template_order = order;
			template_completion_word0 = static_cast<u32>(completion_words01 >> 32);
			template_completion_word1 = static_cast<u32>(completion_words01);
			template_emitted = {
				static_cast<u32>(words01 >> 32), static_cast<u32>(words01),
				static_cast<u32>(words23 >> 32), static_cast<u32>(words23),
				static_cast<u32>(words45 >> 32), static_cast<u32>(words45),
				static_cast<u32>(word6_ppu >> 32),
			};
			break;
		}
	}

	std::array<be_t<u32>, 7> template_current{};
	const bool template_current_valid = template_start &&
		vm::try_access(template_start, template_current.data(), sizeof(template_current), false);
	bool template_stable = template_current_valid;
	for (usz index = 0; template_stable && index < template_current.size(); index++)
	{
		template_stable = template_emitted[index] == static_cast<u32>(template_current[index]);
	}
	const bool template_fault_word_stable = template_current_valid &&
		template_fault_word < template_current.size() &&
		template_emitted[template_fault_word] == static_cast<u32>(template_current[template_fault_word]);
	const char* const template_payload_relation = !template_start ? "unknown" :
		!template_current_valid ? "unreadable" : template_stable ? "stable_since_emit" : "changed_since_emit";
	const char* const template_completion_relation = !template_start ? "unknown" :
		template_completion_word0 == template_emitted[0] &&
			template_completion_word1 == template_emitted[1]
			? "header_intact_at_complete"
			: "header_changed_at_complete";
	const char* const template_object_relation = !template_start ? "unknown" :
		template_object == object_address ? "same_as_parser" : "different_from_parser";
	const char* const command9_template_object_relation = !command9_start || !template_start ? "unknown" :
		command9_object == template_object ? "same_object" : "different_object";
	const char* const command9_template_ppu_relation = !command9_start || !template_start ? "unknown" :
		command9_emitter_ppu == template_emitter_ppu ? "same_ppu" : "different_ppu";
	const char* const command9_template_order_relation = !command9_order || !template_order ? "unknown" :
		command9_order < template_order ? "command9_before_template_complete" :
		command9_order > template_order ? "command9_after_template_complete" : "same_order";
	...
	{
		const u64 cursor = thor_es_async_draw_event_published_cursor.load();
		const u64 event_count = std::min<u64>(cursor, thor_es_async_draw_event_capacity);
		for (u64 age = 0; age < event_count; age++)
		{
			const u64 event_sequence = cursor - age;
			const usz index = (cursor - age - 1) & (thor_es_async_draw_event_capacity - 1);
			if (thor_es_async_draw_event_sequences[index].load() != event_sequence)
			{
				continue;
			}
				...
				static_cast<u64>(address) < static_cast<u64>(target) + size)
			{
				...
				const u64 before_hash = thor_es_async_draw_before_hashes[index].load();
				const u64 after_hash = thor_es_async_draw_after_hashes[index].load();
				if (thor_es_async_draw_event_sequences[index].load() != event_sequence)
				{
					continue;
				}
				...
				async_initial_hash = initial_hash;
				async_before_hash = before_hash;
				async_after_hash = after_hash;
				break;
			}
		}
	}
	...
	ppu_log.error(
		...
		"command61_source=0x%x command61_emitted_arg0=0x%x "
		"command61_emitted_arg1=0x%x command61_emitted_arg2=0x%x "
		"command61_payload_relation=%s "
		"command9_start=0x%x command9_emitter=0x%x command9_emitter_ppu=0x%x "
		"command9_object=0x%x command9_argument=0x%x command9_event_age=%u command9_order=%llu "
		"command9_current_valid=%u command9_stable=%u command9_current=[0x%x,0x%x] "
		"command9_object_relation=%s "
		"template_start=0x%x template_emitter=0x%x template_emitter_ppu=0x%x template_object=0x%x "
		"template_event_age=%u template_fault_word=%u template_fault_word_stable=%u template_order=%llu "
		"template_payload_relation=%s template_current_valid=%u template_completion_relation=%s "
		"template_object_relation=%s command9_template_object_relation=%s "
		"command9_template_ppu_relation=%s command9_template_order_relation=%s "
		"template_completion_words01=[0x%x,0x%x] "
		"template_emitted=[0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x] "
		"template_current=[0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x] "
		...
		command61_source_pointer, command61_emitted_arg0, command61_emitted_arg1,
		command61_emitted_arg2, command61_payload_relation,
		command9_start, command9_emitter_cia, command9_emitter_ppu, command9_object,
		command9_argument, command9_event_age, command9_order, command9_current_valid,
		command9_stable, command9_current_command, command9_current_argument,
		command9_object_relation,
		template_start, template_emitter_cia, template_emitter_ppu, template_object,
		template_event_age, template_fault_word, template_fault_word_stable, template_order,
		template_payload_relation, template_current_valid, template_completion_relation,
		template_object_relation, command9_template_object_relation,
		command9_template_ppu_relation, command9_template_order_relation,
		template_completion_word0, template_completion_word1,
		template_emitted[0], template_emitted[1], template_emitted[2], template_emitted[3],
		template_emitted[4], template_emitted[5], template_emitted[6],
		static_cast<u32>(template_current[0]), static_cast<u32>(template_current[1]),
		static_cast<u32>(template_current[2]), static_cast<u32>(template_current[3]),
		static_cast<u32>(template_current[4]), static_cast<u32>(template_current[5]),
		static_cast<u32>(template_current[6]),
...
bool ppu_thor_es_async_draw_barrier_range(u32 address, u32 size)
{
	...
	if (contains_site)
	{
		...
		if (!logged.exchange(true))
		{
			ppu_log.notice("Thor Eternal Sonata async draw consume guard v7 enabled: mode=%s descriptor=0x%x consumer_entry=0x%x dispatch_loads=[0x%x,0x%x] max_target_size=0x%x consume_window=%u wait_limit_us=%llu stable_intervals=%u fingerprint=fnv-fold64",
				...
				thor_es_async_draw_consume_window,
				thor_es_async_draw_wait_limit_us, thor_es_async_draw_stable_intervals);
		}
	}
...
void ppu_thor_es_async_draw_target(ppu_thread&, u64 target_address64, u64 target_size64, u32 cia)
{
	...
	lock_thor_es_async_draw_snapshot(index);
	thor_es_async_draw_event_sequences[index].release(0);
	invalidate_locked_thor_es_async_draw_snapshot(index);
	...
	thor_es_async_draw_event_sequences[index].release(cursor + 1);
	unlock_thor_es_async_draw_snapshot(index);

	while (thor_es_async_draw_event_published_cursor.load() != cursor)
	{
		thread_ctrl::wait_for(1, false);
	}
	thor_es_async_draw_event_published_cursor.release(cursor + 1);
}
...
void ppu_thor_es_async_draw_barrier(ppu_thread&, u32 cia)
{
	...
	static atomic_t<u64> repair_hash_changes = 0;
	static atomic_t<u64> event_overflows = 0;

	const u64 hit = barrier_hits.fetch_add(1) + 1;
	const u64 event_end = thor_es_async_draw_event_published_cursor.load();
	...
	u64 batch_hash = 0;
	auto snapshot_targets = [&](bool after_grace)
	{
		u32 invalid = 0;
		...
		for (u64 event_index = event_begin; event_index < event_end; event_index++)
		{
			...
			const u64 event_sequence = event_index + 1;
			if (thor_es_async_draw_event_sequences[index].load() != event_sequence)
			{
				continue;
			}
	...
	const u32 invalid_before = snapshot_targets(false);
	...
	u64 waited_us = 0;
	u64 grace_us = 0;
	u32 stable_intervals = 0;
	u32 hash_changes = 0;
	u32 post_grace_snapshots = 0;
	u32 settled_snapshot_targets = 0;
	u64 settled_snapshot_bytes = 0;
	bool timed_out = false;
	if (mode == thor_es_async_draw_barrier_mode::repair && targets)
	{
		repair_waits.fetch_add(1);
		const u64 wait_start = get_system_time();
		u64 previous_batch_hash = batch_hash_before;
		const bool require_hash_stability = hashed_before != 0;
		do
		{
			std::atomic_thread_fence(std::memory_order_seq_cst);
			thread_ctrl::wait_for(20, false);
			std::atomic_thread_fence(std::memory_order_seq_cst);
			invalid_after = snapshot_targets(true);
			post_grace_snapshots++;
			waited_us = get_system_time() - wait_start;
			if (post_grace_snapshots == 1)
			{
				grace_us = waited_us;
			}

			if (require_hash_stability && previous_batch_hash && batch_hash)
			{
				if (previous_batch_hash == batch_hash)
				{
					stable_intervals++;
				}
				else
				{
					stable_intervals = 0;
					hash_changes++;
					repair_hash_changes.fetch_add(1);
				}
			}

			previous_batch_hash = batch_hash;
		}
		while ((invalid_after || (require_hash_stability && stable_intervals < thor_es_async_draw_stable_intervals)) &&
			waited_us < thor_es_async_draw_wait_limit_us && !Emu.IsStopped());

		timed_out = invalid_after != 0 ||
			(require_hash_stability && stable_intervals < thor_es_async_draw_stable_intervals);
		if (timed_out)
		{
	...
	if (mode == thor_es_async_draw_barrier_mode::repair && targets && !timed_out)
	{
		auto& snapshots = get_thor_es_async_draw_snapshot_store();
		for (u64 event_index = event_begin; event_index < event_end; event_index++)
		{
			const usz index = event_index & (thor_es_async_draw_event_capacity - 1);
			const u64 event_sequence = event_index + 1;
			...
			if (!address || size < sizeof(u32) || size > thor_es_async_draw_target_max_size ||
				static_cast<u64>(address) + size > 0x1'0000'0000ull ||
				!vm::try_access(address, snapshots[index].data(), size, false))
			{
				unlock_thor_es_async_draw_snapshot(index);
				continue;
			}
			...
			std::atomic_thread_fence(std::memory_order_release);
			thor_es_async_draw_snapshot_sequences[index].release(event_sequence);
			unlock_thor_es_async_draw_snapshot(index);
			settled_snapshot_targets++;
			settled_snapshot_bytes += size;
		}
	}

	if (hit == 1 || invalid_before || overflow || hit % 1024 == 0)
	{
		ppu_log.notice("Thor Eternal Sonata async draw post-drain v7: hit=%llu mode=%s targets=%u readable_before=%u readable_bytes_before=%llu readable_after=%u readable_bytes_after=%llu hashed_before=%u hashed_bytes_before=%llu batch_hash_before=0x%llx hashed_after=%u hashed_bytes_after=%llu batch_hash_after=0x%llx invalid_before=%u invalid_after=%u grace_us=%llu waited_us=%llu stable_intervals=%u hash_changes=%u snapshots=%u settled_snapshot_targets=%u settled_snapshot_bytes=%llu timeout=%u overflow=%u first_target=0x%x first_size=0x%x first_initial_first=0x%x first_initial_last=0x%x first_initial_hash=0x%llx first_before_first=0x%x first_before_last=0x%x first_before_hash=0x%llx first_before_invalid=0x%x first_after_target=0x%x first_after_first=0x%x first_after_last=0x%x first_after_hash=0x%llx first_after_invalid=0x%x graces=%llu timeouts=%llu total_hash_changes=%llu overflows=%llu",
			...
			batch_hash, invalid_before, invalid_after, grace_us, waited_us,
			stable_intervals, hash_changes, post_grace_snapshots, settled_snapshot_targets,
			settled_snapshot_bytes, timed_out, overflow,
			first_before_target,
			...
			repair_timeouts.load(), repair_hash_changes.load(), event_overflows.load());
	}
}

void ppu_thor_es_async_draw_consume(ppu_thread&, u64 stream_pointer64, u32 cia)
{
	if (Emu.GetTitleID() != "BLUS30161" ||
		get_thor_es_async_draw_barrier_mode() != thor_es_async_draw_barrier_mode::repair ||
		(cia != thor_es_dispatch_load_first && cia != thor_es_dispatch_load_next) ||
		stream_pointer64 > 0xffff'ffffu)
	{
		return;
	}

	const u32 address = static_cast<u32>(stream_pointer64);
	const u64 cursor = thor_es_async_draw_event_published_cursor.load();
	const u64 event_count = std::min<u64>(cursor, thor_es_async_draw_event_capacity);
	if (!event_count)
	{
	...
	auto& snapshots = get_thor_es_async_draw_snapshot_store();
	static atomic_t<u64> checks = 0;
	static atomic_t<u64> repairs = 0;
	static atomic_t<u64> repaired_bytes = 0;
	static atomic_t<u64> failures = 0;
	static atomic_t<u64> cached_sequence = 0;
	checks.fetch_add(1);

	auto consume_snapshot = [&](u64 event_sequence, u64 age)
	{
		const usz index = (event_sequence - 1) & (thor_es_async_draw_event_capacity - 1);
		if (thor_es_async_draw_snapshot_sequences[index].load() != event_sequence)
		{
			return false;
		}
		thor_es_async_draw_snapshot_readers[index].fetch_add(1);
		if (thor_es_async_draw_snapshot_sequences[index].load() != event_sequence)
		{
			thor_es_async_draw_snapshot_readers[index].fetch_sub(1);
			return false;
		}
		...
		const u32 target = static_cast<u32>(event >> 32);
		const u32 size = static_cast<u32>(event);
		if (!target || size < sizeof(u32) || address < target ||
			static_cast<u64>(address) >= static_cast<u64>(target) + size)
		{
			thor_es_async_draw_snapshot_readers[index].fetch_sub(1);
			return false;
		...
		const u32 offset = address - target;
		const u32 count = std::min(thor_es_async_draw_consume_window, size - offset);
		std::array<u8, thor_es_async_draw_consume_window> current{};
		std::array<u8, thor_es_async_draw_consume_window> settled{};
		if (count < sizeof(u32) || !vm::try_access(address, current.data(), count, false) ||
			thor_es_async_draw_snapshot_sequences[index].load() != event_sequence)
		{
			thor_es_async_draw_snapshot_readers[index].fetch_sub(1);
			return true;
		}

		std::memcpy(settled.data(), snapshots[index].data() + offset, count);
		if (thor_es_async_draw_snapshot_sequences[index].load() != event_sequence)
		{
			thor_es_async_draw_snapshot_readers[index].fetch_sub(1);
			return true;
		...
		if (std::memcmp(current.data(), settled.data(), count) == 0)
		{
			thor_es_async_draw_snapshot_readers[index].fetch_sub(1);
			return true;
		}
		...
		std::atomic_thread_fence(std::memory_order_seq_cst);
		if (!vm::try_access(address, settled.data(), count, true))
		{
		...
		const u64 hit = repairs.fetch_add(1) + 1;
		const u64 bytes = repaired_bytes.fetch_add(count) + count;
		if (hit <= 8 || hit % 1024 == 0)
		{
			ppu_log.notice("Thor Eternal Sonata async draw consume repair v7: hit=%llu ppu_cia=0x%x address=0x%x target=0x%x size=0x%x offset=0x%x bytes=%u event_age=%llu checks=%llu repairs=%llu repaired_bytes=%llu failures=%llu",
				hit, cia, address, target, size, offset, count, age, checks.load(),
				repairs.load(), bytes, failures.load());
		}
		return true;
	};

	const u64 cached = cached_sequence.load();
	if (cached && cached <= cursor && cursor - cached < event_count &&
		consume_snapshot(cached, cursor - cached))
	{
		return;
	}
	...
	{
		const u64 event_sequence = cursor - age;
		if (event_sequence == cached)
		{
			continue;
		}

		if (consume_snapshot(event_sequence, age))
		{
			cached_sequence.release(event_sequence);
			return;
		}
	}
}
...
bool ppu_initialize(const ppu_module<lv2_obj>& info, bool check_only, u64 file_size, concurent_memory_limit& memory_limit)
{
	...
	static const std::unordered_map<std::string, u64> s_link_table = []()
	{
		std::unordered_map<std::string, u64> link_table{
			...
			{"__thor_es_template_probe", reinterpret_cast<u64>(ppu_thor_es_template_probe)},
	...
	while (!jit_mod.init && fpos < info.get_funcs().size())
	{
		...
		std::string obj_name;
		{
			...
			enum class ppu_settings : u32
			{
				...
				thor_es_dispatch_provenance_v3,
				thor_es_async_draw_barrier_v7,
				thor_es_dispatch_provenance_v4,
				thor_es_dispatch_provenance_v5,

				bitset_last = thor_es_dispatch_provenance_v5,
			};
			...
			if (has_thor_es_async_draw_barrier)
				settings += ppu_settings::thor_es_async_draw_barrier_v7;
			if (fpos >= info.get_funcs().size() || module_counter % c_moudles_per_jit == c_moudles_per_jit - 1)
