...
#error "signal_handler not implemented"
	...
	{
		fmt::append(msg, "PPU CIA: 0x%08x.\n", ppu->cia);
	}
	for (u32 i = 0; i < 31; i++)
	{
		fmt::append(msg, "X%u=%016llx%c", i, context->uc_mcontext.regs[i], i % 4 == 3 ? '\n' : ' ');
	}
	fmt::append(msg, "SP=%016llx\n", context->uc_mcontext.sp);
#endif
...
#ifndef _WIN32
	// Publish the native handle atomically. The new thread may initialize before
	// pthread_create returns, so either side is allowed to win this race.
	u64 dest_id = 0;
...
[[noreturn]] void thread_ctrl::emergency_exit(std::string_view reason)
{
	...
	if (auto [total, current] = utils::get_memory_usage(); total > current && total - current <= 256 * 1024 * 1024)
	{
		if (reason_buf.empty())
		{
		...
		fmt::append(reason_buf, " (Possible RAM shortage: free RAM: %lluMB)",
			(total - current) / (1024 * 1024));
	}
