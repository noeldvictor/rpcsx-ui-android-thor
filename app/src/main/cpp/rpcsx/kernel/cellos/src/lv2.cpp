...
static void null_func_(ppu_thread &ppu, ppu_opcode_t, be_t<u32> *this_op,
                       ppu_intrp_func *) {
  ppu_log.todo(
      "Unimplemented syscall %s -> CELL_OK (r3=0x%llx, r4=0x%llx, r5=0x%llx, "
      "r6=0x%llx, r7=0x%llx, r8=0x%llx, r9=0x%llx, r10=0x%llx)",
      ppu_syscall_code(ppu.gpr[11]), ppu.gpr[3], ppu.gpr[4], ppu.gpr[5],
      ppu.gpr[6], ppu.gpr[7], ppu.gpr[8], ppu.gpr[9], ppu.gpr[10]);

  ppu.gpr[3] = 0;
  ppu.cia = vm::get_addr(this_op) + 4;
}

static void uns_func_(ppu_thread &ppu, ppu_opcode_t, be_t<u32> *this_op,
                      ppu_intrp_func *) {
...
const std::array<std::pair<ppu_intrp_func_t, std::string_view>, 1024>
    g_ppu_syscall_table{
        null_func,
        ...
        BIND_SYSC(
            sys_process_is_spu_lock_line_reservation_address), // 14  (0x00E)
        ...
        null_func, // BIND_SYSC(sys_ppu_...),                     //54  (0x036)
                   // ROOT
        null_func, // BIND_SYSC(sys_ppu_...),                     //55  (0x037)
                   // ROOT
        ...
        uns_func, // BIND_SYSC(sys_dbg_...),                      //217 (0x0D9)
                  // DBG  UNS?
        uns_func, // BIND_SYSC(sys_dbg_...),                      //218 (0x0DA)
                  // DBG  UNS?
        uns_func, // BIND_SYSC(sys_dbg_...),                      //219 (0x0DB)
                  // DBG  UNS?
        ...
        uns_func,
        uns_func,
        uns_func, // 241-243 ROOT  UNS
        NULL_FUNC(
            sys_spu_thread_group_system_set_next_group), // 244 (0x0F4)  ROOT
        NULL_FUNC(
            sys_spu_thread_group_system_unset_next_group), // 245 (0x0F5)  ROOT
        NULL_FUNC(
            sys_spu_thread_group_system_set_switch_group), // 246 (0x0F6)  ROOT
        NULL_FUNC(
            sys_spu_thread_group_system_unset_switch_group), // 247 (0x0F7) ROOT
        null_func, // BIND_SYSC(sys_spu_thread_group...),         //248 (0x0F8)
                   // ROOT
        null_func, // BIND_SYSC(sys_spu_thread_group...),         //249 (0x0F9)
                   // ROOT
        BIND_SYSC(sys_spu_thread_group_set_cooperative_victims), // 250 (0x0FA)
        BIND_SYSC(
            sys_spu_thread_group_connect_event_all_threads), // 251 (0x0FB)
        BIND_SYSC(
            sys_spu_thread_group_disconnect_event_all_threads), // 252 (0x0FC)
        ...
        BIND_SYSC(
            sys_mmapper_allocate_shared_memory_from_container_ext), // 328
                                                                    // (0x148)
        ...
        BIND_SYSC(
            sys_memory_container_destroy_parent_with_childs), // 345 (0x159)
        ...
        null_func, // BIND_SYSC(sys_sm_...),                      //400 (0x190)
                   // PM
        null_func, // BIND_SYSC(sys_sm_...),                      //401 (0x191)
                   // ROOT
        BIND_SYSC(sys_tty_read),  // 402 (0x192)
        BIND_SYSC(sys_tty_write), // 403 (0x193)
        null_func, // BIND_SYSC(sys_...),                         //404 (0x194)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //405 (0x195)
                   // PM
        null_func, // BIND_SYSC(sys_...),                         //406 (0x196)
                   // PM
        null_func, // BIND_SYSC(sys_...),                         //407 (0x197)
                   // PM
        ...
        null_func, // BIND_SYSC(sys_...),                         //413 (0x19D)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //414 (0x19E)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //415 (0x19F)
                   // ROOT

        uns_func,
        uns_func,
        uns_func,
        uns_func, // 416-419  UNS
        uns_func,
        uns_func,
        uns_func,
        uns_func,
        ...
        NULL_FUNC(
            sys_hid_manager_map_logical_id_to_port_id), // 504 (0x1F8)  ROOT
        NULL_FUNC(
            sys_hid_manager_unmap_logical_id_to_port_id),   // 505 (0x1F9)  ROOT
        ...
        BIND_SYSC(
            sys_hid_manager_is_process_permission_root),    // 512 (0x200)  ROOT
        ...
        null_func, // BIND_SYSC(sys_...),                         //560 (0x230)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //561 (0x231)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //562 (0x232)
                   // ROOT
        ...
        null_func, // BIND_SYSC(sys_pad_manager_...),             //577 (0x241)
                   // ROOT  PM
        null_func, // BIND_SYSC(sys_bluetooth_...),               //578 (0x242)
        null_func, // BIND_SYSC(sys_bluetooth_aud_serial_...),    //579 (0x243)
        null_func, // BIND_SYSC(sys_bluetooth_...),               //580 (0x244)
                   // ROOT
        null_func, // BIND_SYSC(sys_bluetooth_...),               //581 (0x245)
                   // ROOT
        null_func, // BIND_SYSC(sys_bluetooth_...),               //582 (0x246)
                   // ROOT
        ...
        null_func, // BIND_SYSC(sys_bluetooth_...),               //598 (0x256)
                   // ROOT
        null_func, // BIND_SYSC(sys_bluetooth_...),               //599 (0x257)
                   // ROOT
        ...
        null_func, // BIND_SYSC(sys_...),                         //620 (0x26C)
                   // ROOT
        ...
        null_func, // BIND_SYSC(sys_...),                         //648
                   // DEPRECATED
        null_func, // BIND_SYSC(sys_...),                         //649
                   // DEPRECATED
        ...
        null_func, // BIND_SYSC(sys_...),                         //679 (0x2A7)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //680 (0x2A8)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //681 (0x2A9)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //682 (0x2AA)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //683 (0x2AB)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //684 (0x2AC)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //685 (0x2AD)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //686 (0x2AE)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //687 (0x2AF)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //688 (0x2B0)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //689 (0x2B1)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //690 (0x2B2)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //691 (0x2B3)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //692 (0x2B4)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //693 (0x2B5)
                   // ROOT
        null_func, // BIND_SYSC(sys_...),                         //694 (0x2B6)
                   // DEPRECATED
        null_func, // BIND_SYSC(sys_...),                         //695 (0x2B7)
                   // DEPRECATED
        null_func, // BIND_SYSC(sys_...),                         //696 (0x2B8)
                   // ROOT
        uns_func,  // BIND_SYSC(sys_...),                          //697 (0x2B9)
                   // UNS
        uns_func,  // BIND_SYSC(sys_...),                          //698 (0x2BA)
                   // UNS
        ...
        uns_func,
        uns_func,
        uns_func, // 848-853  UNS
        uns_func,
        uns_func,
        uns_func,
        uns_func,
        uns_func,
        uns_func, // 854-859  UNS
        ...
        null_func, // BIND_SYSC(sys_ss_~utoken_if),              //877 (0x36D)
                   // ROOT
        ...
        uns_func,
        uns_func,
        uns_func,  // 886-888  UNS
        null_func, // BIND_SYSC(sys_...),                         //889 (0x379)
                   // ROOT
        ...
        NULL_FUNC(
            sys_dbg_enable_floating_point_enabled_exception), // 955 (0x3BB)
        NULL_FUNC(
            sys_dbg_disable_floating_point_enabled_exception), // 956 (0x3BC)
        NULL_FUNC(
            sys_dbg_get_process_memory_container_information), // 957 (0x3BD)
        ...
        null_func, // BIND_SYSC(sys_dbg_...),                     //986 (0x3DA)
                   // ROOT  DBG
        null_func, // BIND_SYSC(sys_dbg_...),                     //987 (0x3DB)
                   // ROOT
...
constexpr auto formatter_of =
    std::make_pair(EnumMin, &fmt_class_string<E>::format);

const std::map<u64, void (*)(std::string &, u64)>
    s_error_codes_formatting_by_type{
...
stx::init_lock acquire_lock(stx::init_mutex &mtx, ppu_thread *ppu) {
  ...
  return mtx.init(
      [](int invoke_count, const stx::init_lock &, ppu_thread *ppu) {
        if (!invoke_count) {
...
public:
  ...
  void print_stats(bool force_print) noexcept {
    ...
    for (u32 i = 0; i < 1024; i++) {
      if (u64 v = stat[i]; m_old_stat[i] != v || (force_print && v)) {
        // Only add syscalls with non-zero usage counter and only if caught new
        // calls since last print
        usage.emplace(v, i);
        m_old_stat[i] = v;
      }
    }
    ...
    for (auto &&pair : usage) {
      fmt::append(m_stats, u8"\n\t⁂ %s [%u]", ppu_get_syscall_name(pair.second),
                  pair.first);
    }
  ...
  ~ppu_syscall_usage() { print_stats(true); }
...
std::string ppu_get_syscall_name(u64 code) {
  if (code < g_ppu_syscall_table.size() &&
      !g_ppu_syscall_table[code].second.empty()) {
...
bool lv2_obj::sleep_unlocked(cpu_thread &thread, u64 timeout,
                             u64 current_time) {
  ...
  auto on_to_sleep_update = [&]() {
    if (g_to_sleep.size() > 5u) {
      ...
      // Print-out their IDs for further inspection (focus at 5 at max for now
      // to avoid log spam)
  ...
  if (auto ppu = thread.try_get<ppu_thread>()) {
    ...
    if (std::exchange(ppu->cancel_sleep, 0) == 2) {
      // Signal that the underlying LV2 operation has been cancelled and
      // replaced with a short yield
      return_val = false;
    }
    ...
    if (!unqueue(g_ppu, ppu, &ppu_thread::next_ppu)) {
      if (auto it = std::find(g_to_sleep.begin(), g_to_sleep.end(), ppu);
          it != g_to_sleep.end()) {
        g_to_sleep.erase(it);
    ...
    ppu->start_time = start_time;
    ppu->end_time =
        timeout ? start_time + std::min<u64>(timeout, ~start_time) : u64{umax};
  } else if (auto spu = thread.try_get<spu_thread>()) {
    if (auto it = std::find(g_to_sleep.begin(), g_to_sleep.end(), spu);
        it != g_to_sleep.end()) {
      g_to_sleep.erase(it);
      on_to_sleep_update();
      return true;
...
bool lv2_obj::awake_unlocked(cpu_thread *cpu, s32 prio) {
  ...
  default: {
    // Priority set
    auto set_prio = [](atomic_t<ppu_thread::ppu_prio_t> &prio, s32 value,
                       bool increment_order_last, bool increment_order_first) {
      s64 tag = 0;
    ...
    // If priority is the same, push ONPROC/RUNNABLE thread to the back of the
    // priority list if it is not the current thread
    if (old_prio == prio && cpu == cpu_thread::get_current()) {
      set_prio(static_cast<ppu_thread *>(cpu)->prio, prio, false, false);
      return true;
    }

    if (!unqueue(g_ppu, static_cast<ppu_thread *>(cpu),
                 &ppu_thread::next_ppu)) {
      set_prio(static_cast<ppu_thread *>(cpu)->prio, prio, old_prio > prio,
               old_prio < prio);
      return true;
    }
  ...
  case yield_cmd: {
    ...
    for (auto ppu_next = &g_ppu;; i++) {
      ...
      if (ppu == cpu) {
        ...
        // Rotate current thread to the last position of the 'same prio' threads
        // list Exchange forward pointers
        *ppu_next =
  ...
  const auto emplace_thread = [push_first](cpu_thread *const cpu) {
    for (auto it = &g_ppu;;) {
      ...
      if (!next ||
          (push_first ? next->prio.load().prio >=
                            static_cast<ppu_thread *>(cpu)->prio.load().prio
                      : next->prio.load().prio >
                            static_cast<ppu_thread *>(cpu)->prio.load().prio)) {
        atomic_storage<ppu_thread *>::release(
            static_cast<ppu_thread *>(cpu)->next_ppu, next);
        atomic_storage<ppu_thread *>::release(*it,
                                              static_cast<ppu_thread *>(cpu));
        break;
      }
    ...
    for (auto it = g_waiting.cbegin(), end = g_waiting.cend(); it != end;
         it++) {
      if (it->second == cpu) {
        g_waiting.erase(it);
        break;
  ...
  if (cpu && prio != yield_cmd) {
    ...
    if (emplace_thread(cpu)) {
      changed_queue = true;
      lowest_new_priority =
          std::min<s32>(static_cast<ppu_thread *>(cpu)->prio.load().prio,
                        lowest_new_priority);
    }
  } else
    for (const auto _cpu : g_to_awake) {
      ...
      if (emplace_thread(_cpu)) {
        changed_queue = true;
        lowest_new_priority =
            std::min<s32>(static_cast<ppu_thread *>(_cpu)->prio.load().prio,
                          lowest_new_priority);
      }
    }
  ...
  // In real PS3 (it seems), when a thread with a higher priority than the
  // caller is signaled and -
  // - that there is available space on the running queue for the other hardware
  // thread to start It prioritizes signaled thread - caller's hardware thread
  // switches instantly to the new thread code While signaling to the other
  // hardware thread to execute the caller's code. Resulting in a delay to the
  // caller after such thread is signaled
    ...
    if (current_ppu->prio.load().prio > lowest_new_priority) {
      ...
      if (is_create_thread || g_cfg.core.sleep_timers_accuracy ==
        ...
        if (!current_ppu->state.test_and_set(cpu_flag::yield) ||
          ...
          current_ppu->hw_sleep_time =
              30000; // In addition to another flag's use (TODO: Refactor and
                     // clean this)
        }
      }
    }
...
void lv2_obj::schedule_all(u64 current_time) {
  auto it = std::find(g_to_notify, std::end(g_to_notify),
                      std::add_pointer_t<const void>{});
  ...
  if (!g_pending && g_scheduler_ready) {
    ...
    for (usz x = g_cfg.core.ppu_threads; target && x;
      ...
      if (target->state & cpu_flag::suspend) {
        ...
        if ((target->state.fetch_op(AOFN(x += cpu_flag::signal,
                                         x -= cpu_flag::suspend,
                                         x -= remove_yield, void())) &
             (cpu_flag::wait + cpu_flag::signal)) != cpu_flag::wait) {
          continue;
        }

        if (it == std::end(g_to_notify)) {
          // Out of notification slots, notify locally (resizable container is
          // not worth it)
          target->state.notify_one();
        } else {
          *it++ = &target->state;
    ...
    if (tsc >= last_tsc && tsc <= s_max_allowed_yield_tsc &&
      ...
      for (usz x = g_cfg.core.ppu_threads;; target = target->next_ppu, x--) {
        if (!target || !x) {
          ...
          // TODO: If this case is common enough it may be valuable to iterate
          // over all CPU threads to find a perfect candidate (one without a
          // wait or suspend flag)
          else if (auto current = cpu_thread::get_current();
                   current && cpu_flag::suspend - current->state) {
      ...
      if (cpu && cpu_flag::preempt - cpu->state &&
          !cpu->state.test_and_set(cpu_flag::preempt)) {
        s_last_yield_tsc = tsc;
        g_lv2_preempts_taken.release(
            g_lv2_preempts_taken.load() +
            1); // Has a minor race but performance is more important
...
std::pair<ppu_thread_status, u32>
lv2_obj::ppu_state(ppu_thread *ppu, bool lock_idm, bool lock_lv2) {
  ...
  if (!Emu.IsReady() ? ppu->state.all_of(cpu_flag::stop)
                     : ppu->stop_flag_removal_protection) {
    return {PPU_THREAD_STATUS_IDLE, 0};
  }

  switch (ppu->joiner) {
  case ppu_join_status::zombie:
    return {PPU_THREAD_STATUS_ZOMBIE, 0};
  case ppu_join_status::exited:
    return {PPU_THREAD_STATUS_DELETED, 0};
  default:
    break;
  }
...
// Cstate is represented in bits [7:4]+1 cstate. So C0 requires bits [7:4] to be
// set to 0xf, C1 requires bits [7:4] to be set to 0.
mwaitx_func static void __mwaitx(u32 cycles, u32 cstate) {
  constexpr u32 timer_enable = 0x2;

  // monitorx will wake if the cache line is written to. We don't want this, so
  // place the monitor value on it's own cache line.
...
bool lv2_obj::wait_timeout(u64 usec, ppu_thread *cpu, bool scale,
                           bool is_usleep) {
  static_assert(u64{umax} / max_timeout >= 100,
                "max timeout is not valid for scaling");
  ...
  for (;; old_state = state) {
    ...
    if (g_cfg.core.sleep_timers_accuracy <
      ...
      if (remaining > host_min_quantum) {
        ...
        wait_for(remaining);
#else
        // Wait on multiple of min quantum for large durations to avoid
        // overloading low thread cpus
      ...
      // TODO: Determine best value for yield delay
        ...
        const u32 us_in_tsc_clocks =
            ::narrow<u32>(remaining * (utils::get_tsc_freq() / 1000000ULL));
...
void lv2_obj::notify_all() noexcept {
  ...
  if (cpu->get_class() != thread_class::spu &&
      cpu->state.none_of(cpu_flag::suspend)) {
    return;
  }
  ...
  for (usz i = 0, checked = 0; checked < 4 && i < total_waiters; i++) {
    ...
    if (vm::check_addr(raddr)) {
      if (((raddr >> 28) < 2 || (raddr >> 28) == 0xd)) {
        ...
        if (compute_rdata_hash32(*vm::get_super_ptr<spu_rdata_t>(raddr)) !=
      ...
      if (spu_thread::reservation_check(raddr, static_cast<u32>(value >> 32),
                                        range_lock)) {
        // Clear address to avoid a race, keep waiter counter
        if (waiter
                .fetch_op([&](u64 &x) {
