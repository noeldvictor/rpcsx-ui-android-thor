...
{
	closed = 0,    // Also called non-existent. Needs to be opened before anything
	               // can be done with it.
	dormant = 1,   // Waiting for the next sequence. The last picture and pic-item
	               // can be aqcuired in this state.
	ready = 2,     // Ready for decoding. Can also restart sequences in this state.
	ending = 3,    // Ending a sequence. Goes to dormant afterwards.
	resetting = 4, // Stops the current sequence and starts a new one. The
	               // pictures of the old sequence are flushed
...
vm::gvar<s32>
	_cell_vdec_prx_ver; // TODO: this should probably specify the VDEC module
                        // that was loaded. E.g. CELL_SYSMODULE_VDEC_MPEG2
...
struct vdec_cmd
{
	...
	explicit vdec_cmd(vdec_cmd_type _type, u64 _seq_id, u64 _id, s32 _mode,
		CellVdecAuInfo _au)
...
struct vdec_context final
{
	...
	vdec_context(s32 type, u32 /*profile*/, u32 addr, u32 size,
		vm::ptr<CellVdecCbMsg> func, u32 arg)
		: type(type), mem_addr(addr), mem_size(size), cb_func(func), cb_arg(arg)
	{
		switch (type)
		{
		...
		if (err || opts)
		{
			...
			fmt::throw_exception("avcodec_open2() failed (err=0x%x='%s', opts=%s)",
				err, utils::av_error_to_string(err), dict_content);
		}
	...
	void exec(ppu_thread& ppu, u32 vid)
	{
		...
		for (auto slice = in_cmd.pop_all();
			thread_ctrl::state() != thread_state::aborting; [&]
			{
				if (slice)
				{
					slice.pop_front();
			...
			}())
		{
			...
			case vdec_cmd_type::start_sequence:
			{
				...
				{
					cellVdec.trace(
						"Reset sequence... (handle=0x%x, seq_id=%d, cmd_id=%d)", handle,
						cmd->seq_id, cmd->id);
				}
				else
				{
					cellVdec.trace(
						"Start sequence... (handle=0x%x, seq_id=%d, cmd_id=%d)", handle,
						cmd->seq_id, cmd->id);
				}
			...
			{
				cellVdec.trace("End sequence... (handle=0x%x, seq_id=%d, cmd_id=%d)",
					handle, cmd->seq_id, cmd->id);
				...
				cellVdec.trace("Sending CELL_VDEC_MSG_TYPE_SEQDONE (handle=0x%x, "
							   "seq_id=%d, cmd_id=%d)",
					handle, cmd->seq_id, cmd->id);
			...
			case vdec_cmd_type::au_decode:
			{
				...
				ctx->skip_frame =
					au_mode == CELL_VDEC_DEC_MODE_NORMAL ? AVDISCARD_DEFAULT : au_mode == CELL_VDEC_DEC_MODE_B_SKIP ? AVDISCARD_NONREF :
																													  AVDISCARD_NONINTRA;
				...
				{
					cellVdec.trace("AU decoding: handle=0x%x, seq_id=%d, cmd_id=%d, "
								   "size=0x%x, pts=0x%llx, dts=0x%llx, userdata=0x%llx",
						handle, cmd->seq_id, cmd->id, au_size, au_pts, au_dts,
						au_usrd);

					if (int ret = avcodec_send_packet(ctx, &packet); ret < 0)
					{
						fmt::throw_exception("AU queuing error (handle=0x%x, seq_id=%d, "
											 "cmd_id=%d, error=0x%x): %s",
							handle, cmd->seq_id, cmd->id, ret,
							utils::av_error_to_string(ret));
					}
					...
					while (!abort_decode && seq_id == cmd->seq_id)
					{
						...
						{
							fmt::throw_exception(
								"av_frame_alloc() failed (handle=0x%x, seq_id=%d, cmd_id=%d)",
								handle, cmd->seq_id, cmd->id);
						}

						if (int ret = avcodec_receive_frame(ctx, frame.avf.get());
							ret < 0)
						{
							...
							fmt::throw_exception("AU decoding error (handle=0x%x, seq_id=%d, "
												 "cmd_id=%d, error=0x%x): %s",
								handle, cmd->seq_id, cmd->id, ret,
								utils::av_error_to_string(ret));
						}
						...
						const int ticks_per_frame =
							(codec_desc->props & AV_CODEC_PROP_FIELDS) ? 2 : 1;
#endif
						...
						{
							// NPEB01838, NPUB31260
							cellVdec.todo("Interlaced frames not supported (handle=0x%x, "
										  "seq_id=%d, cmd_id=%d)",
								handle, cmd->seq_id, cmd->id);
						}
						...
						{
							fmt::throw_exception(
								"Repeated frames not supported (handle=0x%x, seq_id=%d, "
								"cmd_id=%d, repear_pict=0x%x)",
								handle, cmd->seq_id, cmd->id, frame->repeat_pict);
						}
						...
						if (frc_set)
						{
							...
							switch (frc_set)
							{
							case CELL_VDEC_FRC_24000DIV1001:
								amend = 1001 * 90000 / 24000;
								break;
							case CELL_VDEC_FRC_24:
								amend = 90000 / 24;
								break;
							case CELL_VDEC_FRC_25:
								amend = 90000 / 25;
								break;
							case CELL_VDEC_FRC_30000DIV1001:
								amend = 1001 * 90000 / 30000;
								break;
							case CELL_VDEC_FRC_30:
								amend = 90000 / 30;
								break;
							case CELL_VDEC_FRC_50:
								amend = 90000 / 50;
								break;
							case CELL_VDEC_FRC_60000DIV1001:
								amend = 1001 * 90000 / 60000;
								break;
							...
							{
								fmt::throw_exception(
									"Invalid frame rate code set (handle=0x%x, seq_id=%d, "
									"cmd_id=%d, frc=0x%x)",
									handle, cmd->seq_id, cmd->id, frc_set);
							}
							}
						...
						{
							if (log_time_base.den != ctx->time_base.den ||
								log_time_base.num != ctx->time_base.num)
							{
								cellVdec.error("time_base.num is 0 (handle=0x%x, seq_id=%d, "
											   "cmd_id=%d, %d/%d, tpf=%d framerate=%d/%d)",
									handle, cmd->seq_id, cmd->id, ctx->time_base.num,
									ctx->time_base.den, ticks_per_frame,
						...
						{
							u64 amend = u64{90000} * ctx->time_base.num * ticks_per_frame /
							            ctx->time_base.den;
							const auto freq = 1. * ctx->time_base.den / ctx->time_base.num /
							                  ticks_per_frame;
							...
							else
							{
								if (log_time_base.den != ctx->time_base.den ||
									log_time_base.num != ctx->time_base.num)
								{
									// 1/1000 usually means that the time stamps are written in
									// 1ms units and that the frame rate may vary.
									cellVdec.error(
										"Unsupported time_base (handle=0x%x, seq_id=%d, "
										"cmd_id=%d, %d/%d, tpf=%d framerate=%d/%d)",
										handle, cmd->seq_id, cmd->id, ctx->time_base.num,
										ctx->time_base.den, ticks_per_frame, ctx->framerate.num,
										ctx->framerate.den);
									log_time_base = ctx->time_base;
								}
						...
						cellVdec.trace("Got picture (handle=0x%x, seq_id=%d, cmd_id=%d, "
									   "pts=0x%llx[0x%llx], dts=0x%llx[0x%llx])",
							handle, cmd->seq_id, cmd->id, frame.pts, frame->pts,
				...
				{
					// Send AUDONE even if the current sequence was reset and a new
					// sequence was started.
					cellVdec.trace("Sending CELL_VDEC_MSG_TYPE_AUDONE (handle=0x%x, "
								   "seq_id=%d, cmd_id=%d)",
						handle, cmd->seq_id, cmd->id);
					...
					while (!decoded_frames.empty() && seq_id == cmd->seq_id)
					{
						...
						// Do this after pushing the frame to the queue. That way the game
						// can consume the frame and we can move on.
						u32 elapsed = 0;
						while (thread_ctrl::state() != thread_state::aborting &&
							   !abort_decode && seq_id == cmd->seq_id)
						{
							{
								std::lock_guard lock{mutex};
							...
							{
								cellVdec.error("Video au decode has been waiting for a "
											   "consumer for 5 seconds. (handle=0x%x, "
											   "seq_id=%d, cmd_id=%d, queue_size=%d)",
									handle, cmd->seq_id, cmd->id, out_queue.size());
								elapsed = 0;
							}
						}
						...
						cellVdec.trace("Sending CELL_VDEC_MSG_TYPE_PICOUT (handle=0x%x, "
									   "seq_id=%d, cmd_id=%d)",
							handle, cmd->seq_id, cmd->id);
				...
				if (abort_decode || seq_id != cmd->seq_id)
				{
					cellVdec.warning("AU decoding: aborted (handle=0x%x, seq_id=%d, "
									 "cmd_id=%d, abort_decode=%d)",
						handle, cmd->seq_id, cmd->id, abort_decode.load());
				}
				else
				{
					cellVdec.trace(
						"AU decoding: done (handle=0x%x, seq_id=%d, cmd_id=%d)", handle,
						cmd->seq_id, cmd->id);
				}
			...
			default:
				fmt::throw_exception("Unknown vdec_cmd_type (handle=0x%x, seq_id=%d, "
									 "cmd_id=%d, type=%d)",
					handle, cmd->seq_id, cmd->id,
					static_cast<u32>(cmd->type));
				break;
			}
...
static error_code vdecQueryAttr(s32 type, u32 profile,
	u32 spec_addr /* may be 0 */,
	CellVdecAttr* attr)
{
	// Write 0 at start
	attr->memSize = 0;
	...
	case CELL_VDEC_CODEC_TYPE_AVC:
	{
		...
		case CELL_VDEC_AVC_LEVEL_1P0:
			memSize = new_sdk ? 0x70167D : 0xA014FD;
			break;
		case CELL_VDEC_AVC_LEVEL_1P1:
			memSize = new_sdk ? 0x86CB7D : 0xB6C9FD;
			break;
		case CELL_VDEC_AVC_LEVEL_1P2:
			memSize = new_sdk ? 0x9E307D : 0xCE2D7D;
			break;
		case CELL_VDEC_AVC_LEVEL_1P3:
			memSize = new_sdk ? 0xA057FD : 0xD054FD;
			break;
		case CELL_VDEC_AVC_LEVEL_2P0:
			memSize = new_sdk ? 0xA057FD : 0xD054FD;
			break;
		case CELL_VDEC_AVC_LEVEL_2P1:
			memSize = new_sdk ? 0xE90DFD : 0x1190AFD;
			break;
		case CELL_VDEC_AVC_LEVEL_2P2:
			memSize = new_sdk ? 0x14E49FD : 0x17E46FD;
			break;
		case CELL_VDEC_AVC_LEVEL_3P0:
			memSize = new_sdk ? 0x155B5FD : 0x185B17D;
			break;
		case CELL_VDEC_AVC_LEVEL_3P1:
			memSize = new_sdk ? 0x1CD327D : 0x1FD2AFD;
			break;
		case CELL_VDEC_AVC_LEVEL_3P2:
			memSize = new_sdk ? 0x2397B7D : 0x2696F7D;
			break;
		case CELL_VDEC_AVC_LEVEL_4P0:
			memSize = new_sdk ? 0x33A5FFD : 0x36A527D;
			break;
	...
	case CELL_VDEC_CODEC_TYPE_DIVX:
	{
		...
		{
			if (profile != CELL_VDEC_DIVX_QMOBILE &&
				profile != CELL_VDEC_DIVX_MOBILE)
			{
				return CELL_VDEC_ERROR_ARG;
			}
		}
		...
		switch (profile)
		{
		case CELL_VDEC_DIVX_QMOBILE:
			memSize = new_sdk ? 0x11B720 : 0x1DEF30;
			break;
		case CELL_VDEC_DIVX_MOBILE:
			memSize = new_sdk ? 0x19A740 : 0x26DED0;
			break;
		case CELL_VDEC_DIVX_HOME_THEATER:
			memSize = new_sdk ? 0x386A60 : 0x498060;
			break;
		case CELL_VDEC_DIVX_HD_720:
			memSize = new_sdk ? 0x692070 : 0x805690;
			break;
		case CELL_VDEC_DIVX_HD_1080:
			memSize = new_sdk ? 0xD78100 : 0xFC9870;
			break;
		default:
			return CELL_VDEC_ERROR_ARG;
		}
...
error_code cellVdecQueryAttr(vm::cptr<CellVdecType> type,
	vm::ptr<CellVdecAttr> attr)
{
...
error_code cellVdecQueryAttrEx(vm::cptr<CellVdecTypeEx> type,
	vm::ptr<CellVdecAttr> attr)
{
	...
	return vdecQueryAttr(type->codecType, type->profileLevel,
		type->codecSpecificInfo_addr, attr.get_ptr());
}

template <typename T, typename U>
static error_code vdecOpen(ppu_thread& ppu, T type, U res,
	vm::cptr<CellVdecCb> cb, vm::ptr<u32> handle)
{
	...
	if (!res->memAddr || res->ppuThreadPriority + 0u >= 3072 ||
		res->spuThreadPriority + 0u >= 256 || res->ppuThreadStackSize < 4096 ||
	...
	if constexpr (std::is_same_v<std::decay_t<typename T::type>,
					  CellVdecTypeEx>)
	{
		spec_addr = type->codecSpecificInfo_addr;
	}

	if (CellVdecAttr attr{}; vdecQueryAttr(type->codecType, type->profileLevel,
								 spec_addr, &attr) != CELL_OK ||
	...
	{
		vdec = idm::make_ptr<vdec_context>(type->codecType, type->profileLevel,
			res->memAddr, res->memSize, cb->cbFunc,
			cb->cbArg);
	}
	else
	{
	...
	ppu_execute<&sys_ppu_thread_create>(
		ppu, +_tid, 0x10000, vid, +res->ppuThreadPriority,
		+res->ppuThreadStackSize, SYS_PPU_THREAD_CREATE_INTERRUPT, +_name);
	*handle = vid;

	const auto thrd =
		idm::get_unlocked<named_thread<ppu_thread>>(static_cast<u32>(*_tid));
...
error_code cellVdecOpen(ppu_thread& ppu, vm::cptr<CellVdecType> type,
	vm::cptr<CellVdecResource> res, vm::cptr<CellVdecCb> cb,
	vm::ptr<u32> handle)
{
	cellVdec.warning(
		"cellVdecOpen(type=*0x%x, res=*0x%x, cb=*0x%x, handle=*0x%x)", type, res,
		cb, handle);

	return vdecOpen(ppu, type, res, cb, handle);
}

error_code cellVdecOpenEx(ppu_thread& ppu, vm::cptr<CellVdecTypeEx> type,
	vm::cptr<CellVdecResourceEx> res,
	vm::cptr<CellVdecCb> cb, vm::ptr<u32> handle)
{
	cellVdec.warning(
		"cellVdecOpenEx(type=*0x%x, res=*0x%x, cb=*0x%x, handle=*0x%x)", type,
		res, cb, handle);
...
error_code cellVdecClose(ppu_thread& ppu, u32 handle)
{
	...
	cellVdec.trace("Adding close cmd (handle=0x%x, seq_id=%d, cmd_id=%d)", handle,
		seq_id, cmd_id);
...
error_code cellVdecStartSeq(ppu_thread& ppu, u32 handle)
{
...

	{
		...
		if (old_state != sequence_state::dormant &&
			old_state != sequence_state::ready)
		{
	...
	const u64 seq_id = ++vdec->seq_id;
	const u64 cmd_id = vdec->next_cmd_id++;
	cellVdec.trace("Adding start cmd (handle=0x%x, seq_id=%d, cmd_id=%d)", handle,
		seq_id, cmd_id);
...
error_code cellVdecEndSeq(ppu_thread& ppu, u32 handle)
{
	...
	const u64 seq_id = vdec->seq_id;
	const u64 cmd_id = vdec->next_cmd_id++;
	cellVdec.trace("Adding end cmd (handle=0x%x, seq_id=%d, cmd_id=%d)", handle,
		seq_id, cmd_id);
...
error_code cellVdecDecodeAu(ppu_thread& ppu, u32 handle,
	CellVdecDecodeMode mode,
	vm::cptr<CellVdecAuInfo> auInfo)
{
	ppu.state += cpu_flag::wait;
	...
	{
		return {CELL_VDEC_ERROR_ARG,
			"vdec=%d, auInfo=%d, size=%d, startAddr=0x%x",
			!!vdec,
			!!auInfo,
			auInfo ? auInfo->size.value() : 0,
			auInfo ? auInfo->startAddr.value() : 0};
	}
	...
	if (mode < 0 ||
		mode > (CELL_VDEC_DEC_MODE_B_SKIP | CELL_VDEC_DEC_MODE_PB_SKIP))
	{
		return {CELL_VDEC_ERROR_ARG, "mode=%d", +mode};
	}

	// TODO: what does the 3 stand for ?
	if ((mode == (CELL_VDEC_DEC_MODE_B_SKIP | CELL_VDEC_DEC_MODE_PB_SKIP) &&
			vdec->type != 3) ||
		(mode == CELL_VDEC_DEC_MODE_PB_SKIP &&
			vdec->type != CELL_VDEC_CODEC_TYPE_AVC))
	{
	...
	const u64 seq_id = vdec->seq_id;
	const u64 cmd_id = vdec->next_cmd_id++;
	cellVdec.trace("Adding decode cmd (handle=0x%x, seq_id=%d, cmd_id=%d)",
		handle, seq_id, cmd_id);

	// TODO: check info
	vdec->in_cmd.push(
		vdec_cmd(vdec_cmd_type::au_decode, seq_id, cmd_id, mode, *auInfo));
	return CELL_OK;
}

error_code cellVdecDecodeAuEx2(ppu_thread& ppu, u32 handle,
	CellVdecDecodeMode mode,
	vm::cptr<CellVdecAuInfoEx2> auInfo)
{
	ppu.state += cpu_flag::wait;

	cellVdec.todo("cellVdecDecodeAuEx2(handle=0x%x, mode=%d, auInfo=*0x%x)",
		handle, +mode, auInfo);
	...
	// TODO: check info
	vdec->in_cmd.push(
		vdec_cmd(vdec_cmd_type::au_decode, seq_id, cmd_id, mode, au_info));
	return CELL_OK;
}

error_code cellVdecGetPictureExt(ppu_thread& ppu, u32 handle,
	vm::cptr<CellVdecPicFormat2> format,
	vm::ptr<u8> outBuff, u32 arg4)
{
	ppu.state += cpu_flag::wait;

	cellVdec.trace("cellVdecGetPictureExt(handle=0x%x, format=*0x%x, "
				   "outBuff=*0x%x, arg4=*0x%x)",
		handle, format, outBuff, arg4);
	...
	if (format->formatType > 4 ||
		(format->formatType <= CELL_VDEC_PICFMT_RGBA32_ILV &&
	...
	{
		return {CELL_VDEC_ERROR_EMPTY, "sequence_id=%d, seq_id=%d", sequence_id,
			frame.seq_id};
	}
	...
	if (outBuff)
	{
		...
		case CELL_VDEC_PICFMT_ARGB32_ILV:
			out_f = AV_PIX_FMT_ARGB;
			alpha_plane.reset(new u8[w * h]);
			break;
		case CELL_VDEC_PICFMT_RGBA32_ILV:
			out_f = AV_PIX_FMT_RGBA;
			alpha_plane.reset(new u8[w * h]);
			break;
		case CELL_VDEC_PICFMT_UYVY422_ILV:
			out_f = AV_PIX_FMT_UYVY422;
			break;
		...
		default:
		{
			fmt::throw_exception("cellVdecGetPictureExt: Unknown formatType "
								 "(handle=0x%x, seq_id=%d, cmd_id=%d, type=%d)",
				handle, frame.seq_id, frame.cmd_id, type);
		}
		}
		...
		case AV_PIX_FMT_YUVJ420P:
			cellVdec.error("cellVdecGetPictureExt: experimental AVPixelFormat "
						   "(handle=0x%x, seq_id=%d, cmd_id=%d, format=%d). This may "
						   "cause suboptimal video quality.",
				handle, frame.seq_id, frame.cmd_id, frame->format);
			[[fallthrough]];
		case AV_PIX_FMT_YUV420P:
			in_f = alpha_plane ? AV_PIX_FMT_YUVA420P : static_cast<AVPixelFormat>(frame->format);
			break;
		default:
			fmt::throw_exception("cellVdecGetPictureExt: Unknown frame format (%d)",
				frame->format);
		}

		cellVdec.trace("cellVdecGetPictureExt: handle=0x%x, seq_id=%d, cmd_id=%d, "
					   "w=%d, h=%d, frameFormat=%d, formatType=%d, in_f=%d, "
					   "out_f=%d, alpha_plane=%d, alpha=%d, colorMatrixType=%d",
			handle, frame.seq_id, frame.cmd_id, w, h, frame->format,
			format->formatType, +in_f, +out_f, !!alpha_plane,
			format->alpha, format->colorMatrixType);

		vdec->sws = sws_getCachedContext(vdec->sws, w, h, in_f, w, h, out_f,
			SWS_POINT, nullptr, nullptr, nullptr);

		u8* in_data[4] = {frame->data[0], frame->data[1], frame->data[2],
			alpha_plane.get()};
		int in_line[4] = {frame->linesize[0], frame->linesize[1],
			frame->linesize[2], w * 1};
		...
		if (!alpha_plane)
		{
			...
			if (const int ret = av_image_fill_linesizes(out_line, out_f, w);
				ret < 0)
			{
				fmt::throw_exception(
					"cellVdecGetPictureExt: av_image_fill_linesizes failed "
					"(handle=0x%x, seq_id=%d, cmd_id=%d, ret=0x%x): %s",
					handle, frame.seq_id, frame.cmd_id, ret,
					utils::av_error_to_string(ret));
			}
		}
...
error_code cellVdecGetPicture(ppu_thread& ppu, u32 handle,
	vm::cptr<CellVdecPicFormat> format,
	vm::ptr<u8> outBuff)
{
	ppu.state += cpu_flag::wait;

	cellVdec.trace("cellVdecGetPicture(handle=0x%x, format=*0x%x, outBuff=*0x%x)",
		handle, format, outBuff);
...
error_code cellVdecGetPicItem(ppu_thread& ppu, u32 handle,
	vm::pptr<CellVdecPicItem> picItem)
{
	ppu.state += cpu_flag::wait;
	...
	struct all_info_t
	{
		...
		std::aligned_union_t<0, CellVdecAvcInfo, CellVdecDivxInfo,
			CellVdecMpeg2Info>
			picInfo;
	};
	...
	vm::ptr<CellVdecPicItem> info;
	{
		...
		for (auto& picture : vdec->out_queue)
		{
			if (!picture.pic_item_received)
			{
				...
				if (vdec->mem_bias + size_needed >=
					vdec->mem_size / size_needed * size_needed)
				{
					vdec->mem_bias = 0;
					break;
				}
	...
	if (!frame || seq_id != sequence_id)
	{
		...
		return {CELL_VDEC_ERROR_EMPTY, " frame=%d, sequence_id=%d, seq_id=%d",
	...
	if (vdec->type == CELL_VDEC_CODEC_TYPE_AVC)
	{
		...
		default:
		{
			...
			cellVdec.error("cellVdecGetPicItem(AVC): unknown pict_type value "
						   "(handle=0x%x, seq_id=%d, cmd_id=%d, pct=0x%x)",
				handle, seq_id, cmd_id, pct);
			break;
		}
		}
		...
		case CELL_VDEC_FRC_24000DIV1001:
			...
			break;
		case CELL_VDEC_FRC_24:
			avc->frameRateCode = CELL_VDEC_AVC_FRC_24;
			break;
		case CELL_VDEC_FRC_25:
			avc->frameRateCode = CELL_VDEC_AVC_FRC_25;
			break;
		case CELL_VDEC_FRC_30000DIV1001:
			avc->frameRateCode = CELL_VDEC_AVC_FRC_30000DIV1001;
			break;
		case CELL_VDEC_FRC_30:
			avc->frameRateCode = CELL_VDEC_AVC_FRC_30;
			break;
		case CELL_VDEC_FRC_50:
			avc->frameRateCode = CELL_VDEC_AVC_FRC_50;
			break;
		case CELL_VDEC_FRC_60000DIV1001:
			avc->frameRateCode = CELL_VDEC_AVC_FRC_60000DIV1001;
			break;
		case CELL_VDEC_FRC_60:
			avc->frameRateCode = CELL_VDEC_AVC_FRC_60;
			break;
		default:
			cellVdec.error("cellVdecGetPicItem(AVC): unknown frc value (handle=0x%x, "
						   "seq_id=%d, cmd_id=%d, frc=0x%x)",
				handle, seq_id, cmd_id, frc);
		}
	...
	else if (vdec->type == CELL_VDEC_CODEC_TYPE_DIVX)
	{
		...
		{
		case AV_PICTURE_TYPE_I:
			dvx->pictureType = CELL_VDEC_DIVX_VCT_I;
			break;
		case AV_PICTURE_TYPE_P:
			dvx->pictureType = CELL_VDEC_DIVX_VCT_P;
			break;
		case AV_PICTURE_TYPE_B:
			dvx->pictureType = CELL_VDEC_DIVX_VCT_B;
			break;
		default:
			cellVdec.error("cellVdecGetPicItem(DivX): unknown pict_type value "
						   "(handle=0x%x, seq_id=%d, cmd_id=%d, pct=0x%x)",
				handle, seq_id, cmd_id, pct);
		}
		...
		{
		case CELL_VDEC_FRC_24000DIV1001:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_24000DIV1001;
			break;
		case CELL_VDEC_FRC_24:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_24;
			break;
		case CELL_VDEC_FRC_25:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_25;
			break;
		case CELL_VDEC_FRC_30000DIV1001:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_30000DIV1001;
			break;
		case CELL_VDEC_FRC_30:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_30;
			break;
		case CELL_VDEC_FRC_50:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_50;
			break;
		case CELL_VDEC_FRC_60000DIV1001:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_60000DIV1001;
			break;
		case CELL_VDEC_FRC_60:
			dvx->frameRateCode = CELL_VDEC_DIVX_FRC_60;
			break;
		default:
			cellVdec.error("cellVdecGetPicItem(DivX): unknown frc value "
						   "(handle=0x%x, seq_id=%d, cmd_id=%d, frc=0x%x)",
				handle, seq_id, cmd_id, frc);
		}
	}
	else if (vdec->type == CELL_VDEC_CODEC_TYPE_MPEG2)
	{
		...
		{
		case CELL_VDEC_FRC_24000DIV1001:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_24000DIV1001;
			break;
		case CELL_VDEC_FRC_24:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_24;
			break;
		case CELL_VDEC_FRC_25:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_25;
			break;
		case CELL_VDEC_FRC_30000DIV1001:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_30000DIV1001;
			break;
		case CELL_VDEC_FRC_30:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_30;
			break;
		case CELL_VDEC_FRC_50:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_50;
			break;
		case CELL_VDEC_FRC_60000DIV1001:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_60000DIV1001;
			break;
		case CELL_VDEC_FRC_60:
			mp2->frame_rate_code = CELL_VDEC_MPEG2_FRC_60;
			break;
		default:
			cellVdec.error("cellVdecGetPicItem(MPEG2): unknown frc value "
						   "(handle=0x%x, seq_id=%d, cmd_id=%d, frc=0x%x)",
				handle, seq_id, cmd_id, frc);
		}
		...
		{
		case AV_PICTURE_TYPE_I:
			mp2->picture_coding_type[0] = CELL_VDEC_MPEG2_PCT_I;
			break;
		case AV_PICTURE_TYPE_P:
			mp2->picture_coding_type[0] = CELL_VDEC_MPEG2_PCT_P;
			break;
		case AV_PICTURE_TYPE_B:
			mp2->picture_coding_type[0] = CELL_VDEC_MPEG2_PCT_B;
			break;
		default:
			cellVdec.error("cellVdecGetPicItem(MPEG2): unknown pict_type value "
						   "(handle=0x%x, seq_id=%d, cmd_id=%d, pct=0x%x)",
				handle, seq_id, cmd_id, pct);
		}
...
{
	cellVdec.trace("cellVdecSetFrameRate(handle=0x%x, frameRateCode=0x%x)",
	...
	if (vdec->seq_state == sequence_state::closed ||
		vdec->seq_state >= sequence_state::invalid)
	{
		return {CELL_VDEC_ERROR_SEQ, vdec->seq_state.load()};
	}
	...
	const u64 seq_id = vdec->seq_id;
	const u64 cmd_id = vdec->next_cmd_id++;
	cellVdec.trace("Adding framerate cmd (handle=0x%x, seq_id=%d, cmd_id=%d)",
		handle, seq_id, cmd_id);

	vdec->in_cmd.push(
		vdec_cmd(vdec_cmd_type::framerate, seq_id, cmd_id, frameRateCode & 0x87));
	return CELL_OK;
}

error_code cellVdecOpenExt(ppu_thread& ppu, vm::cptr<CellVdecType> type,
	vm::cptr<CellVdecResourceExt> res,
	vm::cptr<CellVdecCb> cb, vm::ptr<u32> handle)
{
	cellVdec.warning(
		"cellVdecOpenExt(type=*0x%x, res=*0x%x, cb=*0x%x, handle=*0x%x)", type,
		res, cb, handle);
